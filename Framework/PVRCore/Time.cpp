...
{
#if defined(_WIN32)
	m_highResolutionSupported = QueryPerformanceFrequency((LARGE_INTEGER*)&m_timerFrequency) != 0;
	if (!m_highResolutionSupported)
	{
		...
		m_timerFrequency = 1000ull / 16ull;
	}
#elif defined(__APPLE__)
	m_timeBaseInfo = new mach_timebase_info_data_t;
	mach_timebase_info(m_timeBaseInfo);
	m_timerFrequency = (uint64)((1.0e9 * (float64)m_timeBaseInfo->numer) / (float64)m_timeBaseInfo->denom);
#elif !defined(__QNX__)
	timespec timerInfo;
	if (clock_getres(PVR_TIMER_CLOCK, &timerInfo) != 0)
	{
		m_timerFrequency = static_cast<uint64>(timerInfo.tv_sec);
	}
#endif
...
void Time::Reset()
{
	m_startTime = getCurrentTimeNanoSecs();
}

uint64	Time::getElapsedNanoSecs()
{
	return getCurrentTimeNanoSecs() - m_startTime;
}
...
uint64 Time::getCurrentTimeNanoSecs() const
{
	...
	if (m_highResolutionSupported)
	{
		...
		currentTime = (1000000000 * (currentTime - initialTime)) / m_timerFrequency;
	}
	else
	{
	...
	currentTime = static_cast<uint64>(time * (m_timeBaseInfo->numer / m_timeBaseInfo->denom));
#elif defined(__QNX__)
	timeval tv;
...
uint64 Time::getCurrentTimeSecs() const
{
	...
	if (m_highResolutionSupported)
	{
		uint64 currentTime;
		QueryPerformanceCounter((LARGE_INTEGER*)&currentTime);
		retTime = static_cast<float64>(m_timerFrequency);
	}
	else
	{
...
#elif defined(__APPLE__)
	uint64_t time = mach_absolute_time();
	retTime = static_cast<float64>(time * m_timerFrequency);
#elif defined(__QNX__)
