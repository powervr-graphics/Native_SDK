...
public:
	...
	struct Header
	{
		...
		Header() : flags(0), pixelFormat(0), colorSpace(ColorSpace::lRGB), channelType(VariableType::UnsignedByteNorm),
			height(1), width(1), depth(1), numSurfaces(1), numFaces(1), numMipMaps(1), metaDataSize(0)
		{}
	};
...
public:
	...
	TextureHeader(Header fileHeader, uint32_t numMetaData, TextureMetaData* metaData);
	...
	TextureHeader(PixelFormat pixelFormat, uint32_t width, uint32_t height, uint32_t depth = 1, uint32_t numMipMaps = 1,
	              ColorSpace colorSpace = ColorSpace::lRGB, VariableType channelType = VariableType::UnsignedByteNorm,
	              uint32_t numSurfaces = 1, uint32_t numFaces = 1, uint32_t flags = 0, TextureMetaData* metaData = NULL, uint32_t metaDataSize = 0);
