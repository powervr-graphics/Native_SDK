...
{
	//performance warning...
	bool operator()(const DynamicArray<T>& left, const DynamicArray<T>& right) const
	{
...
struct PipelineSet
{
	DynamicArray<StringHash> pipelines;
	bool operator<(const PipelineSet& rhs)const
	{
		return DynamicArrayComparator<StringHash>()(this->pipelines, rhs.pipelines);
	}
	PipelineSet() {}
...
////////// PIPELINE SELECTION ///////////
namespace {
inline std::pair<StringHash, api::effect::PipelineDef*> selectPipelineForSubpassMeshMaterial(
  api::effect::EffectApi& effect, const api::effect::ConditionalPipelines& pipes, const assets::Mesh& mesh, const assets::Material& material)
{
	...
	for (; cond_pipe_it != pipes.pipelines.end(); ++cond_pipe_it)
	{
		...
		for (auto condition = cond_pipe_it->conditions.begin(); condition != cond_pipe_it->conditions.end() && !incompatible; ++condition)
		{
			...
			{
			case api::effect::PipelineCondition::AttributeRequired:
				incompatible = mesh.getVertexAttributeByName(condition->value) == 0;
				break;
			case api::effect::PipelineCondition::AttributeRequiredNo:
				incompatible = mesh.getVertexAttributeByName(condition->value) != 0;
				break;
			case api::effect::PipelineCondition::UniformRequired:
				incompatible = !material.hasSemantic(condition->value);
				break;
			case api::effect::PipelineCondition::UniformRequiredNo:
				incompatible = material.hasSemantic(condition->value);
				break;
			}
		}
	...
	{
		return std::make_pair(cond_pipe_it->pipeline, effect->getPipelineDefinition(cond_pipe_it->pipeline));
	}


	Log("failed to find a compatible pipeline for mesh a mesh");
	return std::make_pair(StringHash(), (api::effect::PipelineDef*)NULL);
}

inline std::pair<StringHash, const api::effect::PipelineDef*> selectPipelineForSubpassMeshMaterial(
  api::effect::EffectApi& effect, uint32 passId, uint32 subpassId, const assets::Mesh& mesh, const assets::Material& material)
{
	auto& pass = effect->getPass(passId);
	return selectPipelineForSubpassMeshMaterial(effect, pass.subpasses[subpassId], mesh, material);
}
}
...
inline bool createVbos(RenderManager& renderman, const std::map<assets::Mesh*, AttributeConfiguration*>& meshAttribConfig)
{
	...
	for (uint32 model_id = 0; model_id < apiModels.size(); ++model_id)
	{
		for (uint32 mesh_id = 0; mesh_id < apiModels[model_id].assetModel->getNumMeshes(); ++mesh_id)
		{
			...
			apimesh.vbos.resize(attribConfig.size());
			for (uint32 vbo_id = 0; vbo_id < attribConfig.size(); ++vbo_id)
			{
...
inline std::vector<StringHash> getVertexBindingsForPipeline(const api::effect::EffectApi& effect, const StringHash& pipelineName)
{
...
inline std::vector<StringHash> getAllActiveVertexAttributesForMeshAndEffect(api::effect::EffectApi& effect,
    const assets::Mesh& mesh, const assets::Material& material)
{
	...
	for (uint32 pass = 0; pass < effect->getNumPasses(); ++pass)
	{
		...
		{
			std::pair<StringHash, const api::effect::PipelineDef*> pipe = selectPipelineForSubpassMeshMaterial(effect, pass, subpass, mesh, material);
...
inline AttributeConfiguration getVertexBindingsForPipeNoStride(const api::effect::EffectApi& effect, const StringHash& pipelineName)
{
...
inline AttributeConfiguration getVertexBindingsForPipe(const api::effect::EffectApi& effect, const StringHash& pipelineName)
{
	AttributeConfiguration retval;
	auto& pipe = effect->getEffectAsset().versionedPipelines.find(effect->getApiString())->second.find(pipelineName)->second;
	uint32 count[32] = {};
...
inline void getUniformLocationsForNodes(RendermanSubpass& subpass)
{
	for (auto& model : subpass.subpassModels)
                             ...
                             std::map<StringHash, BufferEntrySemantic>& bufferEntries, bool checkDuplicates)
{
	...
	for (size_t i = 0; i < list.size(); ++i)
	{
		...
		auto it = bufferEntries.find(list[i].name);
		...
		{
			auto& newSemanticEntry = bufferEntries[list[i].name];
...
inline void addUniformSemanticLists(pvr::ContiguousMap<StringHash, api::effect::UniformSemantic>& effectlist,
                                    ...
                                    types::VariableScope scope)
{
	for (auto& uniform : effectlist)
	{
		if (uniform.second.scope == scope)
		{
			...
			//memset(newUniform.memory.raw(), 0, types::GpuDatatypes::getSize(newUniform.memory.dataType_, uniform.second.arrayElements));
		}
	}
}
}
...
inline bool createPipelines(RenderManager& renderman, const std::map<StringHash, AttributeConfiguration*>& vertexConfigs)
{
	...
	for (auto && renderman_effect : renderstruct.effects)
	{
		for (auto && pass_effect : renderman_effect.passes)
		{
			...
			{
				for (auto && pipeline_effect : subpass_effect.pipelines)
				{
				...
				getUniformLocationsForNodes(subpass_effect);
			}
		}
	}
	return succeed;
...
inline bool createDescriptorSetsAndBuffers(RenderManager& renderman, const std::map<assets::Mesh*, AttributeConfiguration*>& meshAttribConfig, api::DescriptorPool& pool)
{
	GraphicsContext& ctx = renderman.getContext();
	RendermanStructure& renderstruct = renderman.renderObjects();
	uint16 log_effect = 0;
	for (auto& renderman_effect : renderstruct.effects)
	{
		...
		for (RendermanBufferDefinition& bufdef : renderman_effect.bufferDefinitions) //The buffer is created per model.
		{
			...
			if (bufdef.numDynamicClients > 1)
			{
				bufdef.buffer.setupDynamic(ctx, bufdef.numDynamicClients, bufdef.allSupportedBindings);
			}
			for (uint32 swapIdx = 0; swapIdx < bufdef.numBuffers; ++swapIdx)
			{

				bufdef.buffer.createConnectedBuffer(swapIdx, ctx, types::bufferViewTypeToBufferBindingUse(bufdef.allSupportedBindings),
				                                    static_cast<pvr::uint32>(bufdef.allSupportedBindings & types::BufferViewTypes::StorageBufferDynamic) != 0 ||
				                                    static_cast<pvr::uint32>(bufdef.allSupportedBindings & types::BufferViewTypes::UniformBufferDynamic) != 0);
			}
		}
		uint16 log_pass = 0;
		for (auto& pass : renderman_effect.passes)
		{
			...
			for (auto& subpass : pass.subpasses)
			{
				...
				for (auto& modeleffect : subpass.subpassModels)
				{
					for (auto& materialeffect : modeleffect.materialeffects)
					{
						for (auto& materialpipeline : materialeffect.materialSubpassPipelines)
						{
							if (materialpipeline.pipeline_ == NULL || !materialpipeline.pipeline_->apiPipeline.isValid()) { continue; }
							...
							for (uint16 set_id = 0; set_id < set_max + 1; ++set_id)
							{
								if (pipedef.descSetExists[set_id])
								{
									...
									else
									{
										...
										assertion(descsetlayout.isValid(), strings::createFormatted(
							...
							for (auto& tex : pipedef.textureSamplersByTexSemantic)
							{
								...
								if (pipedef.descSetIsFixed[tex.second.set])
								{
									debug_assertion(!pipedef.descSetIsFixed[tex.second.set], strings::createFormatted(
									                  "RenderManager::createAll() Creating descriptor sets: Descriptor set was referenced but a semantic, but was marked FIXED "
									                  "Effect[%s] Pass[%d] Spass[%d] Pipeline[%s] Model[%d] Material[%d] Set[%d] TextureSemantic[%s].",
									                  log_effect, log_pass, log_subpass, pipeline.name.c_str(), log_model, log_material, tex.second.set, tex.first.c_str()));
								}
								int32 texIndex;
								if ((texIndex = materialeffect.material->assetMaterial->getTextureIndex(tex.first)) != -1)
								{
									const StringHash& textureName = modeleffect.renderModel_->assetModel->getTexture(texIndex).getName();

									api::TextureView view;
									effect->getAssetLoadingDelegate()->effectOnLoadTexture(textureName, view);
							...
							for (auto& bufentry : materialpipeline.pipeline_->bufferBindings)
							{
							...
							for (uint32 setid = 0; setid < 4; ++setid)
							{
								if (pipedef.descSetExists[setid])
								{
									...
									assertion(!(pipedef.descSetIsMultibuffered[setid] && materialpipeline.sets[setid].size() < swaplength));
...
namespace {
inline RendermanBufferBinding makeRendermanBufferBinding(const StringHash& name, const api::effect::BufferRef& ref,
...
size_t addRendermanPipelineIfNotExists(RendermanSubpass& subpass, api::effect::PipelineDef* pipedef, RendermanEffect& renderEffect, bool& isnew)
{
	//Comparator for the next find
	struct cmppipe
	{
		api::effect::PipelineDef* ptr;
		cmppipe(api::effect::PipelineDef* ptr) : ptr(ptr) {}
		bool operator()(const RendermanPipeline& rhs) { return rhs.pipelineInfo == ptr; }
	};

	auto& container = subpass.pipelines;
...
template<typename Container>
inline size_t addRendermanModelEffectIfNotExists(Container& container, RendermanModel* model, RendermanSubpass* subpass, bool& isnew)
{
	//Comparator for the next find
	struct cmppipe
	{
		...
		bool operator()(const RendermanSubpassModel& rhs) { return rhs.renderModel_ == ptr; }
	};
	...
	else
	{
		...
		container.push_back(RendermanSubpassModel());
		container.back().renderModel_ = model;
		container.back().renderSubpass_ = subpass;
...
inline void addBufferDefinitions(RendermanEffect& renderEffect, api::effect::EffectApi& effect)
{
...
void addNodeDynamicClientToBuffers(RendermanSubpassModel& model, RendermanNode& node, RendermanPipeline& pipeline)
{
                                  ...
                                  std::map<PipelineSet, AttributeConfiguration>& pipeSets)
{
	...
	for (auto effect_it = renderStructure.effects.begin(); effect_it != renderStructure.effects.end(); ++effect_it)
	{
		// PHASE 1: Select combinations
		//a) What are all the distinct combinations of pipelines used? (Use to create the VBO layouts)

		auto& effectapi = effect_it->effect;

		addBufferDefinitions(*effect_it, effectapi);

		for (uint8 passId = 0; passId < effectapi->getNumPasses(); ++passId)
		{
			auto& effectpass = effectapi->getPasses()[passId];
			auto& renderingpass = effect_it->passes[passId];
			renderingpass.renderEffect_ = &*effect_it;

			for (uint8 subpassId = 0; subpassId < effectpass.subpasses.size(); ++subpassId)
			{
				auto& effectsubpass = effectpass.subpasses[subpassId];
				auto& rendersubpass = renderingpass.subpasses[subpassId];
				rendersubpass.renderingPass_ = &renderingpass;


				//CREATE THE RENDERNODE BY SELECTING THE PIPELINE FOR EACH MODEL NODE
				auto& allmodels = effect_it->passes[passId].subpasses[subpassId].allModels;
				for (uint32 modelId = 0; modelId < allmodels.size(); ++modelId)
				{
					auto& rendermodel = *allmodels[modelId];
					...
					for (uint32 nodeId = 0; nodeId < rendermodel.assetModel->getNumMeshNodes(); ++nodeId)
					{
						...
						std::pair<StringHash, api::effect::PipelineDef*> pipe = selectPipelineForSubpassMeshMaterial(effectapi, effectsubpass, assetmesh, assetmaterial);
						if (!pipe.second) { continue; }
						//Add this pipeline as one used by this mesh. Needed? YES, because meshes are NOT arranged with their pipes - nodes are.
						//We need this so that if a mesh is rendered by different pipes (Think of a scene with a glass sphere and a wood sphere...)
						...
						//The structure is populated for now, not finalized. The actual API objects are created later. For now, only the connections between objects
						//are necessary.
						bool isNew = true;
						bool isPipeNew = true;

						size_t pipe_index = addRendermanPipelineIfNotExists(rendersubpass, pipe.second, *effect_it, isPipeNew);

						RendermanPipeline& renderpipe = rendersubpass.pipelines[pipe_index];

						renderpipe.name = pipe.first;
						renderpipe.pipelineInfo = pipe.second;

						size_t model_index = addRendermanModelEffectIfNotExists(rendersubpass.subpassModels, &rendermodel, &rendersubpass, isNew);

						RendermanSubpassModel& rendermodeleffect = rendersubpass.subpassModels[model_index];
						...
						for (uint32 batch_id = 0; batch_id < assetmesh.getNumBoneBatches(); ++batch_id)
						{
	...
	for (auto& mesh : setOfAllPipesUsedPerMesh)
	{
		meshAttributeLayout[mesh.first] = &pipeSets[mesh.second];
	}
}
}
		...
		for (auto& pass : effect.passes)
		{
			for (auto& subpass : pass.subpasses)
			{
				for (auto& modeleffect : subpass.subpassModels)
				{
					...
					{
						//uint32 dynamicoffsetids[4] = { 0, 0, 0, 0 };
						//int32 assert_last_binding[4] = { -1, -1, -1, -1 };
						for (uint32 setid = 0; setid < 4; ++setid)
						{
...
inline bool getPerspectiveMatrix(TypedMem& memory, const RendermanModel& rmodel, uint32 cameraIndex)
{
    ...
    if(isRotated)
    {
        memory.setValue(pvr::math::perspective(ctx->getApiType(), fov,
                                               (pvr::float32)attribs.height / attribs.width, nearClip, farClip,
                                                glm::pi<pvr::float32>() * .5f));
    }
    else
    {
        memory.setValue(pvr::math::perspective(ctx->getApiType(), fov,
                                               (pvr::float32)attribs.width / attribs.height, nearClip, farClip));
    }
	return true;
}
...
inline bool getModelViewProjectionMatrix(TypedMem& mem, const RendermanNode& node)
{
	...
	node.subpassMesh_->rendermesh_->renderModel_->getModelSemantic(VIEWPROJECTIONMATRIX_STR, viewprojmtx);
...
void RendermanEffect::recordRenderingCommandsNoBeginEndRenderpass(api::CommandBuffer& cbuff, uint16 swapIdx, bool recordUpdateUniforms)
{
	recordRenderingCommands(cbuff, swapIdx, false, recordUpdateUniforms);
}
...
void RendermanPass::recordRenderingCommands(api::CommandBuffer& cbuff, uint16 swapIdx, bool beginEndRendermanPass, bool recordUpdateUniforms)
{
	...
	if (beginEndRendermanPass)
	{
		...
		{
			for (uint32 j = 0; j < subpasses[i].allModels.size(); ++j)
			{
				if (subpasses[i].allModels[j])
				{
					memcpy(&clearColor, subpasses[i].allModels[j]->assetModel->getInternalData().clearColor,
					       sizeof(subpasses[i].allModels[j]->assetModel->getInternalData().clearColor));
					i = static_cast<pvr::uint32>(subpasses.size());// break the outer loop as well.
					break;
				}
			}
		}
...
void RendermanSubpass::recordRenderingCommands(api::CommandBuffer& cbuff, uint16 swapIdx, bool beginWithNextSubpassCommand, bool recordUpdateUniforms)
{
...
void RendermanSubpass::recordRenderingCommands(api::CommandBufferBase cbuff, uint16 swapIdx, bool recordUpdateUniforms)
{
	for (auto& spmodels : subpassModels)
	{
		spmodels.recordRenderingCommands(cbuff, swapIdx, recordUpdateUniforms);
	}
}

void RendermanSubpassModel::recordRenderingCommands(api::CommandBufferBase cbuff, uint16 swapIdx, bool recordUpdateUniforms)
{
	api::DescriptorSet::ElementType* prev_sets[4];

	bool bindSets[4] = { true, true, true, true };

	uint32* dynamicOffsets[4] { NULL, NULL, NULL, NULL };
...
void RendermanNode::recordRenderingCommands(api::CommandBufferBase cbuff, uint16 swapidx, bool recordBindPipeline,
    bool* recordBindDescriptorSets, bool recordUpdateUniforms,
    bool recordBindVboIbo, bool recordDrawCalls)
{
	...
	if (recordBindVboIbo)
	{
		...
		cbuff->bindIndexBuffer(rmesh.ibo, 0, rmesh.indexType);
	}
	if (recordUpdateUniforms)
	{
		if (recordBindPipeline)
		{
			...
			for (auto& uniform : pipe.toRendermanEffect().uniformSemantics)
			{
				recordUpdateUniformSemanticToExternalMemory(cbuff, uniform.second.uniformLocation, uniform.second.memory);
			}
		}
