...
#include "PVRCore/DataStructures/ListOfInterfaces.h"
...
class CommandBufferImplGles_ : public ICommandBufferImpl_, public native::HCommandBuffer_
{
public:
	...
	CommandQueue _queue;
	CommandBufferBase_* _myowner;
	CommandBufferImplGles_(const GraphicsContext& context, CommandPool pool) : ICommandBufferImpl_(context, pool), _queue(1024)
	{
	}

	~CommandBufferImplGles_();

	void valdidateRecordState()
	{
#ifdef DEBUG
		if (!_isRecording)
		{
	...
	{
		valdidateRecordState();

		_queue.emplace_back<ClassType>(obj);
#ifdef DEBUG
		_queue.last()->interfacePtr->debug_commandCallSiteStackTrace = getStackTraceInfo(2);
#endif
	}
	...
	void bindDescriptorSet_(const api::PipelineLayout& pipelineLayout, uint32 index, const DescriptorSet& set, const uint32* dynamicOffsets = NULL, uint32 numDynamicOffsets = 0);
	void bindDescriptorSetCompute_(const api::PipelineLayout& pipelineLayout, uint32 index, const DescriptorSet& set, const uint32* dynamicOffsets = NULL, uint32 numDynamicOffsets = 0);
	...
	void beginRecording_()
	{
		if (_isRecording)
		{
			...
			debug_assertion(0, "Called CommandBuffer::beginRecording while a recording was already in progress. Call CommandBuffer::endRecording first");
		}
		_queue.clear();
		_isRecording = true;
	}
...
#undef SET_UNIFORM_DECLARATION
	...
	void submitSecondary(CommandBufferBase_& cmdBuf)
	{
