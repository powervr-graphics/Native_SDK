...
namespace pvrvk {
template<typename> class EmbeddedRefCountedResource;
template<typename> class RefCountedResource;
template<typename> class RefCountedWeakReference;
template<typename> class EmbeddedRefCount;
...
private:
	...
	bool is_deleting_; //!< This flags protects from recursive deletion : If the last weak ref to an object is released during its own destroyObject
...
protected:
	void increment_count()
	{
		...
		{
			std::lock_guard<std::recursive_mutex> lockguard(mylock);
			PVR_REFCOUNTED_ASSERT(count_.load() != 0 && "RefCounted::increment_count:  Tried to increment the count of an object but it had already been destroyed!");
		}
	...
	void increment_weakcount()
	{
		...
		{
			std::lock_guard<std::recursive_mutex> lockguard(mylock);
			PVR_REFCOUNTED_ASSERT(weakcount_.load() != 0 && "RefCounted::increment_weakcount:  Tried to increment the count of an object but it had already been destroyed!");
		}
	...
	void decrement_count()
	{
		...
		int32_t cnt = 0;
		{
			...
			{
				std::lock_guard<std::recursive_mutex> lockguard(mylock);
				is_deleting_ = true; // This flag tests notifies the decrement_weakcount that the object is deleting itself
				destroyObject();
				is_deleting_ = false;
				if (weakcount() == 0)
				{
	...
	void decrement_weakcount()
	{
		...
		{
			std::lock_guard<std::recursive_mutex> lockguard(mylock);
			if (count_.load() == 0 && !is_deleting_) // Test to the is_deleting means that decrement_count will actually delete the entry
			{
				deleter = true;
			}
		}
	...
	IRefCountEntry() : count_(1), weakcount_(0), is_deleting_(false) {}
};
...
public:
	...
	EmbeddedRefCountedResource(const EmbeddedRefCountedResource& rhs) : Dereferenceable<MyClass_>(rhs), RefcountEntryHolder<MyClass_>(rhs)
	{
		if (RefcountEntryHolder<MyClass_>::refCountEntry)
		{
			...
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::refCountEntry->count() >= 0 &&
	...
	{
		if (rhs.refCountEntry)
		{
			rhs.refCountEntry->increment_count();
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::refCountEntry->weakcount() > 0 &&
				"Reference count was not positive");
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::refCountEntry->count() > 0 &&
				"Reference count was not positive");
		}
	}
	...
	{
		swap(rhs);
		PVR_REFCOUNTED_ASSERT((!RefcountEntryHolder<MyClass_>::refCountEntry ||
		                       (RefcountEntryHolder<MyClass_>::refCountEntry->count() >= 0 &&
		                        RefcountEntryHolder<MyClass_>::refCountEntry->weakcount() >= 0)) &&
		                      "BUG - Count was negative.");
		return *this;
...
public:
	...
	{
		RefcountEntryHolder<MyClass_>::refCountEntry->_count = 1;
	}
...
public:
	...
	RefCountedWeakReference(const RefCountedWeakReference& rhs) : Dereferenceable<MyClass_>(rhs), RefcountEntryHolder<MyClass_>(rhs)
	{
		if (RefcountEntryHolder<MyClass_>::refCountEntry)
		{
			...
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::refCountEntry->weakcount() > 0 &&
			                      "BUG - Reference count was nonpositive.");
		}
	}
	...
	/// is not used.</remarks>
	template<class OldType_>
	RefCountedWeakReference(const EmbeddedRefCountedResource<OldType_>& rhs,
	                        typename std::enable_if<std::is_convertible<OldType_*, MyClass_*>::value, void*>::type* = 0)
		: Dereferenceable<MyClass_>(rhs._pointee), RefcountEntryHolder<MyClass_>(rhs.refCountEntry)
	{
		//Compile-time check: are these types compatible?
		...
		{
			RefcountEntryHolder<MyClass_>::refCountEntry->increment_weakcount();
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::refCountEntry->weakcount() > 0 && "Reference count was not positive");
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::refCountEntry->count() > 0 && "Reference count was not positive");
		}
	}
	...
	/// is not used.</remarks>
	template<class OldType_>
	explicit RefCountedWeakReference(const EmbeddedRefCountedResource<OldType_>& rhs,
	                                 typename std::enable_if < !std::is_convertible<OldType_*, MyClass_*>::value, void* >::type* = 0)
		: Dereferenceable<MyClass_>((MyClass_*)(rhs._pointee)), RefcountEntryHolder<MyClass_>(rhs.refCountEntry)
	{
		//Compile-time check: are these types compatible?
		OldType_* checkMe = static_cast<MyClass_*>(Dereferenceable<MyClass_>::_pointee);
		(void)checkMe;
