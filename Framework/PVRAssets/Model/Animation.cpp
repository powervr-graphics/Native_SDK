...
glm::mat4x4 Animation::getTranslationMatrix(uint32 frame, float32 interp) const
{
	if (m_data.positions.size())
	{
		...
		{
			PVR_ASSERT(frame + 1 < m_data.numberOfFrames);
...
glm::mat4x4 Animation::getTransformationMatrix(uint32 frame, float32 interp) const
{
	if (m_data.matrices.size())
	{
		...
		{
			PVR_ASSERT(frame < m_data.numberOfFrames);
			uint32 index;
		...
		{
			internal::optimisedMat4 m1(getTranslationMatrix(frame, interp));
			internal::optimisedMat4 m2(getRotationMatrix(frame, interp));
			internal::optimisedMat4 m3(getScalingMatrix(frame, interp));
			internal::optimisedMat4 m4(m1 * m2 * m3);
...
bool Animation::setPositions(uint32 numFrames, const float32* const data, const uint32* const indices)
{
	...
	if (numFrames > 1 && m_data.flags && numFrames != m_data.numberOfFrames)
	{
		...
		PVR_ASSERT(0 && "Wrongly sized data passed to Animation::setPositions. The data must correspond to the number of frames.");
		return false;
	}
	if (!data || !numFrames)
	{
...
bool Animation::setRotations(uint32 numFrames, const float32* const data, const uint32* const indices)
{
	...
	if (numFrames > 1 && m_data.flags && numFrames != m_data.numberOfFrames)
	{
		...
		PVR_ASSERT(0 && "Wrongly sized data passed to Animation::setRotations. The data must correspond to the number of frames.");
		return false;
	}
	if (!data || !numFrames)
	{
...
bool Animation::setScales(uint32 numFrames, const float32* const data, const uint32* const indices)
{
	...
	if (numFrames > 1 && m_data.flags && numFrames != m_data.numberOfFrames)
	{
		...
		PVR_ASSERT(0 && "Wrongly sized data passed to Animation::setScales. The data must correspond to the number of frames.");
		return false;
	}
	if (!data || !numFrames)
	{
...
bool Animation::setMatrices(uint32 numFrames, const float32* const data, const uint32* const indices)
{
	...
	if (numFrames > 1 && m_data.flags && numFrames != m_data.numberOfFrames)
	{
		...
		PVR_ASSERT(0 && "Wrongly sized datapassed to Animation::setMatrices. The data must correspond to the number of frames.");
		return false;
	}
	if (!data || !numFrames)
	{
