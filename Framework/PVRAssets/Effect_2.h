...
struct TextureDefinition : public NameComparable<TextureDefinition>
{
	...
	TextureDefinition(const StringHash& name, const StringHash& path, uint32_t width, uint32_t height,
	                  const ImageDataFormat& fmt) :
...
struct TextureRef
{
	...
	TextureRef(StringHash textureName, uint8_t set, uint8_t binding, StringHash variableName):
...
struct Shader : public NameComparable<Shader>
{
	...
	Shader(StringHash&& name, ShaderType type, std::string&& source) :
...
struct BufferDefinition : public NameComparable<BufferDefinition>
{
	...
	BufferDefinition() : allSupportedBindings(BufferUsageFlags(0)), isDynamic(false),
		scope(VariableScope::Effect), multibuffering(0) {}
};
...
struct PipelineDefinition: public NameComparable<PipelineDefinition>
{
	...
	PipelineDefinition() : enableDepthTest(false), enableDepthWrite(true),
		depthCmpFunc(CompareOp::Less), enableStencilTest(false),
		windingOrder(PolygonWindingOrder::FrontFaceCCW), cullFace(Face::None) {}
};
...
struct PipelineReference
{
	...
	DynamicArray<PipelineCondition> conditions; ///< The conditions of the pipeline. Used to select different pipelines for different objects
	DynamicArray<StringHash> identifiers; ///< Any custom semantic identifiers that a pipeline reference may have exported with the conditions
};
...
struct SubpassGroup
{
	...
	DynamicArray<PipelineReference> pipelines; ///< The group of pipeline references (pipelines with their selection criteria)
};
...
struct Effect : public Asset<Effect>
{
	...
	pvr::ContiguousMap<StringHash, std::string> headerAttributes; ///< Free name value pairs provided in the header of the effect

	pvr::ContiguousMap<StringHash, pvr::ContiguousMap<StringHash, Shader>> versionedShaders; ///< Lists of shaders along with their corresponding apis
	pvr::ContiguousMap<StringHash, pvr::ContiguousMap<StringHash, PipelineDefinition>> versionedPipelines; ///< Lists of pipelines, along with their corresponding apis that they apply to

	pvr::ContiguousMap<StringHash, TextureDefinition> textures; ///< All the (possible) textures defined for this effect
	pvr::ContiguousMap<StringHash, BufferDefinition> buffers; ///< All the (possible) buffers defined for this effect
