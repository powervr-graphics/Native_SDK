...
void Volume::findOrCreateTriangle(const glm::vec3& v0, const glm::vec3& v1,
                                  const glm::vec3& v2)
{
	...
	if (alreadyExisted[0] && alreadyExisted[1] && alreadyExisted[2])
	{
		for (uint32_t i = 0; i < _volumeMesh.numTriangles; ++i)
		{
			if ((_volumeMesh.triangles[i].edgeIndices[0] == edgeIndex0 || _volumeMesh.triangles[i].edgeIndices[0] == edgeIndex1
			    ...
			    (_volumeMesh.triangles[i].edgeIndices[2] == edgeIndex0 || _volumeMesh.triangles[i].edgeIndices[2] == edgeIndex1
			     || _volumeMesh.triangles[i].edgeIndices[2] == edgeIndex2))
			{
...
bool Volume::init(const assets::Mesh& mesh)
{
	...
	return init(static_cast<const uint8_t*>(mesh.getData(posIdx)), mesh.getNumVertices(), mesh.getStride(posIdx),
	            positions->getVertexLayout().dataType, faceData.getData(), mesh.getNumFaces(), faceData.getDataType());
}

bool Volume::init(const uint8_t* const data, uint32_t numVertices,
                  uint32_t verticesStride, DataType vertexType, const uint8_t* const faceData,
