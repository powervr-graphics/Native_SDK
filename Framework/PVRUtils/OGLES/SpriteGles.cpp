...
MatrixGroup_)
\file PVRUtils/OGLES/SpriteGles.cpp
...
void Image_::calculateMvp(uint64_t parentIds, glm::mat4 const& srt, const glm::mat4& viewProj,
...
{
	debugLogApiError("Image_::onRender Enter");
	...
	{
		debugLogApiError("Image_::onRender bind texture");
	...
	if (!_uiRenderer->isEs2())
	{
		_uiRenderer->_uiStateTracker.sampler7 = getSampler();
		gl::BindSampler(7, getSampler());
		debugLogApiError("Image_::onRender bind sampler");
	}

	GLuint vbo = _uiRenderer->getImageVbo();
	debugLogApiError("Image_::onRender getImageVbo");
	_uiRenderer->_uiStateTracker.vbo = vbo;
	gl::BindBuffer(GL_ARRAY_BUFFER, vbo);
	debugLogApiError("Image_::onRender bind vbo");
	...
	debugLogApiError("Image_::onRender vertexattribptr 0");
	...
	debugLogApiError("Image_::onRender vertexattribptr 1");
	...
	debugLogApiError("Image_::onRender uniform 3");

	gl::DrawArrays(GL_TRIANGLES, 0, 6);
	debugLogApiError("Image_::onRender draw");
}
	...
	Sprite_(uiRenderer), _texW(width), _texH(height), _texture(texture), _isTextureDirty(true), _sampler(sampler)
{
	...
	if (!uiRenderer.isEs2() && _sampler != 0)
	{
...
bool Font_::init(UIRenderer& uiRenderer, const GLuint& tex2D, const Texture& tex, const GLuint& sampler)
{
	...
	return (!loadFontData(tex));
}

uint32_t TextElement_::updateVertices(float fZPos, float xPos, float yPos, const std::vector<uint32_t>& text,
...
{
	debugLogApiError("TextElement_::updateVbo enter");
	if (_vertices.size())
	{
		if (!_vboCreated)
		{
		...
		gl::BufferData(GL_ARRAY_BUFFER, (static_cast<uint32_t>(sizeof(Vertex) * _vertices.size())),
	...
	debugLogApiError("TextElement_::updateVbo exit");
}
                               ...
                               pvr::Rectanglei const& viewport) const
{
	...
	for (ChildContainer::iterator it = _children.begin(); it != _children.end(); ++it)
	{
		(*it)->calculateMvp(packId(parentIds, _id), tmpMatrix, viewProj,
		                    Rectanglei(0, 0, static_cast<int32_t>(_boundingRect.getSize().x), static_cast<int32_t>(_boundingRect.getSize().y)));
	}
}
