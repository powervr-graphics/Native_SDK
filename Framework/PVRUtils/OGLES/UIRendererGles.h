...
\file PVRUtils/OGLES/UIRendererGles.h
\author PowerVR by Imagination, Developer Technology Team
\copyright Copyright (c) Imagination Technologies Limited.
*/
#pragma once
#include "PVRUtils/OGLES/SpriteGles.h"
...
struct GLState
{
	...
	/// <param name="isEs2">Specifies whether OpenGL ES 2.0 is being used as this affects what state must be checked.</param>
	void storeCurrentGlState(bool isEs2);
	...
	GLState() :
		...
		blendEqationAlpha(GL_FUNC_ADD),
		blendEqationRgb(GL_FUNC_ADD),
		...
		vbo(-1),
		vao(-1),
		vertexAttribArray(8, GL_FALSE),
		vertexAttribBindings(8, -1),
		vertexAttribSizes(8, -1),
		vertexAttribTypes(8, -1),
		vertexAttribNormalized(8, -1),
		vertexAttribStride(8, -1),
...
struct GLStateTracker : public GLState
{
	...
	/// <summary>Sets the OpenGL ES state based on what state must be changed from the current OpenGL ES state to the states required for rendering the UIRenderer and its sprites.</summary>
	/// <param name="isEs2">Specifies whether OpenGL ES 2.0 is being used as this affects what state must be changed.</param>
	void setUiState(bool isEs2);
	...
	/// <param name="currentGlState">Specifies whether OpenGL ES state which should be checked for differences with the current GLState object to determine what state has been modified.</param>
	void checkStateChanged(const GLState& currentGlState);

	/// <summary>Uses the provided GLStateTracker to blindly determine what state has been changed. Usage of this function requires full tracking of the OpenGL ES state as no state will be checked but will be taken as fact from stateTracker.</summary>
	/// <param name="stateTracker">Specifies what OpenGL ES state has been changed.</param>
	...
	/// <param name="isEs2">Specifies whether OpenGL ES 2.0 is being used as this affects what state must be changed.</param>
	void restoreState(const GLState& currentGlState, bool isEs2);

	/// <summary>Default constructor for GLStateTracker which initialises the OpenGL ES state tracking.</summary>
	GLStateTracker() :
		activeProgramChanged(GL_FALSE),
		activeTextureUnitChanged(GL_FALSE),
		boundTextureChanged(GL_FALSE),
		blendEnabledChanged(GL_FALSE),
		blendSrcRgbChanged(GL_FALSE),
		blendDstRgbChanged(GL_FALSE),
		blendDstAlphaChanged(GL_FALSE),
		blendEqationRgbChanged(GL_FALSE),
		blendEqationAlphaChanged(GL_FALSE),
		colorMaskChanged(GL_FALSE),
		depthTestChanged(GL_FALSE),
		depthMaskChanged(GL_FALSE),
		stencilTestChanged(GL_FALSE),
		cullingEnabledChanged(GL_FALSE),
		cullingChanged(GL_FALSE),
		windingOrderChanged(GL_FALSE),
		sampler7Changed(GL_FALSE),
		vboChanged(GL_FALSE),
		iboChanged(GL_FALSE),
		vaoChanged(GL_FALSE),
		vertexAttribArrayChanged(8, GL_FALSE),
...
public:
	...
	UIRenderer() : _screenRotation(.0f), _program(0), _fontIboCreated(false), _imageVboCreated(false),
		_samplerBilinearCreated(false), _samplerTrilinearCreated(false),
		_samplerBilinear(-1), _samplerTrilinear(-1), _fontIbo(-1), _imageVbo(-1) {}
	...
	{
		_program = std::move(rhs._program); rhs._program = 0;
	...
	/// <param name="isEs2">Indicates whether the UIRenderer must limit itself to using only OpenGL ES 2.0 functionality.</param>
	/// <returns>True indicating the result of initialising the UIRenderer was successful otherwise False.</returns>
	bool init(uint32_t width, uint32_t height, bool fullscreen, bool isEs2 = false);
	...
	/// <summary>returns whether the UIRenderer is OpenGL ES 2.0 only</summary>
	/// <returns>True if the UIRenderer is using OpenGL ES 2.0</returns>
	bool isEs2() { return _isEs2; }
