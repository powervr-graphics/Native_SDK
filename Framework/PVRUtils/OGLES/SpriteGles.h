...
\file PVRUtils/OGLES/SpriteGles.h
...
private:
	...
	bool init(UIRenderer& uiRenderer, const GLuint& tex2D, const Texture& tex, const GLuint& sampler);
...
private:
	...
	TextElement_(UIRenderer& uiRenderer, const Font& font) : _uiRenderer(&uiRenderer), _font(font), _isTextDirty(false), _vboCreated(false), _vbo(-1) {}
	...
	TextElement_(UIRenderer& uiRenderer, const std::string& str, const Font& font) : _uiRenderer(&uiRenderer),
		_font(font), _vboCreated(false), _vbo(-1)
	{
		setText(str);
		updateText();
	}
...
private:
	...
	void calculateMvp(uint64_t parentIds, glm::mat4 const& srt, const glm::mat4& viewProjection, pvr::Rectanglei const& viewport)const
	{
		...
		{
			(*it)->calculateMvp(packId(parentIds, _id), tmpMatrix, viewProjection, viewport);
		}
	}
	...
	virtual void onRender(uint64_t parentId) const
	{
		for (ChildContainer::iterator it = _children.begin(); it != _children.end(); ++it)
		{
			(*it)->onRender(packId(parentId, _id));
		}
	}
...
public:
	...
	void remove(const Sprite& sprite)
	{
		...
		_boundingRect.clear();
		std::for_each(_children.begin(), _children.begin() + _children.size(),
		              [&](const Sprite & sprite)
		{
			_boundingRect.add(sprite->getBoundingBox());
		});
	}
