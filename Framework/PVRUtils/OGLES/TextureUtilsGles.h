...
\file PVRUtils/OGLES/TextureUtilsGles.h
...
struct TextureUploadResults
{
	...
	/// <summary>Will be set to 'true' if the texture upload was successfull otherwise will return 'false'.</summary>
	bool successful;

	/// <summary>Default constructor for a TextureUploadResults structure.</summary>
	TextureUploadResults(): target(0), image(0), isDecompressed(0), successful(false) {}
	...
	TextureUploadResults(TextureUploadResults&& rhs) : target(rhs.target), image(rhs.image), format(rhs.format),
		isDecompressed(rhs.isDecompressed), successful(rhs.successful)
	{
		rhs.target = 0; rhs.image = 0;
	}
private:
