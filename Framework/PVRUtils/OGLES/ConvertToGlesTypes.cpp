...
\file PVRUtils/OGLES/ConvertToGlesTypes.cpp
...
namespace pvr {
namespace utils {
bool getOpenGLFormat(PixelFormat pixelFormat, pvr::ColorSpace colorSpace, pvr::VariableType dataType, uint32_t& glInternalFormat,
                     uint32_t& glFormat, uint32_t& glType, uint32_t& glTypeSize, bool& isCompressedFormat)
{
	isCompressedFormat = (pixelFormat.getPart().High == 0)
	                     && (pixelFormat.getPixelTypeId() != static_cast<uint64_t>(CompressedPixelFormat::SharedExponentR9G9B9E5));
...
bool getOpenGLStorageFormat(PixelFormat pixelFormat, pvr::ColorSpace colorSpace, VariableType dataType, GLenum& glInternalFormat)
{
	if (pixelFormat.getPart().High == 0)
	{
		//Format and type == 0 for compressed textures.
...
GLenum convertToGles(PrimitiveTopology primitiveType)
{
	...
	static GLenum glPrimtiveType[] =
	{
		GL_POINTS, GL_LINES, GL_LINE_STRIP, GL_TRIANGLES, GL_TRIANGLE_STRIP, GL_TRIANGLE_FAN, GL_LINES_ADJACENCY_OES,
		GL_LINE_STRIP_ADJACENCY_OES, GL_TRIANGLES_ADJACENCY_OES, GL_TRIANGLE_STRIP_ADJACENCY_OES, GL_PATCHES
	};
#endif
