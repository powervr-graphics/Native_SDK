...
public:
	...
	StructuredMemoryDescription(const StructuredMemoryDescription& rhs) :
		_name(rhs._name),
	...
	StructuredMemoryDescription(const std::string& name, uint32_t arraySize, std::initializer_list<StructuredMemoryDescription> children):
		_name(name), _numArrayElements(arraySize), _children(std::move(children)) { }
	...
	StructuredMemoryDescription(std::string&& name, uint32_t arraySize, std::initializer_list<StructuredMemoryDescription> children):
		_name(std::move(name)), _numArrayElements(arraySize), _children(std::move(children)) { }
	...
	StructuredMemoryDescription(std::string&& name, GpuDatatypes type) :
		_name(std::move(name)), _type(type), _numArrayElements(1) {}
	...
	/// <param name="arraySize">The array size for a new element to add.</param>
	/// <param name="type">The type of a new element to add.</param>
	StructuredMemoryDescription(const std::string& str, uint32_t arraySize, pvr::GpuDatatypes type) :
		_name(std::move(str)), _type(type), _numArrayElements(arraySize) {}
	...
	StructuredMemoryDescription(StructuredMemoryDescription&& rhs) :
		...
		_numArrayElements(std::move(rhs._numArrayElements)) {}
	...
	StructuredMemoryDescription& setName(const std::string& name) { _name = name; return *this; }

	/// <summary>Sets the name of the StructuredMemoryDescription.</summary>
	/// <param name="name">The new name of the StructuredMemoryDescription</param>
	/// <returns>Returns *this.</returns>
	StructuredMemoryDescription& setName(std::string&& name) { _name = std::move(name); return *this; }
	...
	/// <returns>Returns *this.</returns>
	StructuredMemoryDescription& setNumArrayElements(uint32_t numArrayElements) { _numArrayElements = numArrayElements; return *this; }
...
private:
	...
	void calcDynamicAlignment(BufferUsageFlags usage = BufferUsageFlags::UniformBuffer,
	...
	void calcSizeAndOffset(const uint32_t& offset)
	{
		...
		uint32_t current_offset = _offset;
		if (isStructure())
		{
		...
		else
		{
			...
			_size =
			  _variableArray ?
			  _arrayMemberSize * _numArrayElements :
			  pvr::getSize(_type, _numArrayElements);
		}
	}
	...
	void layout(BufferUsageFlags usage = BufferUsageFlags::UniformBuffer,
	            const uint64_t minUboDynamicAlignment = 0, const uint64_t minSsboDynamicAlignment = 0)
	{
		calcBaseAlignment();
...
public:
	...
	StructuredMemoryEntry() : _name(), _parent(0), _type(GpuDatatypes::none),
		_numArrayElements(0), _size(0), _arrayMemberSize(0), _offset(0), _mappedMemory(0), _mappedDynamicSlice(0) {}
...
//!\endcond
	...
	{
		privateInit(desc, NULL, true, false);
		layout();
	}
	...
	void initDynamic(const StructuredMemoryDescription& desc, BufferUsageFlags usage = BufferUsageFlags::UniformBuffer,
	                 const uint64_t minUboDynamicAlignment = 0, const uint64_t minSsboDynamicAlignment = 0)
	{
		privateInit(desc, NULL, true, false);
	...
	{
		{
			...
			if (isStructure())
			{
				str << "\n"; printPreamble(str, level * 2 + 5); str << "{";
				for (auto& child : _childEntries)
				{
...
private:
	...
	StructuredBufferViewElement(const StructuredMemoryEntry& entry, uint32_t level, uint32_t elementArrayIndex, const uint32_t* parentIndices, uint32_t dynamicSlice = 0) :
		_prototype(entry), _level(level), _mappedMemory(nullptr)
	{
...
/// <param name="value">The value to set by pointer</param>
	...
	{
		debug_assertion(this->_prototype.getPrimitiveType() == value.dataType() || value.dataType() == GpuDatatypes::mat3x3,
		                "StructuredBufferView: Mismatched FreeValue datatype");
	...
	void setArrayValuesStartingFromThis(const TypedMem& value)
	{
		...
		assertion(value.arrayElements() == _prototype.getNumArrayElements());
		...
		if (value.dataType() == GpuDatatypes::mat3x3)
		{
			...
			for (uint32_t i = 0; i < value.arrayElements(); ++i)
			{
				...
				//Log(LogLevel::Debug, "setArrayValuesStartingFromThis: Set value at offset %d with size %d to value (INT: %d, FLOAT: %f)",
				//    myoff,
				//    myvaluesize,
				//    value.interpretValueAs<uint32_t>(), value.interpretValueAs<float>(i));
		...
		else
		{
			...
			//    myoff,
			//    myvaluesize,
			//    value.interpretValueAs<uint32_t>(), value.interpretValueAs<float>());
