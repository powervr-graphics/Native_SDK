...
struct RendermanBufferDefinition
{
	...
	RendermanBufferDefinition() : numMultiBuffers(1), numDynamicClients(0), isDynamic(false) {}
};
...
struct RendermanMesh
{
	...
	DynamicArray<pvrvk::Buffer> vbos; //!< ONLY ONE - OPTIMISED FOR ALL PIPELINES
...
struct AutomaticNodeBufferEntrySemantic
{
	...
	uint32_t bufferDynamicOffset[pvrvk::FrameworkCaps::MaxSwapChains];
	uint32_t bufferDynamicSlice[pvrvk::FrameworkCaps::MaxSwapChains];
};
...
struct RendermanNode
{
	...
	DynamicArray<uint32_t> dynamicClientId[4]; //!< The indexes of each dynamic slice that this node owns in its buffers, respectively. Fixed index is pvrvk::DescriptorSet.
	DynamicArray<uint32_t> dynamicOffset[4][pvrvk::FrameworkCaps::MaxSwapChains]; //!< The (pre-calculated) byte offsets of the dynamic slices that this node owns in its buffers, respectively. Fixed index is pvrvk::DescriptorSet.
	DynamicArray<uint32_t> dynamicSliceId[4][pvrvk::FrameworkCaps::MaxSwapChains]; //!< The (pre-calculated) byte offsets of the dynamic slices that this node owns in its buffers, respectively. Fixed index is pvrvk::DescriptorSet.
	DynamicArray<RendermanBufferDefinition*> dynamicBuffer[4]; //!< The Dynamic Buffers that this node is being contained in. Fixed index is pvrvk::DescriptorSet.
	pvr::ContiguousMap<StringHash, UniformSemantic> uniformSemantics; //!< Uniform semantics used by this node.

	std::vector<AutomaticNodeBufferEntrySemantic> automaticEntrySemantics; //!< Automatic pvrvk::Buffer Entry semantics that were generated for this node. Used for auto-updating of shader buffer variables. (Automatic variables can be generated when an effect and a model's Semantics match, each such match can generate an automatic semantic.)
	std::vector<AutomaticNodeUniformSemantic> automaticUniformSemantics; //!<  Automatic Uniform semantics that were generated for this node. Used for auto-updating of shader uniform variables(Automatic variables can be generated when an effect and a model's Semantics match, each such match can generate an automatic semantic.)


	const uint32_t* getDynamicOffsetsPtr(uint32_t setId, uint32_t swapchainId)const
	{
		return dynamicOffset[setId][swapchainId].data();
	}


	uint32_t getDynamicOffset(uint32_t setId, uint32_t swapchainId, uint32_t dynamicClientId)const
	{
		return dynamicOffset[setId][swapchainId][dynamicClientId];
	}


	const DynamicArray<uint32_t>& getDynamicOffsets(uint32_t setId, uint32_t swapchainId)
	{
	...
	void recordRenderingCommands(pvrvk::CommandBufferBase cbuff,
	                             uint16_t swapIdx, bool recordBindPipeline = true,
	                             bool* recordBindDescriptorSets = nullptr,
...
struct RendermanPipeline
{
	...
	effect::PipelineDef* pipelineInfo; //!< Additional info on the pipeline
	...
	pvr::ContiguousMap<StringHash, UniformSemantic> uniformSemantics; //!< Connection of uniforms to semantics

	std::vector<AutomaticModelBufferEntrySemantic> automaticModelBufferEntrySemantics; //!< Automatic Model /pvrvk::Buffer Entry semantics generated for this pipeline (Node scope semantics can be found in nodes)
	std::vector<AutomaticModelUniformSemantic> automaticModelUniformSemantics; //!< Automatic Model /Uniform semantics generated for this pipeline (Node scope semantics can be found in nodes)
	...
	void recordUpdateAllUniformSemantics(pvrvk::CommandBufferBase cbuff);
	...
	/// <param name="cbuff">A command buffer to record the commands to</param>
	void recordUpdateAllUniformEffectSemantics(pvrvk::CommandBufferBase cbuff);
	...
	/// <param name="cbuff">A command buffer to record the commands to</param>
	void recordUpdateAllUniformModelSemantics(pvrvk::CommandBufferBase cbuff);
	...
	void recordUpdateAllUniformNodeSemantics(pvrvk::CommandBufferBase cbuff, RendermanNode& node);
	...
	bool recordUpdateUniformCommandsModelSemantic(pvrvk::CommandBufferBase cbuff, const StringHash& semantic);
	...
	bool recordUpdateUniformCommandsEffectSemantic(
	  pvrvk::CommandBufferBase cbuff, const StringHash& semantic);
	...
	/// <returns>Return true on success</returns>
	bool recordUpdateUniformCommandsNodeSemantic(
	  pvrvk::CommandBufferBase cbuff, const StringHash& semantic, RendermanNode& node);
	...
	/// <returns> Return true on success</returns>
	bool updateBufferEntryEffectSemantic(
	  const StringHash& semantic, const FreeValue& value,
	  uint32_t swapid, uint32_t dynamicClientId = 0);
	...
	/// <returns>Return true on success, false if the semantic is not found.</returns>
	bool updateBufferEntrySemantic(
	  const StringHash& semantic, const FreeValue& value,
	  uint32_t swapid, uint32_t dynamicClientId = 0);
	...
	/// <returns>Return true on success, false if the semantic is not found.</returns>
	bool updateBufferEntryNodeSemantic(
	  const StringHash& semantic, const FreeValue& value,
	  uint32_t swapid, RendermanNode& node);
	...
	/// <param name="node">The RendermanNode for which to set the value.</param>
	/// <returns>Return true on success, false if the semantic is not found.</returns>
	bool updateBufferEntryNodeSemantics(
	  const StringHash* semantics, const FreeValue* values, uint32_t numSemantics,
	  uint32_t swapid, RendermanNode& node)
	{
		bool result = true;
	...
	/// semantics. Default 0.</param>
	/// <returns>Return true on success, false if the semantic is not found.</returns>
	bool updateBufferEntryEffectSemantics(const StringHash* semantics, const FreeValue* values, uint32_t numSemantics,
	                                      uint32_t swapid, uint32_t dynamicClientId = 0);
...
struct RendermanSubpassGroup
{
	...
	/// <summary>Update the frame number for all models</summary>
	/// <param name="frame">Framenumber</param>
	void updateAllModelsFrame(float frame)
	{
	...
	/// <param name="cbuff">Recording Commandbuffer</param>
...
struct RendermanSubpass
{
	...
	void recordRenderingCommands(pvrvk::CommandBufferBase cbuff, uint16_t swapIdx)
	{
		...
		{
			group.recordRenderingCommands(cbuff, swapIdx);
		}
	}
	...
	void recordRenderingCommands(pvrvk::CommandBuffer& cbuff, uint16_t swapIdx, bool beginWithNextSubpassCommand)
	{
		...
		{
			cbuff->nextSubpass(VkSubpassContents::e_INLINE);
		}
		pvrvk::CommandBufferBase base(cbuff);
...
struct RendermanPass
{
	pvrvk::Framebuffer framebuffer[(uint32_t)pvrvk::FrameworkCaps::MaxSwapChains]; //!< The Framebuffer object chain this pass renders to
	...
	void recordRenderingCommands(pvrvk::CommandBuffer& cbuff, uint16_t swapIdx, bool beginEndRendermanPass);
	...
	void recordRenderingCommandsWithClearColor(pvrvk::CommandBuffer& cbuff, uint16_t swapIdx, const pvrvk::ClearValue& clearColor  = pvrvk::ClearValue())
	{
		const pvrvk::ClearValue clearValue(clearColor);
		recordRenderingCommands_(cbuff, swapIdx, &clearValue, 1);
	}
...
private:
	void recordRenderingCommands_(pvrvk::CommandBuffer& cbuff, uint16_t swapIdx, const pvrvk::ClearValue* clearValues, uint32_t numClearValues);
};
...
struct RendermanEffect
{
	...
	effect::EffectApi effect; //!< The EffectApi object used
	...
	/// be recorded</param>
	void recordRenderingCommands(pvrvk::CommandBuffer& cbuff, uint16_t swapIdx, bool beginEnderRenderPasses);
	...
	/// <param name="dynamicClientId"></param>
	/// <returns> Return true on success</returns>
	bool updateBufferEntryEffectSemantic(const StringHash& semantic, const FreeValue& value,
	                                     uint32_t swapid, uint32_t dynamicClientId = 0)
	{
		...
		void* memory;
		sem.buffer[0]->getDeviceMemory()->map(&memory, sem.structuredBufferView->getDynamicSliceOffset(swapid), sem.structuredBufferView->getDynamicSliceSize());
		sem.structuredBufferView->pointToMappedMemory(memory, swapid);
		sem.structuredBufferView->getElement(sem.entryIndex, dynamicClientId).setValue(value);
		if ((sem.buffer[0]->getDeviceMemory()->getMemoryFlags() & VkMemoryPropertyFlags::e_HOST_COHERENT_BIT) == 0)
		{
		...
		sem.buffer[0]->getDeviceMemory()->unmap();
		return true;
	}
};
...
public:
	...
	private:
		...
		pvrvk::GraphicsPipeline::ElementType* pipeline;
		...
		void advancePass(RendermanEffect& eff)
		{
			...
			cached = &eff.toRendermanPass(passId).toRendermanSubpass(0)
			         .toSubpassGroup(0).toSubpassGroupModel(0).toRendermanNode(0);
		}
		void advanceEffect()
		{
...
private:
	...
	bool buildRenderObjects_(pvrvk::CommandBuffer& texUploadCmdBuffer,
	                         std::vector<utils::ImageUploadResults>& uploadResult);
...
public:
	...
	RendermanEffect& toEffect(uint32_t effect)
	{
		return _renderStructure.effects[effect];
	}
	...
	RendermanPass& toPass(uint32_t effect, uint32_t pass)
	{
		return toEffect(effect).toRendermanPass(pass);
	}
	...
	RendermanSubpass& toSubpass(uint32_t effect, uint32_t pass, uint32_t subpass)
	{
	...
	/// <returns>The RendermanPipeline object with index 'pipeline' in subpass 'subpass' in pass 'pass' of effect
	/// 'effect'.</returns>
	RendermanSubpassGroupModel& toSubpassGroupModel(uint32_t effect, uint32_t pass, uint32_t subpass,
	    uint32_t subpassGroup, uint32_t model)
	{
	...
	RendermanMesh& toRendermanMeshByEffect(uint32_t effect, uint32_t pass, uint32_t subpass, uint32_t subpassGroup,
	                                       uint32_t model, uint32_t mesh)
	{
		return _renderStructure.effects[effect].passes[pass].subpasses[subpass].groups[subpassGroup]
	...
	{
		int32_t index = -1;
		for (std::size_t pass = 0; pass != _renderStructure.effects[effect].passes.size(); ++pass)
		{
			index = addModelForAllSubpasses(model, static_cast<uint16_t>(pass), effect);
		}
		return index;
	}
	...
	{
		int32_t index = -1;
		for (uint32_t subpass = 0; subpass !=
		     _renderStructure.effects[effect].passes[pass].subpasses.size(); ++subpass)
		{
	...
	{
		int32_t index = -1;
		for (uint32_t subpassGroup = 0; subpassGroup < toSubpass(effect, pass, subpass).getNumSubpassGroups();
		     ++subpassGroup)
		{
	...
	int32_t addModelForSubpassGroup(const assets::ModelHandle& model,
	                                uint32_t pass, uint32_t subpass, uint32_t subpassGroup,
	                                uint32_t effect = 0)
	{
	...
	/// <param name="uploadResults">(Output) An array where all the texture upload results will be stored.</param>
	/// <returns>The order of the Effect within the RenderManager (the index to use for toEffect()). Return -1 if
	/// error.</returns>
	uint32_t addEffect(const assets::effect::Effect& effect, pvrvk::CommandBuffer& cmdBuffer,
	                   std::vector<utils::ImageUploadResults>& uploadResults)
	{
		...
		this->_device = device;
		effect::EffectApi effectapi;
		effectapi.construct(device);
		if (!effectapi->init(effect, _swapchain, cmdBuffer, getAssetProvider(), uploadResults)) //validate
		{
		...
		for (uint32_t passId = 0; passId < effectapi->getNumPasses(); ++passId)
		{
			...
			for (uint32_t i = 0; i < ARRAY_SIZE(pass.framebuffer); ++i)
			{
	...
	/// <returns>True if successful.</returns>
	bool buildRenderObjects(pvrvk::CommandPool& texUploadCmdPool, pvrvk::Queue submitQueue)
	{
		pvrvk::CommandBuffer cmdBuffer = texUploadCmdPool->allocateCommandBuffer();
		pvrvk::Fence fence = submitQueue->getDevice()->createFence(VkFenceCreateFlags::e_SIGNALED_BIT);
		std::vector<ImageUploadResults> uploadResult;
		cmdBuffer->begin();
		bool rslt = buildRenderObjects_(cmdBuffer, uploadResult);
	...
	/// <param name="uploadResult">An array where all the texture upload results are stored.</param>
	/// <returns>True if successful.</returns>
	bool buildRenderObjects(pvrvk::CommandBuffer& texUploadCmdBuffer, std::vector<utils::ImageUploadResults>& uploadResult)
	{
		return buildRenderObjects_(texUploadCmdBuffer, uploadResult);
	}
...
inline bool recordUpdateUniformSemanticToExternalMemory(
  pvrvk::CommandBufferBase cbuff, uint32_t uniformLocation, TypedMem& value_ptr)
{
	return true;
}
	...
	if (isUpdating[swapChainIndex])
	{
		for (auto& buffer : bufferDefinitions)
		{
			...
			if (apibuf.getDeviceMemory()->isMapped())
			{
				...
				if ((apibuf.getDeviceMemory()->getMemoryFlags() & VkMemoryPropertyFlags::e_HOST_COHERENT_BIT) == 0)
				{
					apibuf.getDeviceMemory()->flushRange(apibuf.getDeviceMemory()->getMappedOffset(), apibuf.getDeviceMemory()->getMappedSize());
				}
				apibuf.getDeviceMemory()->unmap();
			}
		}
	}
...
inline void RendermanPipeline::recordUpdateAllUniformSemantics(pvrvk::CommandBufferBase cbuff)
{
...
inline void RendermanPipeline::recordUpdateAllUniformModelSemantics(pvrvk::CommandBufferBase cbuff)
{
	for (auto& sem : uniformSemantics)
	{
		recordUpdateUniformSemanticToExternalMemory(cbuff, sem.second.uniformLocation, sem.second.memory);
...
inline void RendermanPipeline::recordUpdateAllUniformEffectSemantics(pvrvk::CommandBufferBase cbuff)
{
	for (auto& sem : backToRendermanEffect().uniformSemantics)
	{
		recordUpdateUniformSemanticToExternalMemory(cbuff, sem.second.uniformLocation, sem.second.memory);
...
inline void RendermanPipeline::recordUpdateAllUniformNodeSemantics(pvrvk::CommandBufferBase cbuff, RendermanNode& node)
{
...
inline bool RendermanPipeline::recordUpdateUniformCommandsModelSemantic(pvrvk::CommandBufferBase cbuff, const StringHash& semantic)
{
...
inline bool RendermanPipeline::recordUpdateUniformCommandsEffectSemantic(pvrvk::CommandBufferBase cbuff, const StringHash& semantic)
{
...
inline bool RendermanPipeline::updateBufferEntryModelSemantic(const StringHash& semantic, const FreeValue& value, uint32_t dynamicSlice)
{
	...
	buffer->getDeviceMemory()->map(&memory, sem.structuredBufferView->getDynamicSliceOffset(dynamicSlice), sem.structuredBufferView->getDynamicSliceSize());
	sem.structuredBufferView->pointToMappedMemory(memory, dynamicSlice);
	sem.structuredBufferView->getElement(sem.entryIndex, dynamicSlice).setValue(value);
	if ((buffer->getDeviceMemory()->getMemoryFlags() & VkMemoryPropertyFlags::e_HOST_COHERENT_BIT) == 0)
	{
...
inline bool RendermanPipeline::updateBufferEntryModelSemantics(const StringHash* semantics,
    const FreeValue* value, uint32_t numSemantics, uint32_t dynamicSlice)
{
	for (uint32_t i = 0; i < numSemantics; ++i)
	{
...
inline bool RendermanPipeline::updateBufferEntryEffectSemantic(
  const StringHash& semantic, const FreeValue& value, uint32_t swapid, uint32_t dynamicClientId)
{
...
inline bool RendermanPipeline::updateBufferEntryNodeSemantic(
  const StringHash& semantic, const FreeValue& value, uint32_t swapid, RendermanNode& node)
{
	...
	{
		sem.buffer[0]->getDeviceMemory()->map(&memory, sem.structuredBufferView->getDynamicSliceOffset(dynamicSliceId),
		                                      sem.structuredBufferView->getDynamicSliceSize());
	}
	else
	{
		if (!sem.buffer[0]->getDeviceMemory()->isMapped())
		{
			sem.buffer[0]->getDeviceMemory()->map(&memory, sem.structuredBufferView->getDynamicSliceOffset(dynamicSliceId),
			                                      sem.structuredBufferView->getDynamicSliceSize());
		}
	}
	sem.structuredBufferView->pointToMappedMemory(memory, dynamicSliceId);
...
inline bool RendermanPipeline::updateBufferEntryEffectSemantics(
  const StringHash* semantics, const FreeValue* value, uint32_t numSemantics, uint32_t swapid, uint32_t dynamicClientId)
{
	std::vector<pvrvk::Buffer*> mapedBuffer(numSemantics);
	for (uint32_t i = 0; i < numSemantics; ++i)
	{
		...
		{
			void* memory;
			sem.buffer[0]->getDeviceMemory()->map(&memory, sem.structuredBufferView->getDynamicSliceOffset(swapid),
			                                      sem.structuredBufferView->getDynamicSliceSize());
			sem.structuredBufferView->pointToMappedMemory(memory, swapid);
			mapedBuffer.push_back(sem.buffer);
		}
		sem.structuredBufferView->getElement(sem.entryIndex, dynamicClientId).setValue(value[i]);
	}

	for (auto& walk : mapedBuffer)
	{
		walk[0]->getDeviceMemory()->unmap();
	}
	return true;
}
  ...
  const StringHash& semantic, const FreeValue& value, uint32_t swapid, uint32_t dynamicClientId)
{
	...
	if (!effect.isUpdating[swapid])
	{
		sem.buffer[0]->getDeviceMemory()->map(&memory, it->second.structuredBufferView->getDynamicSliceOffset(swapid),
		                                      it->second.structuredBufferView->getDynamicSliceSize());
		it->second.structuredBufferView->pointToMappedMemory(memory, swapid);
	}
	else
	{
		if (!sem.buffer[0]->getDeviceMemory()->isMapped())
...
{
	{
		...
		for (auto& reqsem : cont)
		{
			...
			if (setter == NULL)
			{
				Log(LogLevel::Information, "Automatic Model semantic [%s] not found.", reqsem.first.c_str());
			}
			else
			{
...
inline bool RendermanPipeline::updateAutomaticModelSemantics(uint32_t swapidx)
{
	...
	for (auto& sem : automaticModelBufferEntrySemantics)
	{
		...
		bool mustMap = !sem.buffer[0]->getDeviceMemory()->isMapped();
		if (mustMap)
		{
			void* memory;
			// Calculate the swapchain slice range
			// Number of dynamic slize must be multiple of 2.
			...
			const VkDeviceSize range = sem.structuredBufferView->getDynamicSliceSize() * numSlicesPerSwapchains;
			const uint64_t slizeIndex = offset / sem.structuredBufferView->getDynamicSliceSize();
			sem.buffer[0]->getDeviceMemory()->map(&memory, offset, range);
			sem.structuredBufferView->pointToMappedMemory(memory, (uint32_t)slizeIndex);
#ifdef PVR_RENDERMANAGER_DEBUG_SEMANTIC_UPDATES
			Log(LogLevel::Information, "*** SWAP INDEX: %d ***", swapidx);
			Log(LogLevel::Information, "Map memory %s", sem.structuredBufferView->getName().c_str());
			Log(LogLevel::Information, "\tnumSlicesPerSwapchains %d", numSlicesPerSwapchains);
			Log(LogLevel::Information, "\toffset %d", offset);
			Log(LogLevel::Information, "\trange %d", range);
			Log(LogLevel::Information, "\tslizeIndex %d", slizeIndex);
#endif
		}
		...
		sem.structuredBufferView->getElement(sem.entryIndex, 0, sem.structuredBufferView->getMappedDynamicSlice()).setValue(val);
		...
		if (mustMap && !wasUpdating)
		{
			...
			{
				Log(LogLevel::Warning, "RenderManager: Performance alert - Pipelines are updating without first calling BeginBufferUpdates on the RenderEffect."
...
inline void RendermanNode::updateAutomaticSemantics(uint32_t swapidx)
{
	...
	for (auto& sem : automaticEntrySemantics)
	{
		...
		if (mustMap)
		{
			...
			// Number of dynamic slize must be multiple of 2.
			const uint32_t numSlicesPerSwapchains = sem.structuredBufferView->getNumDynamicSlices() / numSwapchains;
			const uint64_t offset = sem.bufferDynamicOffset[swapidx];
			const VkDeviceSize range = sem.structuredBufferView->getDynamicSliceSize() * numSlicesPerSwapchains;
			const uint32_t slizeIndex = sem.bufferDynamicSlice[swapidx];
			sem.buffer[0]->getDeviceMemory()->map(&memory, offset, range);
			sem.structuredBufferView->pointToMappedMemory(memory, slizeIndex);

#ifdef PVR_RENDERMANAGER_DEBUG_BUFFERS
			Log(LogLevel::Debug, "MappingBuffer:");
			Log(LogLevel::Debug, "\tBuffer name: %s", sem.structuredBufferView->getName().c_str());
			Log(LogLevel::Debug, "\tBuffer size: %d", sem.buffer[0]->getSize());
			Log(LogLevel::Debug, "\tnumSlicesPerSwapchains: %d", numSlicesPerSwapchains);
			Log(LogLevel::Debug, "\toffset: %d", offset);
			Log(LogLevel::Debug, "\trange: %d", range);
			Log(LogLevel::Debug, "\tslizeIndex: %d", slizeIndex);
#endif
		}

		uint32_t dynamicSlizeId = sem.bufferDynamicSlice[swapidx];
#ifdef PVR_RENDERMANAGER_DEBUG_BUFFERS
		Log(LogLevel::Debug, "dynamicSlizeId: %d, semantic entryIndex: %d  Semantic: %s Offset: %d", dynamicSlizeId,
		    sem.entryIndex, sem.semantic->c_str(), sem.structuredBufferView->getElement(sem.entryIndex, 0, dynamicSlizeId).getOffset());
#endif
		...
		if (!wasUpdating && mustMap)
		{
			...
			if (!shown_multi_warning)
			{
				Log(LogLevel::Warning, "RenderManager: Performance alert - Nodes are updating without first calling BeginBufferUpdates on the RenderEffect."
				    " This means that buffers will be mapped and unmapped repeatedly for every node. This must usually be avoided.");
				shown_multi_warning = true;
			}
		}
...
inline void RendermanNode::createAutomaticSemantics()
{
	...
	for (auto& reqsem : this->toRendermanPipeline().bufferEntrySemantics)
	{
		...
		if (setter == NULL)
		{
			Log(LogLevel::Information, "Renderman: Automatic node semantic [%s] not found.", reqsem.first.c_str());
		}
		else
		{
	...
	for (auto& reqsem : uniformSemantics)
	{
		...
		if (setter == NULL)
		{
			Log(LogLevel::Information, "Automatic node semantic [%s] not found.", reqsem.first.c_str());
		}
		else
		{
	...
	std::sort(automaticEntrySemantics.begin(), automaticEntrySemantics.end(), [](const AutomaticNodeBufferEntrySemantic & a, const AutomaticNodeBufferEntrySemantic & b) { return a.entryIndex < b.entryIndex; });
}

inline void RendermanSubpassGroupModel::updateFrame(float frame)
{
#ifdef PVR_RENDERMANAGER_DEBUG_SEMANTIC_UPDATES
	Log(LogLevel::Debug, "Render Manager: current frame set to %f", frame);
#endif
