...
typedef DynamicArray<utils::AttributeLayout> AttributeConfiguration;
  ...
  const assets::Material& material)
{
	...
	if (!incompatible) // cond_pipe_it!=pass.subpasses[subpass].pipelines.end()
	{
		...
		pipeDef->createParam.inputAssembler.setPrimitiveTopology(convertToVk(mesh.getPrimitiveType()));
	...
	return std::make_pair(StringHash(), (effect::PipelineDef*)nullptr);
}

inline std::pair<StringHash, const effect::PipelineDef*>
selectPipelineForSubpassGroupMeshMaterial(
  effect::EffectApi& effect, uint32_t passId, uint32_t subpassId,
  uint32_t subpassGroupId, const assets::Mesh& mesh, const assets::Material& material)
{
	auto& pass = effect->getPass(passId);
	return selectPipelineForSubpassGroupMeshMaterial(effect,
	       pass.subpasses[subpassId].groups[subpassGroupId], mesh, material);
}
}
...
typedef void(*Reswizzler)(uint8_t* to, uint8_t* from, uint32_t toOffset, uint32_t fromOffset, uint32_t toWidth,
                          uint32_t fromWidth, uint32_t tostride, uint32_t fromstride, uint32_t items);

template<typename Fromtype, typename Totype>
void attribToAttrib(
  uint8_t* to, uint8_t* from, uint32_t toOffset, uint32_t fromOffset, uint32_t toWidth, uint32_t fromWidth,
...
{
	attribToAttrib<Fromtype, Totype>(static_cast<unsigned char*>(remap.to), static_cast<unsigned char*>(remap.from),
...
inline void populateVbos(AttributeConfiguration& attribConfig,
                         pvr::DynamicArray<Buffer>& vbos, assets::Mesh& mesh)
{
	...
	for (uint32_t i = 0; i < vbos.size(); ++i)
	{
		if (vbos[i].isValid())
		{
			...
			pvr::utils::updateBuffer(deviceTemp, vbos[i], ptrs[i].data(), 0, vbos[i]->getSize(), true);
		}
	}

}

inline bool createVbos(utils::RenderManager& renderman,
                       const std::map<assets::Mesh*, AttributeConfiguration*>& meshAttribConfig)
{
	...
	for (uint32_t model_id = 0; model_id < apiModels.size(); ++model_id)
	{
		for (uint32_t mesh_id = 0; mesh_id < apiModels[model_id].assetModel->getNumMeshes(); ++mesh_id)
		{
			...
			{
				apimesh.ibo = pvr::utils::createBuffer(device, mesh.getFaces().getDataSize(), VkBufferUsageFlags::e_INDEX_BUFFER_BIT, VkMemoryPropertyFlags::e_HOST_VISIBLE_BIT);
				...
				pvr::utils::updateBuffer(deviceTemp, apimesh.ibo, static_cast<const void*>(mesh.getFaces().getData()), 0, mesh.getFaces().getDataSize(), true);
			}

			uint32_t size = attribConfig.size();
			...
			for (uint32_t vbo_id = 0; vbo_id < size; ++vbo_id)
			{
				...
				apimesh.vbos[vbo_id] = pvr::utils::createBuffer(device, vboSize, VkBufferUsageFlags::e_VERTEX_BUFFER_BIT, VkMemoryPropertyFlags::e_HOST_VISIBLE_BIT);
...
inline std::vector<StringHash> getVertexBindingsForPipeline(
  const effect::EffectApi& effect, const StringHash& pipelineName)
{
...
inline std::vector<StringHash> getAllActiveVertexAttributesForMeshAndEffect(effect::EffectApi& effect,
...
inline AttributeConfiguration getVertexBindingsForPipeNoStride(
  const effect::EffectApi& effect, const StringHash& pipelineName)
{
	...
	for (auto it = pipe.attributes.begin(); it != pipe.attributes.end(); ++it)
	{
		...
		uint32_t width = getNumMatrixColumns(it->dataType) *
		                 getNumVecElements(it->dataType);
		DataType datatype = DataType::None;
		if (retval[binding].size() <= it->location)
		{
			retval[binding].resize(it->location + 1);
		}
		retval[binding][it->location] = utils::Attribute(it->semantic, datatype,
		                                static_cast<uint16_t>(width), static_cast<uint16_t>(retval[binding].stride), it->variableName);
	}
	return retval;
}

inline AttributeConfiguration getVertexBindingsForPipe(
  const effect::EffectApi& effect, const StringHash& pipelineName)
{
	AttributeConfiguration retval;
	auto& pipe = effect->getEffectAsset().versionedPipelines
	             .find(effect->getApiString())->second.find(pipelineName)->second;
	uint32_t count[32] = {};
	...
	for (auto it = pipe.attributes.begin(); it != pipe.attributes.end(); ++it)
	{
		...
		retval[binding][count[binding]++] =
		  utils::Attribute(it->semantic, datatype,
		                   static_cast<uint16_t>(width), static_cast<uint16_t>(retval[binding].stride), it->variableName);
...
inline utils::Attribute mergeAttribute(
  utils::Attribute one, const utils::Attribute& two)
{
	//Assume semantic1 = semantic2
	assertion(one.semantic == two.semantic, "RenderManager: Error processing effects. "
...
{
	assertion(one.semantic == two.getSemantic(), "RenderManager: Error processing effects. "
	          "Attempted to merge attributes with different semantics");
	one.datatype = (one.datatype == pvr::DataType::None ?
	                two.getVertexLayout().dataType : std::min(one.datatype, two.getVertexLayout().dataType));
}

inline void mergeAttributeLayouts(
  utils::AttributeLayout& inout_inner,
  utils::AttributeLayout& willBeDestroyed_outer)
{
...
inline void createAttributeConfigurations(
  RenderManager& renderman, std::map<PipelineSet,
  AttributeConfiguration>& pipeSets,
  std::map<StringHash, AttributeConfiguration*>& pipeToAttribMapping,
  std::map<assets::Mesh*, AttributeConfiguration*>& meshAttribConfig,
...
namespace {
inline void addSemanticLists(RendermanBufferBinding& buff,
                             std::map<StringHash, StructuredBufferView*>& bufferDefinitions,
                             std::map<StringHash, BufferEntrySemantic>& bufferEntries, bool checkDuplicates)
{
	...
	for (uint32_t i = 0; i < numElements; ++i)
	{
		...
		{
			debug_assertion(false, strings::createFormatted("DUPLICATE BUFFER ENTRY "
...
inline void addUniformSemanticLists(
  pvr::ContiguousMap<StringHash, effect::UniformSemantic>& effectlist,
...
/////////  PIPELINES /////////////
inline bool createPipelines(RenderManager& renderman,
                            const std::map<StringHash, AttributeConfiguration*>& vertexConfigs)
{
		...
		for (auto pipeline = vertexConfigs.begin(); pipeline != vertexConfigs.end(); ++pipeline)
		{
			...
			if (pipedef->attributes.size())
			{
				for (uint16_t binding = 0; binding < attributeConfig.size(); ++binding)
				{
					...
					pipecp.vertexInput.addInputBinding(VertexInputBindingDescription(binding,
					                                   vbo.stride, (inputBindingInfo ? (VkVertexInputRate)inputBindingInfo->inputRate :
					                                       VkVertexInputRate::e_VERTEX)));
					for (uint16_t vertexId = 0; vertexId < vbo.size(); ++vertexId)
					{
						for (uint32_t i = 0; i < pipedef->attributes.size(); ++i) // make sure it matches with the pipeline attribute
						{
							...
							if (pipedef->attributes[i].semantic == vertex.semantic &&
							    vertexId == pipedef->attributes[i].location &&
							    binding == pipedef->attributes[i].vboBinding)
							{
								pipecp.vertexInput.addInputAttribute(VertexInputAttributeDescription(
								                                       vertexId, binding, convertToVkVertexInputFormat(vertex.datatype,
								                                           static_cast<uint8_t>(vertex.width)), vertex.offset));
							}
						}
					}
				}
			...
			if (pipecp.viewport.getNumViewportScissors() == 0)
			{
				...
				pipecp.viewport.setViewportAndScissor(0, Viewport(0, 0, static_cast<float>(screendDim.width),
				                                      static_cast<float>(screendDim.height)), Rect2Di(0, 0, screendDim.width, screendDim.height));
			}
			GraphicsPipeline pipelineApi = effect->getDevice()->createGraphicsPipeline(pipecp, pvrvk::PipelineCache());
							...
							for (uint32_t setid = 0; setid < FrameworkCaps::MaxDescriptorSetBindings; ++setid)
							{
								...
								for (uint32_t dynamicClient = 0; dynamicClient < node.dynamicClientId[setid].size(); ++dynamicClient) // "dynamic client" is basically the buffer index.
								{
									...
									for (uint32_t swapId = 0; swapId < pvrvk::FrameworkCaps::MaxSwapChains; ++swapId)
									{
										...
										const uint32_t offset = static_cast<uint32_t>(node.dynamicBuffer[setid][dynamicClient]->structuredBufferView.getDynamicSliceOffset(dynamicSliceId));
										...
										Log(LogLevel::Information, "\t\t\t\t\t\t\t\tBuffer: %#010x %s",
										    node.dynamicBuffer[setid][dynamicClient]->buffer->getNativeObject(),
		...
		for (RendermanBufferDefinition& bufdef : renderman_effect.bufferDefinitions) //The buffer is created per model.
		{
			...
			debug_assertion(bufdef.memoryDescription.getNumChildren() != 0,
			                strings::createFormatted("RenderManager::createAll() "
			                    ...
			                    " [%d] was empty", bufdef.name.c_str()));

			bufdef.structuredBufferView.initDynamic(bufdef.memoryDescription, bufdef.numMultiBuffers * std::max<uint32_t>(1, bufdef.numDynamicClients),
			                                        bufdef.allSupportedBindings, static_cast<uint32_t>(device->getPhysicalDevice()->getProperties().limits.minUniformBufferOffsetAlignment),
			                                        static_cast<uint32_t>(device->getPhysicalDevice()->getProperties().limits.minStorageBufferOffsetAlignment));
...
#endif
			bufdef.buffer = pvr::utils::createBuffer(device, bufdef.structuredBufferView.getSize(),
			                                     pvr::utils::convertToVk(bufdef.allSupportedBindings), VkMemoryPropertyFlags::e_HOST_VISIBLE_BIT |
			                                     VkMemoryPropertyFlags::e_HOST_COHERENT_BIT);
		}
	}
...
inline bool createDescriptorSets(RenderManager& renderman,
                                 const std::map<assets::Mesh*, AttributeConfiguration*>& meshAttribConfig,
                                 DescriptorPool& pool, uint32_t swapchainLength, CommandBuffer cmdBuffer,
                                 std::vector<ImageUploadResults>& uploadResult)
{
							...
							for (auto& materialpipeline : materialeffect.materialSubpassPipelines)
							{
									...
									for (auto& inputEntry : pipedef.inputAttachments[swapindex])
									{
										...
										descSetWrites.push_back(
										  pvrvk::WriteDescriptorSet(VkDescriptorType::e_INPUT_ATTACHMENT,
										                            materialpipeline.sets[input.set][swapindex], input.binding)
										  .setImageInfo(0, pvrvk::DescriptorImageInfo(input.tex, VkImageLayout::e_SHADER_READ_ONLY_OPTIMAL)));
									}
								}
								...
								for (auto& tex : pipedef.textureSamplersByTexSemantic)
								{
									...
									if ((texIndex = materialeffect.material->assetMaterial->getTextureIndex(tex.first)) != -1)
									{
										...
										uploadResult.push_back(utils::loadAndUploadImage(device,
										                       texturePath.c_str(), true, cmdBuffer, renderman.getAssetProvider()));
										...
										{
											descSetWrites.push_back(pvrvk::WriteDescriptorSet(VkDescriptorType::e_COMBINED_IMAGE_SAMPLER,
											                        materialpipeline.sets[tex.second.set][swapindex], tex.second.binding)
											                        .setImageInfo(0, pvrvk::DescriptorImageInfo(
											                                        uploadResult.back().getImageView(),
											                                        pipedef.textureSamplersByTexSemantic.find(tex.first)->second.sampler)));
										}
									}
									else
									{
										Log(LogLevel::Information, "RenderManager: Texture semantic [%s] was not found in model material [%s]. "
								...
								for (auto& bufentry : pipeline.bufferBindings)
								{
									...
									for (uint32_t swapindex = 0; swapindex < swaplength; ++swapindex)
									{
										debug_assertion(buf.type >= VkDescriptorType::e_UNIFORM_BUFFER &&
										                buf.type <= VkDescriptorType::e_STORAGE_BUFFER_DYNAMIC, "Invalid buffer type");

										descSetWrites.push_back(pvrvk::WriteDescriptorSet(buf.type,
										                        materialpipeline.sets[buf.set][swapindex], buf.binding)
										                        .setBufferInfo(0, pvrvk::DescriptorBufferInfo(
										                            bufdef.buffer,
										                            0, bufdef.structuredBufferView.getDynamicSliceSize())));
									}
								}
							}
						}
	...
	device->updateDescriptorSets(descSetWrites.data(), (uint32_t)descSetWrites.size(), nullptr, 0);
	return true;
}

/////// FUNCTIONS TO ADD OBJECTS TO THE RENDER DATA STRUCTURE /////////
namespace {
inline RendermanBufferBinding makeRendermanBufferBinding(
  const StringHash& name, const effect::BufferRef& ref,
  std::deque<RendermanBufferDefinition>& definitions)
{
	...
	bufferbinding.type = convertToVk(ref.type);
...
size_t addRendermanPipelineIfNotExists(
  RendermanSubpassGroup& subpassGroup, effect::PipelineDef* pipedef,
  RendermanEffect& renderEffect, bool& isnew)
{
	//Comparator for the next find
	struct cmppipe
	{
		effect::PipelineDef* ptr;
		cmppipe(effect::PipelineDef* ptr) : ptr(ptr) {}
	...
	else
	{
		...
		//uint32_t current_buffer[4] = { 0, 0, 0, 0 };
		...
		//{
		//  newEntry.bufferBindings.find(buffer_it->first)->second.node_dynamic_offset_address = buffer.node_dynamic_offset_address = static_cast<int16_t>(current_buffer[buffer.set]++);
		//}
		//else
		//{
		//  newEntry.bufferBindings.find(buffer_it->first)->second.node_dynamic_offset_address = buffer.node_dynamic_offset_address = static_cast<int16_t>(-1);
		//}
		//}
	}
	return pipe_index;
...
inline void addBufferDefinitions(RendermanEffect& renderEffect, effect::EffectApi& effect)
{
...
void addNodeDynamicClientToBuffers(RendermanSubpassGroupModel&,
                                   RendermanNode& node, RendermanPipeline& pipeline)
{
	...
	for (auto buffer_it = pipeline.bufferBindings.begin();
	     buffer_it != pipeline.bufferBindings.end(); ++buffer_it)
	{
		auto& buffer = buffer_it->second;
		if ((buffer.bufferDefinition->scope == VariableScope::Node ||
		     buffer.bufferDefinition->scope == VariableScope::BoneBatch ||
		     buffer.bufferDefinition->scope == VariableScope::Effect)
		    &&
		    (buffer.type == VkDescriptorType::e_UNIFORM_BUFFER_DYNAMIC ||
		     buffer.type == VkDescriptorType::e_STORAGE_BUFFER_DYNAMIC))
		{
	...
	for (auto buffer_it = sortedBuffer.begin(); buffer_it != sortedBuffer.end(); ++buffer_it)
	{
		...
		if ((buffer.type == VkDescriptorType::e_UNIFORM_BUFFER_DYNAMIC ||
		     buffer.type == VkDescriptorType::e_STORAGE_BUFFER_DYNAMIC))
		{
			if (buffer.bufferDefinition->scope == VariableScope::Node ||
			    buffer.bufferDefinition->scope == VariableScope::BoneBatch)
...
inline void prepareDataStructures(RenderManager& renderman,
                                  std::map<assets::Mesh*, AttributeConfiguration*>& meshAttributeLayout,
				...
				for (uint8_t subpassGroupid = 0; subpassGroupid < effectsubpass.groups.size(); ++subpassGroupid)
				{
					...
					for (uint32_t modelId = 0; modelId < renderSubpassGroup.allModels.size(); ++modelId)
					{
						...
						for (uint32_t nodeId = 0; nodeId < rendermodel.assetModel->getNumMeshNodes(); ++nodeId)
						{
							...
							size_t pipe_index = addRendermanPipelineIfNotExists(renderSubpassGroup,
							                    pipe.second, *effect_it, isPipeNew);
  ...
  TypedMem& memory, const RendermanModel& rmodel, uint32_t cameraIndex)
{
	...
	memory.setValue(pvr::math::perspectiveFov(Api::Vulkan, fov,
	                static_cast<float>(screenDim.width), static_cast<float>(screenDim.height), nearClip, farClip));
	return true;
}
  ...
  TypedMem&  memory, const RendermanModel& rmodel, uint32_t cameraIndex)
{
	...
	bool isRotated = screenDim.height > screenDim.width;
	if (isRotated)
	{
		memory.setValue(
		  pvr::math::perspective(Api::Vulkan, fov,
		                         static_cast<float>(screenDim.height) / screenDim.width, nearClip, farClip,
	...
	else
	{
		memory.setValue(
		  pvr::math::perspective(Api::Vulkan, fov,
		                         static_cast<float>(screenDim.width) / screenDim.height, nearClip, farClip) * view);
	}
...
inline bool getLightColour(TypedMem& mem,
                           const RendermanModel& rmodel, int32_t lightNodeId)
{
	const assets::Model& model = *rmodel.assetModel;
...
inline bool getBoneMatrix(TypedMem& mem, const RendermanNode& node, uint32_t boneid)
{
	...
	debug_assertion(assetmesh.getMeshInfo().isSkinned && (assetmesh.getNumBoneBatches() > node.batchId &&
	                assetmesh.getNumBatchBones(node.batchId) > boneid), "OUT OF BOUNDS");
...
inline bool getBoneMatrices(TypedMem& mem, const RendermanNode& node)
{
	...
	mem.allocate(GpuDatatypes::mat4x4, assetmesh.getNumBatchBones(node.batchId));
...
inline bool getBoneMatricesIT(TypedMem& mem, const RendermanNode& node)
{
	...
	auto& assetmesh = *rmesh.assetMesh;
	debug_assertion(assetmesh.getMeshInfo().isSkinned && (assetmesh.getNumBoneBatches() > node.batchId), "OUT OF BOUNDS");
	mem.allocate(GpuDatatypes::mat3x3, assetmesh.getNumBatchBones(node.batchId));
	for (uint32_t boneid = mem.arrayElements(); boneid > 0;/*done in the loop*/)
	{
		--boneid;
		int i32BoneNodeID = assetmesh.getBatchBone(node.batchId, boneid);
...
void RenderManager::recordAllRenderingCommands(CommandBuffer& cbuff,
    uint16_t swapIdx, bool recordBeginEndRenderpass)
{
...
void RendermanEffect::recordRenderingCommands(CommandBuffer& cbuff,
    uint16_t swapIdx, bool beginEndRenderpass)
{
...
void RendermanPass::recordRenderingCommands(
  CommandBuffer& cbuff, uint16_t swapIdx, bool beginEndRendermanPass)
{
...
void RendermanPass::recordRenderingCommands_(
  CommandBuffer& cbuff, uint16_t swapIdx, const ClearValue* clearValues, uint32_t numClearValues)
{
	if (clearValues)
	{
		cbuff->beginRenderPass(framebuffer[swapIdx], framebuffer[swapIdx]->getRenderPass(), Rect2Di(0, 0,
		                       framebuffer[swapIdx]->getDimensions().width, framebuffer[swapIdx]->getDimensions().height),
		                       true, clearValues, numClearValues);
	}
	bool first = true;
	...
	if (clearValues) { cbuff->endRenderPass(); }
}
...
void RendermanSubpassGroupModel::recordRenderingCommands(CommandBufferBase cbuff, uint16_t swapIdx)
{
	...
	for (auto& node : nodes)
	{
		...
		for (uint32_t setid = 0; setid < FrameworkCaps::MaxDescriptorSetBindings; ++setid)
		{
			...
			{
				bindSets[setid] = false; continue;
			}
			uint32_t setswapid = renderpipeline.pipelineInfo->descSetIsMultibuffered[setid] ? swapIdx : 0;
			const DynamicArray<uint32_t>& nodeDynamicOffsets = node.getDynamicOffsets(setid, swapIdx);
			bindSets[setid] = (bindPipeline || node.pipelineMaterial_->sets[setid][setswapid].get() !=
			                   prev_sets[setid] || nodeDynamicOffsets.data() != dynamicOffsets[setid][swapIdx]);
...
void RendermanNode::recordRenderingCommands(CommandBufferBase cbuff,
    uint16_t swapidx, bool recordBindPipeline, bool* recordBindDescriptorSets,
    bool recordBindVboIbo, bool recordDrawCalls)
{
	...
	for (uint32_t setid = 0; setid < FrameworkCaps::MaxDescriptorSetBindings; ++setid)
	{
		if (!recordBindDescriptorSets || recordBindDescriptorSets[setid])
		{
			...
			const DynamicArray<uint32_t>& dynamicOffset = getDynamicOffsets(setid, setswapid);
			...
			cbuff->bindDescriptorSet(VkPipelineBindPoint::e_GRAPHICS, pipe.apiPipeline->getPipelineLayout(), setid,
			                         pipelineMaterial_->sets[setid][setswapid], dynamicOffset.data(), dynamicOffset.size());
	...
	if (recordBindVboIbo)
	{
		...
		if (rmesh.ibo.isValid()) {cbuff->bindIndexBuffer(rmesh.ibo, 0, convertToVk(rmesh.indexType));}
	}
...
bool RenderManager::buildRenderObjects_(
  CommandBuffer& texUploadCmdBuffer,
  std::vector<utils::ImageUploadResults>& uploadResult)
{
	...
	createDescriptorSets(*this, meshAttributeLayout,
	                     _descPool, _swapchain->getSwapchainLength(), texUploadCmdBuffer, uploadResult);
	return true;
}
}
}
