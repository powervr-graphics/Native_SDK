...
private:
	...
	virtual void calculateMvp(uint64_t parentIds, glm::mat4 const& srt, const glm::mat4& viewProj,
	                          Rect2Di const& viewport)const = 0;

	virtual void onRender(CommandBufferBase& commands, uint64_t parentId) const {}
	...
	virtual bool onAddInstance(uint64_t parentId) = 0;
...
private:
	...
	I2dComponent() : _anchor(Anchor::Center), _position(0.f, 0.f), _scale(1.f, 1.f), _rotation(0.f), _isPositioningDirty(true), _pixelOffset(0, 0),
...
public:
	...
	/// <returns>this object (allow chaining commands with ->)</returns>
	I2dComponent const* setUV(const Rect2Df& uv)const
	{
		_uv = uv;
		_isUVDirty = true;
		return this;
...
private:
	...
	Image_(UIRenderer& uiRenderer, const ImageView& tex, uint32_t width, uint32_t height, const Sampler& sampler);

	/// <summary>Function that will be automatically called by the uiRenderer. Do not call.</summary>
	void calculateMvp(uint64_t parentIds, const glm::mat4& srt, const glm::mat4& viewProj,
	                  const Rect2Di& viewport)const;

	/// <summary>Function that will be automatically called by the uiRenderer. Do not call.</summary>
	void onRender(CommandBufferBase& commands, uint64_t parentId) const;
	...
	bool onAddInstance(uint64_t parentId);
...
protected:
	...
	mutable DescriptorSet _texDescSet;
	...
	/// <summary>Used to initialise an Image_ sprite.</summary>
	/// <returns>True if the sprite was initialised successfully</returns>
	bool init();
...
public:
	...
	const ImageView& getTexture()const { return _texture; }

	/// <summary>Retrieve the Texture2D object that this Image wraps.</summary>
	/// <returns>The Texture2D object that this Image wraps.</returns>
	ImageView& getTexture() { return _texture; }
	...
	const DescriptorSet& getTexDescriptorSet() const
	{
...
private:
	...
	Font_(UIRenderer& uiRenderer, const ImageView& tex2D, const TextureHeader& tex, const Sampler& sampler)
	{
	...
	bool init(UIRenderer& uiRenderer, const ImageView& tex2D, const TextureHeader& tex, const Sampler& sampler);
...
public:
	...
	const Rect2Di& getRectangle(uint32_t index)const { return _rects[index]; }
...
private:
	...
	/// <summary>Function that will be automatically called by the uiRenderer. Do not call.</summary>
	void onRender(CommandBufferBase& commands) const;
...
private:
	...
	const DescriptorSet& getTexDescriptorSet() const { return getFont()->getTexDescriptorSet(); }
	bool init();
	bool onAddInstance(uint64_t parentId);

	void calculateMvp(uint64_t parentIds, glm::mat4 const& srt, const glm::mat4& viewProj,
...
private:
	...
	virtual void onRender(CommandBufferBase& commandBuffer, uint64_t parentId) const
	{
		for (ChildContainer::iterator it = _children.begin(); it != _children.end(); ++it)
		{
	...
	void onRemoveInstance(uint32_t parentId)
	{
...
private:
	...
	/// <param name="viewProjection">A view projection matrix.</param>
	/// <param name="viewport">The viewport currently in use.</param>
	void calculateMvp(uint64_t parentIds, glm::mat4 const& srt, const glm::mat4& viewProj, Rect2Di const& viewport)const
	{
		glm::mat4 tmpMatrix = srt * _cachedMatrix;
		//My cached matrix should always be up-to-date unless overridden. No effect.
		for (ChildContainer::iterator it = _children.begin(); it != _children.end(); ++it)
