...
static inline bool initializeContext(bool wantWindow, DisplayAttributes& original_attributes, NativePlatformHandles& handles, EGLConfig& config, Api graphicsapi)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			Log(LogLevel::Critical, "Failed to create egl::Context. Unknown reason of failure. Last error logged is: %s", platform::eglErrorToStr(egl::GetError()));
...
bool createSharedContext(const DisplayAttributes& original_attributes, NativePlatformHandles& parentHandles, NativeSharedPlatformHandles& handles, Api graphicsapi, EGLConfig& shared_config, uint32_t uploadContextPriority = 2)
{
	std::vector<EGLConfig> configs;
	DisplayAttributes attributes = original_attributes;
	...
	for (;;)
	{
		...
		if (numConfigs > 0)
		{
			...
			if (handles->uploadingContext != EGL_NO_CONTEXT)
			{
				...
				for (uint32_t retrybit = 0; retrybit < retry_DONE && is_final; ++retrybit)
				{
					if (retries[retrybit] == 1) // If was "now trying"...
					{
						Log(LogLevel::Debug, "PBuffer context : Current testing bit was %s. Will mark this as 'definitely not supported'(3), clear all 'tentative'(2) bits if present. If no tentative bits were found, will succeed!", retries_string[retrybit]);
						retries[retrybit] = 3; // Fix that we definitely need that.
						// Now, we try and enable all the rest of them...
						for (uint32_t retrybit_2 = 0; retrybit_2 < retry_DONE; ++retrybit_2)
						{
			...
			if (eglerror != EGL_SUCCESS)
			{
				Log(LogLevel::Debug, "PBuffer Context not created yet. Clearing EGL errors.");
			}
		} //eglChooseConfif failed.
		...
		{
			// Nothing else we can remove, fail
			Log(LogLevel::Critical, "Failed to create EGL PBufferContext. Unknown reason of failure. Last error logged is: %s", platform::eglErrorToStr(egl::GetError()));
...
static inline bool preInitialize(OSDisplay osdisplay, NativePlatformHandles& handles)
{
	if (!handles.get())
	{
		handles.construct();
...
bool EglContext_::init(OSWindow window, OSDisplay display, DisplayAttributes& attributes, Api minApi, Api maxApi)
{
	if (_initialized)
	{
		return false;
	...
	if (maxApi == Api::Unspecified)
	{
		maxApi = getMaxApiVersion();
	}
	if (minApi == Api::Unspecified)
	{
		minApi = Api::OpenGLES2;
	}
	else
	{
...
bool SharedEglContext_::init(EglContext_& context)
{
	std::unique_ptr<SharedEglContext_> retval(new SharedEglContext_());
	...
	bool res = createSharedContext(*context._attributes, _parentContext->_platformContextHandles, _handles, _parentContext->_apiType, config);
...
Api EglContext_::getMaxApiVersion()
{
	if (!_preInitialized)
	{
		if (preInitialize((OSDisplay)EGL_DEFAULT_DISPLAY, _platformContextHandles) != true)
		{
			Log(LogLevel::Critical, "Could not query max API version. Error while initialising OpenGL ES");
...
{
	auto retval = std::unique_ptr<SharedEglContext_>(new SharedEglContext_());
	retval->init(*this);
	return retval;
}
...
bool SharedEglContext_::makeSharedContextCurrent()
{
...
{
	return std::unique_ptr<platform::EglContext_>(new platform::EglContext_());
}
}
//!\endcond
