/*!
\brief Contains the implementation for the pvr::platform::ShellOS class on Screen platforms on Neutrino.
\file PVRShell\OS/NeutrinoScreen/ShellOS.cpp
...
Result ShellOS::initializeWindow(DisplayAttributes& data)
{
	...
	if (screen_context == 0)
	{
		Log(Log.Error, "Failed to Create Screen Context(%s:%i)", __FILE__, __LINE__);
	...
	if (screen_window == 0)
	{
		Log(Log.Error, "Failed to Create Screen Window(%s:%i)", __FILE__, __LINE__);
		return Result::UnknownError;
	}
	...
	{
	case 32:
		ePixelFormat = SCREEN_FORMAT_RGBA8888;
		break;
	default:
