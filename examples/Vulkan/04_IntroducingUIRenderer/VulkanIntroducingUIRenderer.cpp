...
pvr::Result VulkanIntroducingUIRenderer::initView()
{
	...
	pvr::ui::Font subTitleFont, centralTitleFont, centralTextFont;
	{
		...
		auto texview0 = pvr::utils::loadAndUploadImageAndView(_deviceResources->device, CentralTitleFontFile, true, _deviceResources->primaryCommandBuffer[0], *this,
			pvrvk::ImageUsageFlags::e_SAMPLED_BIT, pvrvk::ImageLayout::e_GENERAL, &tmpTexture0, &_deviceResources->vmaBufferAllocator, &_deviceResources->vmaImageAllocator);
		auto texview1 = pvr::utils::loadAndUploadImageAndView(_deviceResources->device, CentralTextFontFile, true, _deviceResources->primaryCommandBuffer[0], *this,
			pvrvk::ImageUsageFlags::e_SAMPLED_BIT, pvrvk::ImageLayout::e_GENERAL, &tmpTexture1, &_deviceResources->vmaBufferAllocator, &_deviceResources->vmaImageAllocator);

		centralTitleFont = _deviceResources->uiRenderer.createFont(texview0, tmpTexture0);
		centralTextFont = _deviceResources->uiRenderer.createFont(texview1, tmpTexture1);
		...
		auto texview2 = pvr::utils::loadAndUploadImageAndView(_deviceResources->device, subtitleFontFileName, true, _deviceResources->primaryCommandBuffer[0], *this,
			pvrvk::ImageUsageFlags::e_SAMPLED_BIT, pvrvk::ImageLayout::e_GENERAL, &tmpTexture2, &_deviceResources->vmaBufferAllocator, &_deviceResources->vmaImageAllocator);
...
{
	static bool centralTextRecorded[10] = {};
	static bool centralTitleRecorded[10] = {};
	...
	bool mustRecord = ((currentTime < IntroTime && !centralTitleRecorded[swapchainIndex]) || (currentTime >= IntroTime && !centralTextRecorded[swapchainIndex]));
	...
	if (currentTime < IntroTime)
	{
		...
		centralTitleRecorded[swapchainIndex] = true;
		if (mustRecord)
		{
	...
	else
	{
		...
		centralTextRecorded[swapchainIndex] = true;
		if (mustRecord)
		{
...
{
	const glm::mat4 mProjection = pvr::math::perspective(
