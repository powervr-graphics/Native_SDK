...
layout (location = 3) in highp vec4 clipPosition;
layout (location = 4) in highp vec4 clipPositionPreviousFrame;
layout (location = 5) in mediump vec2 inTexCoords;
layout (location = 6) in mediump vec3 inTangent;
layout (location = 7) in mediump vec3 inBiTangent;
...
mediump vec3 prefilteredReflection(mediump float roughness, mediump vec3 R)
{
	...
	else
	{
		...
		// There is a negative texture LOD bias applied for the NSR algorithm, which can affect the look of reflections if not adjusted here
		return textureLod(prefilteredMap, R, lod - ubo.textureLODBias).rgb;
	}
}
...
mediump vec3 perturbNormal()
{
	...
	highp vec3 b = normalize(inBiTangent);
...
void main()
{
	...
	highp vec3 V = normalize(ubo.cameraPosition - inWorldPos);
