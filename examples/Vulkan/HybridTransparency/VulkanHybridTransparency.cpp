/*!
\brief TODO
\file VulkanHybridTransparency.cpp
...
struct Material
{
	Material() : baseColor(glm::vec4(0.0f)), reflectanceTextureIndex(-1), isTransparent(0), transparency(0.0f) {
	...
	/// <summary>Flag to know if the material is transparent or opaque.</summary>
	int isTransparent;

	/// <summary>Transparency (1.0 means completely transparent, 1.0 means completely opaque).</summary>
...
struct DeviceResources
{
	...
	/// <summary>Ray traced transparency image view for the ray tracing pass for dielectic materials, also
	/// the result of a shadow ray towards the scene light result is stored here.</summary>
	pvrvk::ImageView raytraceTransparencyImage;

	/// <summary>Temp image used for sotring gaussian blur pass for the ray traced transparency image.</summary>
	pvrvk::ImageView raytraceTransparencyGaussianBlurImage;
	...
	/// <summary>Secondary command buffer used for the ray traced transparency pass.</summary>
	pvrvk::SecondaryCommandBuffer cmdBufferRayTracedTransparency[MAX_NUMBER_OF_SWAP_IMAGES];
	...
	/// <summary>Descriptor set layout used in the ray traced transparency pass to store results.</summary>
	...
	/// <summary>Descriptor set used in the ray traced transparency pass to store results.</summary>
	...
	/// <summary>Pipeline layout used in the ray traced transparency pass.</summary>
	pvrvk::PipelineLayout raytraceTransparencyPipelineLayout;
	...
	/// <summary>Graphics pipeline used in the final composition pass to merge the ray traced transparency with rasterized elements.</summary>
...
class VulkanHybridTransparency : public pvr::Shell
{
public:
	...
	/// <summary>Vector with the initial model translation of each scene element.</summary>
	std::vector<glm::vec3> _vectorInitialModelTranslation;

	/// <summary>Vector with the initial model scaling of each scene element.</summary>
	std::vector<glm::vec3> _vectorInitialModelScale;

	/// <summary>Helper variable to rotate the scene elements to animate the scene.</summary>
	glm::vec3 _positiveXAxis;
	...
	/// <summary>Helper variable to rotate vertically the animated scene elements.</summary>
	float _rotationAngleRadian;
	...
	/// <summary>Build the descriptor set layout with the image where the results of the ray traced transparency pass are stored.</summary>
	void buildWriteTransparencyImageDescriptorSetLayout();
	...
	/// <summary>Build the descriptor set with the image where the results of the ray traced transparency pass are stored.</summary>
	void buildWriteTransparencyImageDescriptorSet();
	...
	/// <summary>Build the pipeline used in the ray traced transparency pass.</summary>
	void buildRayTracingPipeline();

	/// <summary>Build the pipeline for the final composition pass where the ray traced transparency are added to the rasterization of the not ray traced scene elements.</summary>
	...
	/// <summary>record all the secondary command buffers used in the whole sample (recordCommandBufferRenderGBuffer, recordCommandBufferDeferredShading, recordCommandUIRenderer, recordCommandBufferRayTraceTransparency).</summary>
	...
	/// <summary>Record to secondary command buffer cmdBufferRayTracedTransparency the defered shading pass commands.</summary>
	/// <param name="cmdBuffer">SecondaryCommandbuffer to record.</param>
	/// <param name="swapchainIndex">Index of the current swapchain image.</param>
	void recordCommandBufferRayTraceTransparency(pvrvk::SecondaryCommandBuffer& cmdBuffer, uint32_t swapchainIndex);
...
pvr::Result VulkanHybridTransparency::initApplication()
{
	//  Load the scene
	_scene = pvr::assets::loadModel(*this, "Translucency.POD");

	_vectorModelTransform.resize(_scene->getNumMeshNodes());
	...
	// Get translation and scaling transform information to remove initial rotations from glass and frame scene elements
	// which will be animated with a rotation in the y axis, those elements are present at indices 2, 3, 4, and 5
	_vectorInitialModelTranslation.resize(_scene->getNumMeshNodes());
	_vectorInitialModelScale.resize(_scene->getNumMeshNodes());
	for (uint32_t i = 2; i < _scene->getNumMeshNodes(); i++)
	{
		glm::mat4 modelToWorld = _scene->getWorldMatrix(i);

		float sizeX = glm::length(glm::vec3(modelToWorld[0][0], modelToWorld[0][1], modelToWorld[0][2]));
		float sizeY = glm::length(glm::vec3(modelToWorld[1][0], modelToWorld[1][1], modelToWorld[1][2]));
		float sizeZ = glm::length(glm::vec3(modelToWorld[2][0], modelToWorld[2][1], modelToWorld[2][2]));
		_vectorInitialModelScale[i] = glm::vec3(sizeX, sizeY, sizeZ);

		_vectorInitialModelTranslation[i] = glm::vec3(modelToWorld[3]);

		_vectorModelTransform[i] = glm::translate(_vectorInitialModelTranslation[i]) * glm::rotate(1.5707f, glm::vec3(1.0, 0.0, 0.0)) *
			glm::rotate(-1.84f, glm::vec3(0.0, 0.0, 1.0)) * glm::scale(_vectorInitialModelScale[i]);
	}

	// Setup scene camera information
	_scene->getCameraProperties(0, _cameraFieldOfView, _cameraPosition, _cameraLookAt, _cameraUpDirection);
	_viewMatrix = glm::lookAt(_cameraPosition, _cameraLookAt, _cameraUpDirection);

	if (this->isScreenRotated())
	{
		_projectionMatrix = pvr::math::perspective(pvr::Api::Vulkan, _cameraFieldOfView, static_cast<float>(this->getHeight()) / static_cast<float>(this->getWidth()),
			_scene->getCamera(0).getNear(), _scene->getCamera(0).getFar(), glm::pi<float>() * .5f);
	}
	else
	{
		_projectionMatrix = pvr::math::perspective(pvr::Api::Vulkan, _cameraFieldOfView, static_cast<float>(this->getWidth()) / static_cast<float>(this->getHeight()),
...
pvr::Result VulkanHybridTransparency::buildDeviceAndQueues()
{
...
pvr::Result VulkanHybridTransparency::initView()
{
	...
	for (uint32_t i = 0; i < _numSwapImages; ++i)
	{
		...
		_deviceResources->cmdBufferRayTracedTransparency[i] = _deviceResources->commandPool->allocateSecondaryCommandBuffer();
		...
		_deviceResources->cmdBufferRayTracedTransparency[i]->setObjectName("RayTracedTransparencySecondaryCommandBufferSwapchain" + std::to_string(i));
	...
	_deviceResources->uiRenderer.getDefaultTitle()->setText("Hybrid Transparency");
...
pvr::Result VulkanHybridTransparency::releaseView()
{
	_deviceResources.reset();
	return pvr::Result::Success;
}

pvr::Result VulkanHybridTransparency::quitApplication() { return pvr::Result::Success; }

pvr::Result VulkanHybridTransparency::renderFrame()
{
...
void VulkanHybridTransparency::buildCommonDescriptorSetLayout()
{
	...
	commonDescSetInfo.setBinding(2, pvrvk::DescriptorType::e_STORAGE_BUFFER, 1u,
		pvrvk::ShaderStageFlags::e_FRAGMENT_BIT | pvrvk::ShaderStageFlags::e_RAYGEN_BIT_KHR | pvrvk::ShaderStageFlags::e_CLOSEST_HIT_BIT_KHR |
			pvrvk::ShaderStageFlags::e_ANY_HIT_BIT_KHR);
...
void VulkanHybridTransparency::buildGBufferSkyboxDescriptorSetLayout()
{
	// Binding 0: GBuffer reflectance image
	// Binding 1: GBuffer normalMaterialID image
	// Binding 2: GBuffer worldPositionTransparency image
	// Binding 3: Sky box
	...
	// GBuffer worldPositionTransparency image
...
void VulkanHybridTransparency::buildWriteTransparencyImageDescriptorSetLayout()
{
	// Binding 0: ray traced transparency store image
...
void VulkanHybridTransparency::buildGaussianBlurDescriptorSetLayout()
{
...
void VulkanHybridTransparency::buildDescriptorSetLayouts()
{
	buildCommonDescriptorSetLayout();
	buildGBufferSkyboxDescriptorSetLayout();
	buildWriteTransparencyImageDescriptorSetLayout();
	buildGaussianBlurDescriptorSetLayout();
}

void VulkanHybridTransparency::buildCommonDescriptorSet()
{
	// Binding 0: Camera data
	// Binding 1: Light data
	// Binding 2: Material buffer
...
void VulkanHybridTransparency::buildGBufferSkyboxDescriptorSet()
{
	// Binding 0: GBuffer reflectance image
	// Binding 1: GBuffer normalMaterialID image
	// Binding 2: GBuffer worldPositionTransparency image
	...
	pvrvk::WriteDescriptorSet worldPositionTransparencyImageWDS =
		pvrvk::WriteDescriptorSet(pvrvk::DescriptorType::e_COMBINED_IMAGE_SAMPLER, _deviceResources->gbufferSkyBoxDescriptorSet, 2);
	...
	worldPositionTransparencyImageWDS.setImageInfo(
	...
	std::vector<pvrvk::WriteDescriptorSet> writeDescSets = { reflectanceImageWDS, normalMaterialIDImageWDS, worldPositionTransparencyImageWDS, skyBoxWDS };
	_deviceResources->device->updateDescriptorSets(writeDescSets.data(), static_cast<uint32_t>(writeDescSets.size()), nullptr, 0);
}

void VulkanHybridTransparency::buildWriteTransparencyImageDescriptorSet()
{
	// Binding 0: ray traced transparency store image
	...
	writeImageWDS.setImageInfo(0, pvrvk::DescriptorImageInfo(_deviceResources->raytraceTransparencyImage, pvrvk::ImageLayout::e_GENERAL));
...
void VulkanHybridTransparency::buildGaussianBlurHorizontalDescriptorSet()
{
	// Binding 0: ray traced transparency image
	...
	gaussianBlurHorizontalPassWDS.setImageInfo(
		0, pvrvk::DescriptorImageInfo(_deviceResources->raytraceTransparencyImage, _deviceResources->samplerLinear, pvrvk::ImageLayout::e_SHADER_READ_ONLY_OPTIMAL));

	std::vector<pvrvk::WriteDescriptorSet> writeDescSets = { gaussianBlurHorizontalPassWDS };
	_deviceResources->device->updateDescriptorSets(writeDescSets.data(), static_cast<uint32_t>(writeDescSets.size()), nullptr, 0);
}

void VulkanHybridTransparency::buildGaussianBlurVerticalDescriptorSet()
{
	...
	gaussianBlurVerticalPassWDS.setImageInfo(
		0, pvrvk::DescriptorImageInfo(_deviceResources->raytraceTransparencyGaussianBlurImage, _deviceResources->samplerLinear, pvrvk::ImageLayout::e_SHADER_READ_ONLY_OPTIMAL));

	std::vector<pvrvk::WriteDescriptorSet> writeDescSets = { gaussianBlurVerticalPassWDS };
	_deviceResources->device->updateDescriptorSets(writeDescSets.data(), static_cast<uint32_t>(writeDescSets.size()), nullptr, 0);
}

void VulkanHybridTransparency::buildDescriptorSets()
{
	buildCommonDescriptorSet();
	buildGBufferSkyboxDescriptorSet();
	buildWriteTransparencyImageDescriptorSet();
	buildGaussianBlurHorizontalDescriptorSet();
	buildGaussianBlurVerticalDescriptorSet();
}

void VulkanHybridTransparency::definePipelineVertexInputState(pvrvk::PipelineVertexInputStateCreateInfo& pipelineVertexInputStateCreateInfo)
{
...
void VulkanHybridTransparency::buildGBufferPipeline()
{
...
void VulkanHybridTransparency::buildRayTracingPipeline()
{
	// pipeline layout
	...
	_deviceResources->raytraceTransparencyPipelineLayout = _deviceResources->device->createPipelineLayout(pipeLayout);

	pvrvk::ShaderModule raygenSM = _deviceResources->device->createShaderModule(pvrvk::ShaderModuleCreateInfo(getAssetStream("RayTraceTransparency.rgen.spv")->readToEnd<uint32_t>()));
	pvrvk::ShaderModule missSM = _deviceResources->device->createShaderModule(pvrvk::ShaderModuleCreateInfo(getAssetStream("RayTraceTransparency.rmiss.spv")->readToEnd<uint32_t>()));
	pvrvk::ShaderModule chitSM = _deviceResources->device->createShaderModule(pvrvk::ShaderModuleCreateInfo(getAssetStream("RayTraceTransparency.rchit.spv")->readToEnd<uint32_t>()));
	...
	hitCI.setGeneralShader(static_cast<uint32_t>(2));

	raytracingPipeline.shaderGroups = { rayGenCI, missCI, hitCI };
	...
	raytracingPipeline.pipelineLayout = _deviceResources->raytraceTransparencyPipelineLayout;

	_deviceResources->raytraceRefractionPipeline = _deviceResources->device->createRaytracingPipeline(raytracingPipeline, nullptr);
}

void VulkanHybridTransparency::buildDeferredShadingPipeline()
{
	pvrvk::PipelineLayoutCreateInfo pipeLayoutInfo;
...
void VulkanHybridTransparency::buildGaussianBlurHorizontalPipeline()
{
	pvrvk::PipelineLayoutCreateInfo pipeLayoutInfo;
	pipeLayoutInfo.clear();
...
void VulkanHybridTransparency::buildGaussianBlurVerticalPipeline()
{
	pvrvk::PipelineLayoutCreateInfo pipeLayoutInfo;
	pipeLayoutInfo.clear();
	pipeLayoutInfo.setDescSetLayout(0, _deviceResources->gaussianBlurDescriptorSetLayout);
...
void VulkanHybridTransparency::buildShaderBindingTable()
{
...
void VulkanHybridTransparency::buildPipelines()
{
...
void VulkanHybridTransparency::buildFramebufferAndRayTracingStoreImage()
{
	...
	pvrvk::Image raytraceTransparencyImage = pvr::utils::createImage(_deviceResources->device,
	...
	_deviceResources->raytraceTransparencyImage = _deviceResources->device->createImageView(pvrvk::ImageViewCreateInfo(
		raytraceTransparencyImage, pvrvk::ImageViewType::e_2D, raytraceTransparencyImage->getFormat(), pvrvk::ImageSubresourceRange(pvrvk::ImageAspectFlags::e_COLOR_BIT)));

	pvrvk::Image raytraceTransparencyGaussianBlurImage = pvr::utils::createImage(_deviceResources->device,
		pvrvk::ImageCreateInfo(pvrvk::ImageType::e_2D, pvrvk::Format::e_R8G8B8A8_UNORM, dimension,
			pvrvk::ImageUsageFlags::e_COLOR_ATTACHMENT_BIT | pvrvk::ImageUsageFlags::e_STORAGE_BIT | pvrvk::ImageUsageFlags::e_SAMPLED_BIT),
		pvrvk::MemoryPropertyFlags::e_DEVICE_LOCAL_BIT, pvrvk::MemoryPropertyFlags::e_DEVICE_LOCAL_BIT, _deviceResources->vmaAllocator,
		pvr::utils::vma::AllocationCreateFlags::e_DEDICATED_MEMORY_BIT);

	_deviceResources->raytraceTransparencyGaussianBlurImage = _deviceResources->device->createImageView(pvrvk::ImageViewCreateInfo(raytraceTransparencyGaussianBlurImage,
		pvrvk::ImageViewType::e_2D, raytraceTransparencyGaussianBlurImage->getFormat(), pvrvk::ImageSubresourceRange(pvrvk::ImageAspectFlags::e_COLOR_BIT)));
}

void VulkanHybridTransparency::buildRenderPass()
{
...
void VulkanHybridTransparency::buildGaussianBlurRenderPass()
{
	pvrvk::RenderPassCreateInfo renderPassInfo;
	...
	pvrvk::ImageView imageViews[] = { _deviceResources->raytraceTransparencyGaussianBlurImage };
	...
	imageViews[0] = _deviceResources->raytraceTransparencyImage;

	_deviceResources->gaussianBlurVerticalPassFramebuffer = _deviceResources->device->createFramebuffer(
		pvrvk::FramebufferCreateInfo(dimension.getWidth(), dimension.getHeight(), 1, _deviceResources->gaussianBlurRenderPass, 1, &imageViews[0]));
}

void VulkanHybridTransparency::createTextures(pvrvk::CommandBuffer& uploadCmd)
{
	// load textures
...
void VulkanHybridTransparency::buildSceneDescriptionBuffer()
{
...
void VulkanHybridTransparency::eventMappedInput(pvr::SimplifiedInput key)
{
	switch (key)
	{
	// Handle input
...
void VulkanHybridTransparency::buildModelBuffers(pvrvk::CommandBuffer& uploadCmd)
{
	int numMeshes = _scene->getNumMeshNodes();

	_deviceResources->models.reserve(numMeshes);
	...
	for (int32_t j = 0; j < numMeshes; ++j)
	{
		...
		const pvr::assets::Model::Node& node = _scene->getNode(j);
		pvr::assets::Mesh mesh = _scene->getMesh(node.getObjectId());
...
void VulkanHybridTransparency::buildCameraBuffer()
{
	...
	_deviceResources->globalBuffer->setObjectName("GlobalBufferUBO");
...
void VulkanHybridTransparency::buildLightDataBuffer()
{
	_scene->getLightPosition(0, _lightData.lightPosition);
	_lightData.lightColor = glm::vec4(0.8f, 0.8f, 0.8f, 1.0);
	_lightData.ambientColorIntensity = glm::vec4(0.1f, 0.1f, 0.1f, 50000.0);
...
void VulkanHybridTransparency::buildMaterialBuffer(pvrvk::CommandBuffer& uploadCmd)
{
	...
	std::vector<int> vectorMaterialIndex = { 0, 2, 1, 3, 4, 1 };

	// Assign the reflectance texture index to sample for each material (one material for each scene element)
	for (int i = 0; i < numMaterial; ++i) { vectorMaterial[i].reflectanceTextureIndex = vectorMaterialIndex[i]; }

	// The two meshes representing pieces of crystal are tagged as transparent (the reflectance texture does not have an alpha channel mapping transparency)
	vectorMaterial[3].transparency = 0.5f;
	vectorMaterial[4].transparency = 0.5f;
									   
	vectorMaterial[3].isTransparent = 1;
	vectorMaterial[4].isTransparent = 1;
...
void VulkanHybridTransparency::updateCameraLightData()
{
...
void VulkanHybridTransparency::updateScene()
{
	if (!_updateScene) { return; }

	_rotationAngleRadian += float(getFrameTime()) * 0.0002f;
...
void VulkanHybridTransparency::recordMainCommandBuffer()
{
	for (uint32_t i = 0; i < _numSwapImages; ++i)
	{
		...
		// Render raytraced transparency
		_deviceResources->cmdBufferMainDeferred[i]->executeCommands(_deviceResources->cmdBufferRayTracedTransparency[i]);

		// Gaussian Blur pass on the raytraced transparency render target
...
void VulkanHybridTransparency::recordSecondaryCommandBuffers()
{
	...
	for (uint32_t i = 0; i < _numSwapImages; ++i)
	{
		...
		_deviceResources->cmdBufferRayTracedTransparency[i]->begin();
		recordCommandBufferRayTraceTransparency(_deviceResources->cmdBufferRayTracedTransparency[i], i);
		_deviceResources->cmdBufferRayTracedTransparency[i]->end();
...
void VulkanHybridTransparency::recordCommandBufferRenderGBuffer(pvrvk::SecondaryCommandBuffer& cmdBuffer, uint32_t swapchainIndex)
{
...
void VulkanHybridTransparency::recordCommandBufferRayTraceTransparency(pvrvk::SecondaryCommandBuffer& cmdBuffer, uint32_t swapchainIndex)
{
	pvr::utils::beginCommandBufferDebugLabel(cmdBuffer, pvrvk::DebugUtilsLabel(pvr::strings::createFormatted("Ray Trace Transparency - Swapchain (%i)", swapchainIndex)));

	{
		...
		layoutTransitions.addBarrier(pvrvk::ImageMemoryBarrier(pvrvk::AccessFlags::e_SHADER_READ_BIT, pvrvk::AccessFlags::e_SHADER_WRITE_BIT,
			_deviceResources->raytraceTransparencyImage->getImage(), pvrvk::ImageSubresourceRange(pvrvk::ImageAspectFlags::e_COLOR_BIT), sourceImageLayout, destinationImageLayout,
	...
	cmdBuffer->bindDescriptorSets(pvrvk::PipelineBindPoint::e_RAY_TRACING_KHR, _deviceResources->raytraceTransparencyPipelineLayout, 0, arrayDS, 3, offsets, 3);
...
void VulkanHybridTransparency::recordCommandBufferDeferredShading(pvrvk::SecondaryCommandBuffer& cmdBuffer, uint32_t swapchainIndex)
{
...
void VulkanHybridTransparency::recordCommandBufferHorizontalGaussianBlur(pvrvk::SecondaryCommandBuffer& cmdBuffer, uint32_t swapchainIndex)
{
...
void VulkanHybridTransparency::recordCommandBufferVerticalGaussianBlur(pvrvk::SecondaryCommandBuffer& cmdBuffer, uint32_t swapchainIndex)
{
...
void VulkanHybridTransparency::recordCommandUIRenderer(pvrvk::SecondaryCommandBuffer& cmdBuffer)
{
...
std::unique_ptr<pvr::Shell> pvr::newDemo() { return std::make_unique<VulkanHybridTransparency>(); }
