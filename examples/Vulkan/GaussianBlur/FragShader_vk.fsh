#version 450
layout(set = 0, binding = 2) uniform highp sampler2D sTexture;
layout(set = 0, binding = 3) uniform highp sampler2D sOriginalTexture;

layout (set = 0, binding = 4) uniform BloomConfig
{
	mediump float WindowWidth;
};

layout(location = 0) in lowp vec2 TexCoord;
