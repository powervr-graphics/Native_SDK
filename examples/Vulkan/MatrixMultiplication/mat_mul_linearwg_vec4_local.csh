...
void main()
{
	...
	int local = int(gl_LocalInvocationID.y); // 0...linear_wg_size(WG_Y_SIZE)
	...
	for (int i = 0; i < LOADS_PER_INV; i++) { BColumnCache[i + (local * LOADS_PER_INV)] = VecBT[(x * VECS_PER_ROW) + i + (local * LOADS_PER_INV)]; }
	...
	for (int k = 0; k < N / 4; ++k) { sum += BColumnCache[k] * VecA[y * (N / 4) + k]; }
