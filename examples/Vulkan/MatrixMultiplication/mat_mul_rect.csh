...
void main()
{
	...
	for (int t = 0; t < TILE_NUM; ++t)
	{
		...
		for (int a = 0; a < A_LOCAL_EXTRA; ++a) { ACache[localY][(A_LOCAL_EXTRA * localX) + a] = A[globalY][(t * N_TILE) + (A_LOCAL_EXTRA * localX) + a]; }
		for (int b = 0; b < B_LOCAL_EXTRA; ++b) { BCache[(B_LOCAL_EXTRA * localY) + b][localX] = B[(t * N_TILE) + (B_LOCAL_EXTRA * localY) + b][globalX]; }

		// wait for all threads
		barrier();

		// incriment the accumulator for every element in the loaded tiles
		for (int k = 0; k < N_TILE; ++k) { accumulator += ACache[localY][k] * BCache[k][localX]; }
