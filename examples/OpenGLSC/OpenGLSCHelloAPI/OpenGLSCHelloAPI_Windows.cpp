/*!
\brief  asic Tutorial that shows step-by-step how to initialize OpenGL SC 2.0, use it for drawing a triangle and terminate it. Entry Point: WinMain
\port from OpenGL example 
\file OpenGLSCHelloAPI_Windows.cpp
\author PowerVR by Imagination, Developer Technology Team
\copyright Copyright (c) Imagination Technologies Limited.
*/

//#define DYNAMICGLSC_NO_NAMESPACE
...
{
	/*
		glGetError returns the last error that occurred using OpenGL SC, not necessarily the status of the last called function. The user
		has to check after every single OpenGL SC call or at least once every frame. Usually this would be for debugging only, but for this
...
bool chooseEGLConfig(EGLDisplay display, EGLConfig& config)
{
	...
	//	the minimum of needing it to render to a window, and being OpenGL SC 2.0 capable.
...
bool createEGLSurface(HWND nativeWindow, EGLDisplay display, EGLConfig config, EGLSurface& surface)
{
	...
	//	Using a native window created earlier and a suitable config, a surface is created that can be used to render OpenGL SC calls to.
...
/// <summary>Initializes shaders, buffers and other state required to begin rendering with OpenGL SC.</summary>
/// <returns>Whether the function succeeds or not.</returns>
...
{
	//	Concept: Shaders
	//	OpenGL SC 2.0 uses what are known as shaders to determine how to draw objects on the screen. Instead of the fixed function
	...
	//	These shaders are written in GL Shading Language ES: https://www.khronos.org/files/opengles_shading_language.pdf
	//	which is usually abbreviated to simply "GLSL ES".
	//	However, OpenGL SC 2.0 implementations are not required to support the full GLSL ES 1.00 specification.
	//	The GLSL limitations could be found in appendix F of https://registry.khronos.org/OpenGL/specs/sc/sc_spec_2.0.1.pdf.
	//  Unlike the OpenGL and OpenGL ES, OpenGL SC 2.0 require shader source code is compiled offline into a shader object and the
	//	shader object also linked offline into a program binary object which contain bot a vertex and a fragment shader.
	...
	if (num_formats == 1) { glGetIntegerv(GL_PROGRAM_BINARY_FORMATS, &format); }
	else
	{
		MessageBox(0, _T("Failed to get the number of program binary formats"), ERROR_TITLE, MB_OK | MB_ICONEXCLAMATION);
		return false;
	}
	...
	// Using pvr core to handle platform related embed binary shader loading
	auto shaderStream = std::make_unique<pvr::WindowsResourceStream>("SafetyCriticalShader.bin");
	if (!shaderStream->isReadable()) 
	{
		MessageBox(0, _T("Failed to read the binary shader file"), ERROR_TITLE, MB_OK | MB_ICONEXCLAMATION);
		return false;
	}


	glProgramBinary(shaderProgram, format, shaderStream->readToEnd<GLubyte>().data(), shaderStream->getSize());
...
{
	// OpenGL SC don't allow delet any gl object
...
/// <summary>The render loop to draw a triangle.</summary>
/// <param name="nativeWindow">The native window to release.</param>
/// <param name="display">The EGLDisplay used by the application.</param>
/// <param name="surface">The EGLSurface created from the native window.</param>
/// <param name="shaderProgram">Handle to a shader program containing the fragment and vertex shader.</param>
