...
bool render(GLuint shaderProgram, EGLDisplay eglDisplay, EGLSurface eglSurface, Display* nativeDisplay)
{
	// Renders a triangle for 800 frames using the state setup in the previous function
	for (int i = 0; i < 800; ++i)
	{
		if (!renderScene(shaderProgram, eglDisplay, eglSurface, nativeDisplay)) { return false; }
	}
...
int main(int /*argc*/, char** /*argv*/)
{
	...
	// Handles for the program handle which combines them.
	...
	createNativeWindow(nativeDisplay, &nativeWindow) &&
	...
	render(shaderProgram, eglDisplay, eglSurface, nativeDisplay);
