...
class OpenGLESPostProcessing : public pvr::Shell
{
	struct FrameBuffer
	{
		...
		~FrameBuffer()
		{
			...
			if (renderTex)
			{
				gl::DeleteTextures(1, &renderTex);
				renderTex = 0;
			}
			if (depthTex)
			{
				gl::DeleteTextures(1, &depthTex);
				depthTex = 0;
			}
		}
	};
	...
	struct DeviceResources
	{
		...
		DeviceResources() : baseTex(0), bloomMapTex(0), samplerRepeat(), samplerClamp(), progDefault(), progPreBloom(), progPostBloom(), progBlur() {}
		~DeviceResources()
		{
			if (vbos.size())
			{
			...
			if (baseTex)
			{
				gl::DeleteTextures(1, &baseTex);
				baseTex = 0;
			}
			if (bloomMapTex)
			{
				gl::DeleteTextures(1, &bloomMapTex);
				bloomMapTex = 0;
			}

			if (samplerRepeat)
			{
				gl::DeleteSamplers(1, &samplerRepeat);
				samplerRepeat = 0;
			}
			if (samplerClamp)
			{
				gl::DeleteSamplers(1, &samplerClamp);
				samplerClamp = 0;
			}
			...
			if (progPreBloom)
			{
				gl::DeleteProgram(progPreBloom);
				progPreBloom = 0;
			}
			if (progPostBloom)
			{
				gl::DeleteProgram(progPostBloom);
				progPostBloom = 0;
			}
			if (progBlur)
			{
				gl::DeleteProgram(progBlur);
				samplerRepeat = progBlur;
			}
		}
	};
...
public:
	OpenGLESPostProcessing() : _bloomIntensity(1.f) {}
...
***********************************************************************************************/
void OpenGLESPostProcessing::createTexturesAndSamplers()
{
	_deviceResources->baseTex = pvr::utils::textureUpload(*this, BaseTexFile, _deviceResources->context->getApiVersion() == pvr::Api::OpenGLES2);
...
***********************************************************************************************/
void OpenGLESPostProcessing::createPipeline()
{
	...
	// create render _scene pipeline
	{
		...
		_deviceResources->progDefault = pvr::utils::createShaderProgram(*this, VertShaderSrcFile, FragShaderSrcFile, attribNames, attribIndices, 3);
	...
	{
		_deviceResources->progPreBloom = pvr::utils::createShaderProgram(*this, QuadVertShaderSrcFile, PreBloomFragShaderSrcFile, NULL, NULL, 0);
	...
	//   Blur Pipeline
	{
		_deviceResources->progBlur = pvr::utils::createShaderProgram(*this, BlurVertSrcFile, BlurFragSrcFile, NULL, NULL, 0);
	...
	{
		_deviceResources->progPostBloom = pvr::utils::createShaderProgram(*this, QuadVertShaderSrcFile, PostBloomFragShaderSrcFile, NULL, NULL, 0);
...
******************************************************************************/
void OpenGLESPostProcessing::loadVbos()
{
...
pvr::Result OpenGLESPostProcessing::initView()
{
	...
	if (bRotate)
	{
		_proj = pvr::math::perspectiveFov(
			pvr::Api::OpenGLES3, fov, (float)getHeight(), (float)getWidth(), _scene->getCamera(0).getNear(), _scene->getCamera(0).getFar(), glm::pi<float>() * .5f);
	}
	else
	{
		_proj = glm::perspectiveFov<glm::float32>(fov, (float)getWidth(), (float)getHeight(), _scene->getCamera(0).getNear(), _scene->getCamera(0).getFar());
	}
	updateSubtitleText();
...
***********************************************************************************************/
void OpenGLESPostProcessing::createRenderFbo()
{
...
void OpenGLESPostProcessing::DeviceResources::beginPass(uint32_t pass, const glm::vec4& clearColor)
{
...
***********************************************************************************************/
void OpenGLESPostProcessing::createBlurFbo()
{
...
***********************************************************************************************/
void OpenGLESPostProcessing::updateSubtitleText()
{
	if (_applyBloom)
	{
		if (_drawObject)
...
***********************************************************************************************/
void OpenGLESPostProcessing::drawAxisAlignedQuad()
{
...
***********************************************************************************************/
void OpenGLESPostProcessing::executeCommands()
{
	// draw the _scene
	{
	...
	debugThrowOnApiError("OpenGLESPostProcessing::ExecuteCommands UIRenderer beginning");
	// UIRENDERER
	{
		// record the commands
