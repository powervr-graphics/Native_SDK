/*!
\brief  Shows how to perform tangent space bump mapping using a secondary thread to load the texture.
\file OpenGLESMultithreading.cpp
...
class OpenGLESMultithreading : public pvr::Shell
{
...
/// <summary>Entry point for secondary thread to start loading textures</summary>
void OpenGLESMultithreading::loadTexturesOnThread()
{
	_deviceResources->sharedContext->makeCurrent();

	std::this_thread::sleep_for(std::chrono::seconds(5));
	loadingDone = loadtextures();
}

/// <summary>Function that loads the diffuse texture and the bumpmap texture</summary>
...
bool OpenGLESMultithreading::loadtextures()
{
	...
	_deviceResources->texture =
		pvr::utils::textureUpload(*this, TextureFileName + (astcSupported ? "_astc.pvr" : ".pvr"), _deviceResources->sharedContext->getApiVersion() == pvr::Api::OpenGLES2);
	_deviceResources->bumpTexture =
		pvr::utils::textureUpload(*this, BumpTextureFileName + (astcSupported ? "_astc.pvr" : ".pvr"), _deviceResources->sharedContext->getApiVersion() == pvr::Api::OpenGLES2);
	...
	gl::SamplerParameteri(_deviceResources->samplerTrilinear, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
	gl::SamplerParameteri(_deviceResources->samplerTrilinear, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	gl::SamplerParameteri(_deviceResources->samplerTrilinear, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	// This glFinish may look weird, but the spec can be interpreted in such a way that if no sync commands exist,
	// this context's changes may never become visible to the main context's, so we must do something. For this usecase,
	// glFinish is enough. More complicated scenarios should use glFenceSync/glWaitSync/glClientWaitSync to determine if loading is done.
	gl::Finish();
...
pvr::Result OpenGLESMultithreading::initApplication()
{
	loadingDone = false;
...
/// <returns>Return pvr::Result::Success if no error occurred.</returns>
pvr::Result OpenGLESMultithreading::quitApplication() { return pvr::Result::Success; }
...
/// <returns>Return pvr::Result::Success if no error occurred.</returns>
pvr::Result OpenGLESMultithreading::initView()
{
	...
	_deviceResources->sharedContext = _deviceResources->context->createSharedContextFromEGLContext();

	loadingThread = std::thread(&OpenGLESMultithreading::loadTexturesOnThread, this); /// RENAME
	...
	_deviceResources->loadingText = _deviceResources->uiRenderer.createText("Loading...");
	_deviceResources->loadingText->commitUpdates();
...
void OpenGLESMultithreading::createProgram()
{
...
/// <returns>Return pvr::Result::Success if no error occurred </returns>
pvr::Result OpenGLESMultithreading::releaseView()
{
...
pvr::Result OpenGLESMultithreading::renderFrame()
{
	...
	if (!loadingDone)
	{
		...
		_deviceResources->loadingText->setColor(1.0f, 1.0f, 1.0f, f + .01f);
		_deviceResources->loadingText->setScale(sin(f) * 3.f, sin(f) * 3.f);
		_deviceResources->loadingText->commitUpdates();

		_deviceResources->uiRenderer.beginRendering();
		_deviceResources->uiRenderer.getDefaultTitle()->render();
		_deviceResources->loadingText->render();
...
void OpenGLESMultithreading::renderMesh(uint32_t nodeIndex)
{
...
std::unique_ptr<pvr::Shell> pvr::newDemo() { return std::unique_ptr<pvr::Shell>(new OpenGLESMultithreading()); }
