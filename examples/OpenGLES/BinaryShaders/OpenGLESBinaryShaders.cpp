/*!
\brief Show how to use glGetProgramBinary and glProgramBinary to store a shader program into a binary buffer and then load it into a different shader program to be used.
\file OpenGLESBinaryShaders.cpp
...
const char ShaderBinaryFileName[] = "shader.bin"; // POD _scene files
const char ShaderFormatFileName[] = "shaderformat.bin"; // POD _scene files

/// <summary>Class implementing the Shell functions.</summary>
class OpenGLESBinaryShaders : public pvr::Shell
{
...
public:
	...
	void setOpenglState();
	void shaderProgramToBinary(GLuint program);
	bool loadShaderBinary();
};

/// <summary>Caches a shader program in to a binary buffer.</summary>
/// <param name="program"> Program to be cached in the buffer</param>
void OpenGLESBinaryShaders::shaderProgramToBinary(GLuint program)
{
	...
	if (formats < 1) { throw pvr::InvalidOperationError("Driver does not support any binary formats."); }

	GLint length = 0;

	gl::GetProgramiv(program, GL_PROGRAM_BINARY_LENGTH, &length);

	binaryShaders.resize(length);
	gl::GetProgramBinary(program, length, 0, &shaderformat, binaryShaders.data());

	std::unique_ptr<pvr::Stream> formatStream = getWriteAssetStream(ShaderFormatFileName);
	formatStream->writeExact(sizeof(GLenum), 1, &shaderformat);

	std::unique_ptr<pvr::Stream> shaderStream = getWriteAssetStream(ShaderBinaryFileName);
	shaderStream->writeExact(binaryShaders.size(), 1, binaryShaders.data());
}

/// <summary>Loads a cached shader program from the buffer to the _deviceResources->program</summary>
/// <returns>Returns false if the binary file was not found on disk, true if it was found.</returns>
bool OpenGLESBinaryShaders::loadShaderBinary()
{
	GLenum shaderformat = 0;
	// Look for and read
	std::unique_ptr<pvr::Stream> formatStream = this->getAssetStream(ShaderFormatFileName, false);

	if (!formatStream) { return false; }

	shaderformat = (GLenum)*formatStream->readToEnd<GLenum>().data();

	std::unique_ptr<pvr::Stream> shaderStream = this->getAssetStream(ShaderBinaryFileName, false);
...
pvr::Result OpenGLESBinaryShaders::initApplication()
{
	// Load the scene
...
/// <returns>Result::Success if no error occurred</returns>.
pvr::Result OpenGLESBinaryShaders::quitApplication()
{
	_scene.reset();
	return pvr::Result::Success;
}
...
/// <returns>Result::Success if no error occurred.</returns>
pvr::Result OpenGLESBinaryShaders::initView()
{
	...
	_deviceResources->uiRenderer.getDefaultTitle()->setText("Binary Shaders");
	...
	if (!loadShaderBinary())
	{
		GLuint program = pvr::utils::createShaderProgram(*this, VertexShaderFile, FragmentShaderFile, attribs, attribIndices, 3, defines, numDefines);
		shaderProgramToBinary(program);
		gl::DeleteProgram(program);

		if (!loadShaderBinary()) { throw pvr::InvalidOperationError("Cannot Load Shader Binary"); };
	}

	_uniformLocations[Uniforms::WorldViewProjection] = gl::GetUniformLocation(_deviceResources->program, Uniforms::names[Uniforms::WorldViewProjection]);
	_uniformLocations[Uniforms::WorldViewIT] = gl::GetUniformLocation(_deviceResources->program, Uniforms::names[Uniforms::WorldViewIT]);
	_uniformLocations[Uniforms::LightDirEye] = gl::GetUniformLocation(_deviceResources->program, Uniforms::names[Uniforms::LightDirEye]);
	_uniformLocations[Uniforms::AlbedoTexture] = gl::GetUniformLocation(_deviceResources->program, Uniforms::names[Uniforms::AlbedoTexture]);
...
void OpenGLESBinaryShaders::setOpenglState()
{
...
/// <returns>Result::Success if no error occurred.</returns>
pvr::Result OpenGLESBinaryShaders::releaseView()
{
...
/// <returns>Result::Success if no error occurred.</returns>
pvr::Result OpenGLESBinaryShaders::renderFrame()
{
...
void OpenGLESBinaryShaders::renderMesh(uint32_t meshNodeId)
{
...
std::unique_ptr<pvr::Shell> pvr::newDemo() { return std::make_unique<OpenGLESBinaryShaders>(); }
