...
bool OGLES3MultipleRenderTargets::RenderScene()
{
	...
	*/
	...
	PVRTVec2 vLocs[8];
	if(PVRShellGet(prefIsRotated))
	{
		vLocs[0] = PVRTVec2(-1,-1); vLocs[1] = PVRTVec2(0, 0);
		vLocs[2] = PVRTVec2( 0, 0); vLocs[3] = PVRTVec2(1, 1);
		vLocs[4] = PVRTVec2(-1, 0); vLocs[5] = PVRTVec2(0, 1);
		vLocs[6] = PVRTVec2( 0,-1); vLocs[7] = PVRTVec2(1, 0);
	}
	else
	{
		vLocs[0] = PVRTVec2(-1, 0); vLocs[1] = PVRTVec2(0, 1);
		vLocs[2] = PVRTVec2( 0,-1); vLocs[3] = PVRTVec2(1, 0);
		vLocs[4] = PVRTVec2( 0, 0); vLocs[5] = PVRTVec2(1, 1);
		vLocs[6] = PVRTVec2(-1,-1); vLocs[7] = PVRTVec2(0, 0);
	}

	DrawAxisAlignedQuad(vLocs[0], vLocs[1], pEffect);
	glBindTexture(GL_TEXTURE_2D, m_uiRenderTextures[3]);
	DrawAxisAlignedQuad(vLocs[2], vLocs[3], pEffect);
	...
	DrawAxisAlignedQuad(vLocs[4], vLocs[5], pEffect);
	glBindTexture(GL_TEXTURE_2D, m_uiRenderTextures[2]);
	DrawAxisAlignedQuad(vLocs[6], vLocs[7], pEffect);
