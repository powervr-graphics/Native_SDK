/******************************************************************************
 ...
 @Description  Shows how to use PVRTools and matrix palette to generate shadow
               volumes for stencil shadows.
...
******************************************************************************/
class OGLESShadowVolumes : public PVRShell
{
...
******************************************************************************/
bool OGLESShadowVolumes::LoadTextures(CPVRTString* const pErrorStr)
{
...
******************************************************************************/
void OGLESShadowVolumes::LoadVbos()
{
...
/*!****************************************************************************
 ...
 @Description	Code in InitApplication() will be called by PVRShell once per
				...
				not be called again.
******************************************************************************/
bool OGLESShadowVolumes::InitApplication()
{
	m_puiVbo = 0;
	m_puiIndexVbo = 0;
...
/*!****************************************************************************
 ...
 @Description	Code in QuitApplication() will be called by PVRShell once per
				...
				not be called.
******************************************************************************/
bool OGLESShadowVolumes::QuitApplication()
{
...
******************************************************************************/
bool OGLESShadowVolumes::InitView()
{
	unsigned int i;
	...
	*/
	...
	for(i = 0; i < m_Scene.nNumMaterial; ++i)
	{
		m_puiTextures[i] = 0;
	...
	m_mProjection = PVRTMat4::PerspectiveFovRH(fFOV,  (float)PVRShellGet(prefWidth)/(float)PVRShellGet(prefHeight), g_CamNear, 0, PVRTMat4::OGL, bRotate);

	// Modify our projection matrix to give it an infinite far plane so we can accomodate our shadow volumes no matter
	// how far we extrude them.
	m_mProjection.f[10] = -0.999999f;
	m_mProjection.f[14] = -g_CamNear;
...
******************************************************************************/
bool OGLESShadowVolumes::ReleaseView()
{
...
/*!****************************************************************************
 ...
 @Description	This function will create the volume that we will be drawn
				in the stenciltest.
******************************************************************************/
bool OGLESShadowVolumes::BuildVolume(unsigned int ui32ShadowVol, PVRTVec4 *pLightPos)
{
	SPODNode* pNode;
	PVRTMat4 mWorld;
...
******************************************************************************/
bool OGLESShadowVolumes::RenderScene()
{
	...
	//Calculate the time passed since the last frame so we can rotate the cogs in a time-based manner
	...
	*/
	...
	/*
		Update the shadow volumes for any dynamic objects as they have moved so we require a different
	...
	*/
	...
	m_Print3D.DisplayDefaultTitle("ShadowVolumes", m_pDescription, ePVRTPrint3DSDKLogo);
	m_Print3D.Flush();
...
******************************************************************************/
void OGLESShadowVolumes::DrawFullScreenQuad()
{
	glEnable(GL_BLEND);
	...
	static const float afVertexData[] = { -1, -1, 1, -1, -1, 1, 1, 1 };
	...
	glColorPointer(4, GL_UNSIGNED_BYTE, 0, afVertexColourData);
...
******************************************************************************/
void OGLESShadowVolumes::DrawScene()
{
	...
	glMatrixMode(GL_PROJECTION);
	glLoadMatrixf(m_mProjection.ptr());
	...
	for(unsigned int i = 0; i < m_Scene.nNumMeshNode; ++i)
	{
		...
		glLoadMatrixf(mModelView.ptr());

		// Setup the light position in model space
		PVRTVec4 vLightPosModel;
		vLightPosModel = mWorld.inverse() * m_vLightPosWorld;
		glLightfv(GL_LIGHT0, GL_POSITION, vLightPosModel.ptr());
	...
	// Disable lighting and texturing as it is no longer needed
...
******************************************************************************/
void OGLESShadowVolumes::DoStencilTest()
{
	/*
	...
	*/
	...
	// Setup the stencil op and culling for the back faces
...
******************************************************************************/
void OGLESShadowVolumes::DrawShadowVolumes(PVRTVec4 *pLightPos)
{
	PVRTMat4 mModelView, mWorld, mExtrudeMatrix = PVRTMat4::Identity();
	PVRTVec4 vModelLightPos;
	SPODNode *pNode;
	...
	*/
	...
	for(unsigned int i = 0; i < m_ui32NoOfShadows; ++i)
	{
		...
		// The 1000.f is just an abitrary scale and can be changed.
		mExtrudeMatrix.f[0]  = 1000.0f;
		mExtrudeMatrix.f[5]  = 1000.0f;
		mExtrudeMatrix.f[10] = 1000.0f;
		mExtrudeMatrix.f[12] = -vModelLightPos.x * 1000.0f;
		mExtrudeMatrix.f[13] = -vModelLightPos.y * 1000.0f;
		mExtrudeMatrix.f[14] = -vModelLightPos.z * 1000.0f;
		...
		mModelView *= mExtrudeMatrix;
		glLoadMatrixf(mModelView.ptr());

		// Use the tools functions to draw the shadow volumes
		PVRTShadowVolSilhouetteProjectedRender(&m_pShadowMesh[i], &m_pShadowVol[i], &m_Context);
	}
...
******************************************************************************/
void OGLESShadowVolumes::DrawMesh(int i32NodeIndex)
{
...
******************************************************************************/
bool OGLESShadowVolumes::BuildShadowVolume(PVRTShadowVolShadowMesh *pShadowMesh, PVRTShadowVolShadowVol *pVolume, SPODMesh* pMesh)
{
...
PVRShell* NewDemo()
{
	return new OGLESShadowVolumes();
}

/******************************************************************************
 End of file (OGLESShadowVolumes.cpp)
******************************************************************************/
