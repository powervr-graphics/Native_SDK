/******************************************************************************
 ...
 @File         OGLES2MultiThreading_Linux.cpp
 ...
 @Platform     Linux derivatives including Android.
...
#ifdef __android__
#   define DEBUG_LOG(x) __android_log_print(ANDROID_LOG_ERROR, "OGLES2MultiThreading", "%s", x)
#else
#   define DEBUG_LOG(x) printf("%s", x)
#endif
...
/*!****************************************************************************
 ...
 @Description	Loads a basic vertex and fragment shader and returns the
                handles.
******************************************************************************/
bool OGLES2MultiThreading::LoadShaders(GLuint& uiVertShader, GLuint& uiFragShader, const char* pszVertShader,
                                       const char* pszFragShader)
{
	...
	if (!bShaderCompiled)
	{
		...
		// Displays the error
		DEBUG_LOG(PVRTStringFromFormattedStr("Failed to compile fragment shader: %s\n", pszInfoLog).c_str());
		delete [] pszInfoLog;
		return false;
	}
	...
	if (!bShaderCompiled)
	{
		...
		DEBUG_LOG(PVRTStringFromFormattedStr("Failed to compile vertex shader: %s\n", pszInfoLog).c_str());
		delete [] pszInfoLog;
		return false;
	}
...
bool OGLES2MultiThreading::CreateProgram(GLuint& uiProgramObject, const GLuint uiVertShader, const GLuint uiFragShader)
{
	...
	if (!bLinked)
	{
		...
		DEBUG_LOG(PVRTStringFromFormattedStr("Failed to link program: %s\n", pszInfoLog).c_str());
		delete [] pszInfoLog;
		return false;
	}
...
{
	const int iNumCircleVerts  = (c_iNumCirclePoints + 1 + 1) * 3;
	...
	for (float fRad = 0.0f; fRad < PVRT_2PI; fRad += (PVRT_2PI / c_iNumCirclePoints))
	{
...
bool OGLES2MultiThreading::CreateSceneGeometry(GLuint& uiVertexVbo, GLuint& uiIndexVbo)
{
	...
	// Set the buffer's data
	unsigned int uiSize = 24 * (sizeof(GLfloat) *
	                            8);                        // Calc afVertices size (24 vertices * stride (6 GLfloats per vertex))
...
void* ThreadFunc(void* arg)
{
	...
	*/
	...
	{
		DEBUG_LOG("eglCreateSyncKHR returned unexpected EGL_NO_SYNC_KHR.\n");
	}
	...
	if (status == EGL_FALSE)
	{
		DEBUG_LOG("eglClientWaitSyncKHR returned unexpected EGL_FALSE.\n");
	}

	return r ? (void*)0 : (void*)1;
}
	...
	*/
	...
	if (!eglCreateSyncKHR || !eglDestroySyncKHR || !eglClientWaitSyncKHR || !eglGetSyncAttribKHR)
	{
		PVRShellSet(prefExitMessage,
		            "Error: Failed to retrieve function pointers for KHR_fence_sync extension functions.\nIt's possible that the host system does not support this extension.\n");
		return false;
	}
...
void OGLES2MultiThreading::RenderLoadingScene(int iFrame)
{
	...
	*/
	...
	for (int iCircleIdx = 0; iCircleIdx < c_iNumCircles; ++iCircleIdx)
	{
		...
		PVRTMat4 mxMVP = mxProjection * PVRTMat4::Translation(-175.0f + iCircleIdx * 50.0f, fY, 0.0f) * PVRTMat4::Scale(fScale, fScale,
		                 1.0f);
...
/******************************************************************************
 End of file (OGLES2MultiThreading_Linux.cpp)
******************************************************************************/
