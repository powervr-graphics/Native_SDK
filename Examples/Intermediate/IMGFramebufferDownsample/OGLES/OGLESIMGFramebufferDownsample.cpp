/*!*********************************************************************************************************************
\File         OGLESIMGFramebufferDownsample.cpp
...
***********************************************************************************************************************/
class OGLESIMGFramebufferDownsample : public pvr::Shell
{
...
void OGLESIMGFramebufferDownsample::updateMode(int mode)
{
	if(mode == 0)
	{
...
***********************************************************************************************/
void OGLESIMGFramebufferDownsample::eventMappedInput(pvr::SimplifiedInput e)
{
	static int mode = 0;
...
***********************************************************************************************************************/
pvr::Result OGLESIMGFramebufferDownsample::initApplication()
{
...
***********************************************************************************************************************/
pvr::Result OGLESIMGFramebufferDownsample::initView()
{
	_context = pvr::createEglContext();
	_context->init(getWindow(), getDisplay(), getDisplayAttributes(), pvr::Api::OpenGLES2);

	bool isFramebufferDownsamplingSupported = gl::isGlExtensionSupported("GL_IMG_framebuffer_downsample");

	if(!isFramebufferDownsamplingSupported)
	{
		this->setExitMessage("Extension 'GL_IMG_framebuffer_downsample' is not supported on your platform.");
	...
	gl::ClearColor(0.00, 0.40, 0.50, 1.0f);
	...
	if (gl::ext::FramebufferTexture2DDownsampleIMG == 0)
	{
		this->setExitMessage("glFramebufferTexture2DDownsampleIMG function pointer was not found correctly");
		return pvr::Result::UnknownError;
	}
...
***********************************************************************************************************************/
pvr::Result OGLESIMGFramebufferDownsample::renderFrame()
{
	...
	if(_useHalfAndHalf)
	{
		// Use the half and half blit shader program to blit the full dimension framebuffer to the left hand side of the final image and the half size image to the right hand side of the final image
...
***********************************************************************************************************************/
pvr::Result OGLESIMGFramebufferDownsample::releaseView()
{
	_uiRenderer.release();
...
***********************************************************************************************************************/
pvr::Result OGLESIMGFramebufferDownsample::quitApplication() { return pvr::Result::Success; }

void OGLESIMGFramebufferDownsample::getDownScaleFactor(GLint& xDownscale, GLint& yDownscale)
{
	xDownscale = 1;
	yDownscale = 1;
...
***********************************************************************************************************************/
bool OGLESIMGFramebufferDownsample::LoadVbos()
{
	{
		_vertices.clear();
		_vertices.reserve(3);
...
\return	Return true if no error occurred
***********************************************************************************************************************/
bool OGLESIMGFramebufferDownsample::loadShaders()
{
	...
	if (!(_BlitShaderProgram.handle = pvr::utils::createShaderProgram(*this, BlitVertShaderSrcFile, BlitFragShaderSrcFile, attributes, attributeIndices, 1)))
	{
		setExitMessage("Unable to create Blit program (%s, %s)", BlitVertShaderSrcFile, BlitFragShaderSrcFile);
		return false;
	}
	...
	if (!(_HalfAndHalfShaderProgram.handle = pvr::utils::createShaderProgram(*this, HalfAndHalfVertShaderSrcFile, HalfAndHalfFragShaderSrcFile, NULL, NULL, 0)))
	{
		setExitMessage("Unable to create HalfAndHalf program (%s, %s)", HalfAndHalfVertShaderSrcFile, HalfAndHalfFragShaderSrcFile);
		return false;
	}
	...
	gl::Uniform1f(gl::GetUniformLocation(_HalfAndHalfShaderProgram.handle, "WindowWidth"), this->getWidth());
...
***********************************************************************************************************************/
std::unique_ptr<pvr::Shell> pvr::newDemo() {	return std::unique_ptr<pvr::Shell>(new OGLESIMGFramebufferDownsample()); }