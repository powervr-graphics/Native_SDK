...
\return		Whether the function succeeded or not.
\brief	Creates an EGLDisplay from a native native display, and initialises it.
***********************************************************************************************************************/
...
int main(int /*argc*/, char** /*argv*/)
{
	...
	if (!initialiseBuffer(vertexBuffer)){	goto cleanup;	}

	// Initialise the fragment and vertex shaders used in the application
	if (!initialiseShaders(fragmentShader, vertexShader, shaderProgram)){	goto cleanup;	}

	// Renders a triangle for 800 frames using the state setup in the previous function
	for (int i = 0; i < 800; ++i)
	{
		if (!renderScene(shaderProgram, eglDisplay, eglSurface, nativeDisplay))	{	break;	}
	}
