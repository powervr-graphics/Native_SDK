...
{
	commandBufferWithIntro->beginRecording();
	...
	commandBufferWithText->beginRecording();
...
void OGLES3IntroducingUIRenderer::generateBackgroundTexture(pvr::uint32 screenWidth, pvr::uint32 screenHeight)
{
	...
	hd.width = width;
	hd.height = height;
	pvr::assets::Texture myTexture(hd);
	...
	pvr::ui::UIRenderer& uirenderer, pvr::ui::Font& font)
{
	...
	if ((res = pvr::assets::textureLoad(fontFile, pvr::assets::getTextureFormatFromFilename(filename), tmpTexture)) != pvr::Result::Success)
	{
		return res;
	}
...
pvr::Result::Enum OGLES3IntroducingUIRenderer::initView()
{
	...
	uiRenderer.init(getGraphicsContext());

	commandBufferWithIntro = getGraphicsContext()->createSecondaryCommandBuffer();
	commandBufferWithText = getGraphicsContext()->createSecondaryCommandBuffer();
	commandBufferSubtitle = getGraphicsContext()->createSecondaryCommandBuffer();
	primaryCommandBuffer = getGraphicsContext()->createCommandBuffer();
...
pvr::Result::Enum OGLES3IntroducingUIRenderer::renderFrame()
{
	...
	primaryCommandBuffer->beginRenderPass(onScreenFbo, pvr::Rectanglei(0, 0, getWidth(), getHeight()));
	...
	commandBufferSubtitle->beginRecording();
