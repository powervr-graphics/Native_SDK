...
#include "PVRUtils/OGLES/ModelGles.h"
#include <fstream>
#include <iomanip>
...
public:
	OGLESSkinning() : _currentFrame(0), _isPaused(false) {}
...
pvr::Result OGLESSkinning::initView()
{
	...
	_deviceResources->programDefault = pvr::utils::createShaderProgram(
	                                     *this, Configuration::DefaultVertShaderFile, Configuration::DefaultFragShaderFile,
	                                     Configuration::DefaultAttributeNames, Configuration::DefaultAttributeIndices, 3);
	_deviceResources->programSkinned = pvr::utils::createShaderProgram(
	                                     *this, Configuration::SkinnedVertShaderFile, Configuration::SkinnedFragShaderFile,
	                                     Configuration::SkinnedAttributeNames, Configuration::SkinnedAttributeIndices, 7);
	...
	{
		setExitMessage("Failed to initialize UIRenderer");
		return pvr::Result::UnknownError;
	}

	pvr::utils::StructuredMemoryDescription desc("SSbo", 1,
	{
		{"BoneCount", pvr::GpuDatatypes::Integer},
		{
			"Bones", 1,
			{
	...
	pvr::utils::StructuredMemoryDescription descUbo("Ubo", 1,
	{
		{ "ViewProjMatrix", pvr::GpuDatatypes::mat4x4 },
		{ "LightPos", pvr::GpuDatatypes::vec3 }
	});
...
void OGLESSkinning::setDefaultOpenglState()
{
...
***********************************************************************************************************************/
pvr::Result OGLESSkinning::releaseView() { _deviceResources.reset(0); return pvr::Result::Success; }
...
pvr::Result OGLESSkinning::renderFrame()
{
	...
	if (_scene->getNumFrames() > 1)
	{
		if (fDelta > 0.0001f)
		{
			...
			while (_currentFrame >= _scene->getNumFrames() - 1) { _currentFrame -= (_scene->getNumFrames() - 1); }
		}
...
void OGLESSkinning::renderNode(uint32_t nodeId, const glm::mat4& viewMatrix, const glm::mat4& viewProjMatrix, bool& lastRenderWasSkinned)
{
	debugLogApiError("OGLESSkinning::renderNode Enter");
	...
	if (mesh.getMeshInfo().isSkinned && bumpTexId != 0)
	{
		...
		for (int i = 0; i < sizeof(Configuration::SkinnedAttributeSemantics) / sizeof(Configuration::SkinnedAttributeSemantics[0]); ++i)
		{
			...
			gl::VertexAttribPointer(i, attr.getN(),
			                        pvr::utils::convertToGles(attr.getVertexLayout().dataType),
			                        pvr::dataTypeIsNormalised(attr.getVertexLayout().dataType),
			                        mesh.getStride(0),
			                        reinterpret_cast<const void*>(static_cast<uintptr_t>(attr.getOffset())));
		}
		debugLogApiError("OGLESSkinning::renderNode Skinned Setup");
		...
		for (uint32_t batch = 0, end = mesh.getNumBoneBatches(); batch < end; ++batch)
		{
			...
			if (!bones)
			{
				debugLogApiError("OGLESSkinning::renderNode Mapping");
			}
			...
			debugLogApiError("OGLESSkinning::renderNode Skinned Set uniforms");
			gl::DrawElements(GL_TRIANGLES, mesh.getNumFaces(batch) * 3,
			                 pvr::utils::convertToGles(mesh.getFaces().getDataType()),
			                 reinterpret_cast<const void*>(static_cast<uintptr_t>(mesh.getBatchFaceOffsetBytes(batch))));
			debugLogApiError("OGLESSkinning::renderNode Skinned Draw");
		}
	}
	else
	{
		...
		debugLogApiError("OGLESSkinning::renderNode Unskinned Setup");
		...
		debugLogApiError("OGLESSkinning::renderNode Unskinned Set uniforms 0");
		...
		debugLogApiError("OGLESSkinning::renderNode Unskinned Set uniforms 1");
		gl::UniformMatrix4fv(_defaultUniformLocations[static_cast<uint32_t>(DefaultUniforms::MVPMatrix)], 1, GL_FALSE, glm::value_ptr(mvp));
		debugLogApiError("OGLESSkinning::renderNode Unskinned Set uniforms 2");
		...
		debugLogApiError("OGLESSkinning::renderNode Unskinned Set uniforms 3");

		gl::DrawElements(GL_TRIANGLES, mesh.getNumFaces() * 3, pvr::utils::convertToGles(mesh.getFaces().getDataType()), 0);
		debugLogApiError("OGLESSkinning::renderNode Unskinned Draw ");
	}
}
...
\brief  This function must be implemented by the user of the shell. The user should return its Shell object defining the behaviour
  of the application.
***********************************************************************************************************************/
std::unique_ptr<pvr::Shell> pvr::newDemo() { return std::unique_ptr<pvr::Shell>(new OGLESSkinning()); }
