...
const pvr::utils::StructuredMemoryDescription ParticleViewMapping("ParticlesBuffer", 1,
{
	{"vPosition", pvr::GpuDatatypes::vec3},
	{"vVelocity", pvr::GpuDatatypes::vec3},
	{"fTimeToLive", pvr::GpuDatatypes::Float}
});
...
const pvr::utils::StructuredMemoryDescription SphereViewMapping("SphereBuffer", 1,
{
	{"SphereArray", 8, pvr::GpuDatatypes::vec4}
});
...
struct ParticleConfig
{
	...
	{
		void* memory;
		buffer->getDeviceMemory()->map(&memory, view.getDynamicSliceOffset(swapidx), view.getDynamicSliceSize());
		view.pointToMappedMemory(memory, swapidx);
...
public:
	...
	bool init(uint32_t maxParticles, const Sphere* spheres, uint32_t numSpheres,
	...
	void setNumberOfParticles(uint32_t numParticles, pvrvk::Queue& queue);
	...
	bool setCollisionSpheres(const Sphere* spheres, uint32_t numSpheres);
	pvrvk::Buffer getParticleBufferView()const { return particleBufferViewSsbos; }
