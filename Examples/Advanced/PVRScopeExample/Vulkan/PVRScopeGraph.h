...
public:
	...
	void position(const uint32_t nViewportW, const uint32_t nViewportH, pvrvk::Rect2Di const& graph);

	void setUpdateInterval(const uint32_t nUpdateInverval);
	bool isInitialized()const { return _isInitialzed; }

	bool init(pvrvk::Device& device, const pvrvk::Extent2D& dimension, pvrvk::DescriptorPool& descPool, pvr::IAssetProvider& assetProvider,
	          pvr::ui::UIRenderer& uiRenderer, const pvrvk::RenderPass& renderPass, std::string& outMsg);
protected:
