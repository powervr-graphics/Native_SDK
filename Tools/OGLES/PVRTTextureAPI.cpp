										...
										CPVRTMap<unsigned int, CPVRTMap<unsigned int, MetaDataBlock> > *pMetaData)
{
	...
	if (bIsCompressedFormat)
	{
		...
		if (bIsCompressedFormatSupported)
		{
			...
			for (PVRTuint32 uiMIPLevel=0; uiMIPLevel<sTextureHeader.u32MIPMapCount; ++uiMIPLevel)
			{
				...
				{
					//Upload the texture
					glCompressedTexImage2D(GL_TEXTURE_2D,uiMIPLevel-nLoadFromLevel,eTextureInternalFormat,u32MIPWidth, u32MIPHeight, 0, uiCurrentMIPSize, pTempData);
				}
		...
		else
		{
			...
			//Loop through all MIP levels.
			for (PVRTuint32 uiMIPLevel=0; uiMIPLevel<sTextureHeaderDecomp.u32MIPMapCount; ++uiMIPLevel)
			{
				//Get the current MIP size.
				...
				{
					//Upload the texture
					glTexImage2D(GL_TEXTURE_2D,uiMIPLevel-nLoadFromLevel,eTextureInternalFormat, u32MIPWidth, u32MIPHeight, 0, eTextureFormat, eTextureType, pTempData);
				}
