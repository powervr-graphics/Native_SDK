...
void PVRShell::PVRShellOutputDebug(char const * const format, ...) const
{
	...
	va_start(arg, format);
	size_t bufSize = _vscprintf(format,arg);
	va_end(arg);

	char *buffer = new char[bufSize + 1];

	va_start(arg, format);
	vsnprintf(buffer, bufSize + 1, format, arg);
	va_end(arg);

	// Passes the data to a platform dependant function
	m_pShellInit->OsDisplayDebugString(buffer);
	delete[] buffer;
}
...
void PVRShellInit::OsDisplayDebugString(char const * const str)
{
	if(str)
	{
		...
		wchar_t *wstr = new wchar_t[bufSize + 1];

		for(; (str[i] != '\0') && (i < bufSize); ++i)
		{
