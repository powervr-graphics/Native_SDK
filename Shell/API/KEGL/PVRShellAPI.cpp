...
bool PVRShellInit::ApiInitAPI()
{
	...
	do
	{
		...
		do
		{
			...
			int myApiMajorVersion = m_pShell->PVRShellGet(prefApiMajorVersion);
			int myApiMinorVersion = m_pShell->PVRShellGet(prefApiMinorVersion);
			if (!myApiMajorVersion)
			{
			...
			if(myApiMajorVersion)
			{
				...
				ai32ContextAttribs[i++] = myApiMajorVersion;
			}
			...
			if (PVRShellIsExtensionSupported(m_EGLDisplay, "EGL_KHR_create_context"))
			{
				...
				ai32ContextAttribs[i++] = myApiMinorVersion;
#else
				//Gracefully fail if
				if(myApiMinorVersion)
				{
					m_pShell->PVRShellSet(prefExitMessage, "PVRShell: GL Context version %d.%d requested but module was built without EGL_KHR_create_context support.\n");
					return false;
				}
#endif
#if defined(BUILD_OGL)
			...
			else 
			{
				if (myApiMinorVersion)
				{
					m_pShell->PVRShellSet(prefExitMessage, "PVRShell: GL Context version %d.%d requested but EGL_KHR_create_context was not supported.\n");
					return false;
				}
			}
...
bool PVRShellInit::ApiSet(const prefNameIntEnum prefName, const int i32Value)
{
	...
	{
#ifdef EGL_VERSION_1_1
		case prefSwapInterval: m_pShell->m_pShellData->nSwapInterval = i32Value; return true;
#endif
#if defined(BUILD_OGLES) || defined(BUILD_OGLES2) || defined(BUILD_OGLES3)
		case prefPriority: m_pShell->m_pShellData->nPriority = i32Value; return true;
#endif
		case prefRequestedConfig: m_iRequestedConfig = (EGLint) i32Value; return true;
...
bool PVRShellInit::ApiGet(const prefNameIntEnum prefName, int* pn)
{
	switch (prefName)
	{
		case prefEGLMajorVersion: *pn = (int) m_EglMajorVersion; return true;
		case prefEGLMinorVersion: *pn = (int) m_EglMinorVersion; return true;
		...
		case prefRequestedConfig: *pn = (int) m_iRequestedConfig; return true;
		case prefConfig: *pn = (int) m_iConfig; return true;
	}
	return false;
}
...
bool PVRShellInit::ApiGet(const prefNamePtrEnum prefName, void** pp)
{
	switch (prefName)
	{
		case prefEGLDisplay: *pp = (void*)m_EGLDisplay; return true;
		case prefEGLSurface: *pp = (void*)m_EGLWindow; return true;
	}
	return false;
}
