# Findpugixml.cmake
#
# Finds the pugixml library and its dependencies
#
# This will define the following imported targets
#     pugixml

if(PVR_PREBUILT_DEPENDENCIES)
	if(ANDROID)
		set(CMAKE_FIND_ROOT_PATH_MODE_PACKAGE BOTH)
		string(TOLOWER ${CMAKE_BUILD_TYPE} PVR_ANDROID_BUILD_TYPE)
		file(GLOB pugixml_DIR_GLOB "${CMAKE_CURRENT_LIST_DIR}/../../external/pugixml/build-android/.cxx/${CMAKE_BUILD_TYPE}/*/${ANDROID_ABI}/build")
		file(GLOB pugixml_DIR_GLOB2 "${CMAKE_CURRENT_LIST_DIR}/../../external/pugixml/build-android/.cxx/${PVR_ANDROID_BUILD_TYPE}/*/${ANDROID_ABI}/build")
		list(APPEND pugixml_DIR_GLOB ${pugixml_DIR_GLOB2})

		set(pugixml_DIR "")
		foreach(DIR IN LISTS pugixml_DIR_GLOB)
			if(EXISTS "${DIR}/pugixmlConfig.cmake" AND EXISTS "${DIR}/pugixmlTargets.cmake")
				set(pugixml_DIR "${DIR}")
				break()
			endif()
		endforeach()
	endif()
endif()

if(NOT TARGET pugixml)
	if(pugixml_DIR)
		find_package(pugixml REQUIRED CONFIG)
	else()
		# Fallback to building from the bundled source if prebuilt not found
		if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/../../external/pugixml/CMakeLists.txt")
			set(PVR_PREBUILT_DEPENDENCIES_CACHED ${PVR_PREBUILT_DEPENDENCIES})
			set(PVR_PREBUILT_DEPENDENCIES OFF)
			add_subdirectory("${CMAKE_CURRENT_LIST_DIR}/../../external/pugixml" "${CMAKE_BINARY_DIR}/external/pugixml")
			set(PVR_PREBUILT_DEPENDENCIES ${PVR_PREBUILT_DEPENDENCIES_CACHED})
		endif()
	endif()
endif()

if(TARGET pugixml)
    get_filename_component(PUGI_INC_DIR "${CMAKE_CURRENT_LIST_DIR}/../../external/pugixml/source/src" ABSOLUTE)
    set_property(TARGET pugixml APPEND PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${PUGI_INC_DIR}")
    set(pugixml_FOUND TRUE)
endif()
