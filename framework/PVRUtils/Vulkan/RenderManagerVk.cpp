...
inline std::pair<StringHash, effect::PipelineDef*> selectPipelineForSubpassGroupMeshMaterial(
	effect::EffectApi& effect, const effect::SubpassGroup& pipes, const assets::Mesh& mesh, const assets::Material& material)
{
...
inline std::pair<StringHash, const effect::PipelineDef*> selectPipelineForSubpassGroupMeshMaterial(
	effect::EffectApi& effect, uint32_t passId, uint32_t subpassId, uint32_t subpassGroupId, const assets::Mesh& mesh, const assets::Material& material)
{
...
inline void populateVbos(AttributeConfiguration& attribConfig, pvr::DynamicArray<Buffer>& vbos, assets::Mesh& mesh)
{
			...
			if (mesh.getFaces().getDataSize() > 0)
			{
				apimesh.ibo =
					pvr::utils::createBuffer(device, mesh.getFaces().getDataSize(), pvrvk::BufferUsageFlags::e_INDEX_BUFFER_BIT, pvrvk::MemoryPropertyFlags::e_HOST_VISIBLE_BIT,
						...
						&renderman.getBufferAllocator(), pvr::utils::vma::AllocationCreateFlags::e_MAPPED_BIT);
...
inline std::vector<StringHash> getVertexBindingsForPipeline(const effect::EffectApi& effect, const StringHash& pipelineName)
{
...
inline std::vector<StringHash> getAllActiveVertexAttributesForMeshAndEffect(effect::EffectApi& effect, const assets::Mesh& mesh, const assets::Material& material)
{
	...
	for (uint32_t pass = 0; pass < effect->getNumPasses(); ++pass)
	{
		for (uint32_t subpass = 0; subpass < effect->getPass(pass).subpasses.size(); ++subpass)
		{
			...
			{
				std::pair<StringHash, const effect::PipelineDef*> pipe = selectPipelineForSubpassGroupMeshMaterial(effect, pass, subpass, subpassGroup, mesh, material);
...
inline AttributeConfiguration getVertexBindingsForPipeNoStride(const effect::EffectApi& effect, const StringHash& pipelineName)
{
...
inline AttributeConfiguration getVertexBindingsForPipe(const effect::EffectApi& effect, const StringHash& pipelineName)
{
	AttributeConfiguration retval;
	auto& pipe = effect->getEffectAsset().versionedPipelines.find(effect->getApiString())->second.find(pipelineName)->second;
	uint32_t count[32] = {};
...
inline void addUniformSemanticLists(
	pvr::ContiguousMap<StringHash, effect::UniformSemantic>& effectlist, pvr::ContiguousMap<StringHash, UniformSemantic>& newlist, bool checkDuplicates, VariableScope scope)
{
...
#endif
								for (uint32_t dynamicClient = 0; dynamicClient < node.dynamicClientId[setid].size();
									 ++dynamicClient) // "dynamic client" is basically the buffer index.
								{
#if defined(PVR_RENDERMANAGER_DEBUG_BUFFERS) || defined(PVR_RENDERMANAGER_DEBUG_RENDERING_COMMANDS)
								...
								for (auto& tex : pipedef.textureSamplersByTexSemantic)
								{
									...
									if ((texIndex = materialeffect.material->assetMaterial->getTextureIndex(tex.first)) != -1)
									{
										...
										auto imgView = utils::loadAndUploadImageAndView(device, texturePath.c_str(), true, cmdBuffer, renderman.getAssetProvider(),
											pvrvk::ImageUsageFlags::e_SAMPLED_BIT, pvrvk::ImageLayout::e_SHADER_READ_ONLY_OPTIMAL, nullptr, &renderman.getBufferAllocator(),
											&renderman.getImageAllocator());
...
namespace {
inline RendermanBufferBinding makeRendermanBufferBinding(const StringHash& name, const effect::BufferRef& ref, std::deque<RendermanBufferDefinition>& definitions)
{
...
size_t addRendermanPipelineIfNotExists(RendermanSubpassGroup& subpassGroup, effect::PipelineDef* pipedef, RendermanEffect& renderEffect, bool& isnew)
{
				...
				for (uint8_t subpassGroupid = 0; subpassGroupid < effectsubpass.groups.size(); ++subpassGroupid)
				{
					...
					for (uint32_t modelId = 0; modelId < renderSubpassGroup.allModels.size(); ++modelId)
					{
						...
						for (uint32_t nodeId = 0; nodeId < rendermodel.assetModel->getNumMeshNodes(); ++nodeId)
						{
							...
							std::pair<StringHash, effect::PipelineDef*> pipe = selectPipelineForSubpassGroupMeshMaterial(effectapi, effectSubpassGroup, assetmesh, assetmaterial);
	...
	for (uint32_t setid = 0; setid < FrameworkCaps::MaxDescriptorSetBindings; ++setid)
	{
		if (!recordBindDescriptorSets || recordBindDescriptorSets[setid])
		{
			...
			cbuff->bindDescriptorSet(pvrvk::PipelineBindPoint::e_GRAPHICS, pipe.apiPipeline->getPipelineLayout(), setid, pipelineMaterial_->sets[setid][setswapid],
				dynamicOffset.data(), dynamicOffset.size());
