...
struct PipelineDef
{
	...
	pvr::ContiguousMap<StringHash, BufferRef> modelScopeBuffers; //!< All model-scope buffers (buffers whose data are sourced by the model, or in general must be changed every time
	...
	pvr::ContiguousMap<StringHash, BufferRef>
		nodeScopeBuffers; //!< All node-scope buffers (buffers whose data change with every node / are sourced by the node). First item is buffer name
	pvr::ContiguousMap<StringHash, BufferRef> batchScopeBuffers; //!< All batch-scope buffers (buffers whose data change with each bone batch. Normally, only legacy-exported bones
...
public:
	...
	void init(const assets::effect::Effect& effect, pvrvk::Swapchain& swapchain, pvrvk::CommandBuffer& cmdBuffer, IAssetProvider& assetProvider,
		utils::vma::Allocator& bufferAllocator, utils::vma::Allocator& imageAllocator);
	...
	const assets::effect::Effect& getEffectAsset() const
	{
		return _assetEffect;
	}
