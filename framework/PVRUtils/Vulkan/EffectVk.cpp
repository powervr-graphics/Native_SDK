...
inline void addTexture(Effect_& effect, const assets::effect::TextureReference& textureRef, std::map<StringHash, ImageView>& textures, const StringHash& pipeline,
...
inline void addBuffer(Effect_& effect, effect::PipelineDef& pipedef, const assets::effect::BufferRef& bufferRef, std::map<StringHash, BufferDef>& buffers, uint32_t swapchainLength)
{
...
inline void createBuffers(Effect_& effect, std::map<StringHash, effect::PipelineDef>& createParams, std::map<StringHash, BufferDef>& buffers, uint32_t swapchainLength)
{
	const Effect_::AssetEffect assetEffect = effect.getEffectAsset();
...
bool getRenderPassAndFramebufferForPass(Effect_& effect, const assets::effect::Effect& effectAsset, const assets::effect::Pass& pass, Framebuffer* framebuffers, RenderPass& rp,
	std::vector<std::pair<StringHash, uint32_t> /**/>& colorAttachmentIndex)
{
	...
	Swapchain swapchain = effect.getSwapchain();
	{
		...
		std::vector<assets::effect::TextureDefinition> colorAttachments(swapchainTargetIndex != -1 ? colorAttachmentsSet.size() + 1 : colorAttachmentsSet.size());
		...
		{
			colorAttachments[0] = assets::effect::TextureDefinition("default", "", framebufferWidth, framebufferHeight, ImageDataFormat()); // add a dummy object here.
		}
		...
		for (uint32_t i = 0; i < colorAttachments.size(); ++i)
		{
			...
			for (uint32_t swapChainId = 0; swapChainId < swapchain->getSwapchainLength(); ++swapChainId)
			{
				...
				else
				{
					...
					auto found = std::find_if(inputAttachments.begin(), inputAttachments.end(),
						[&](const assets::effect::TextureDefinition& texDefinition) { return (texDefinition.name == texDef.name); });
...
void createPasses(Effect_& effect, std::vector<Pass>& passes, std::map<StringHash, PipelineLayout>& layouts, std::map<StringHash, effect::PipelineDef>& createParams,
	std::map<StringHash, std::map<StringHash, effect::TextureInfo> /**/>& samplersIndexedByPipeAndTexture, uint32_t swapchainLength)
{
				...
				for (auto pipe_it = supassGroup_it->pipelines.begin(); pipe_it != supassGroup_it->pipelines.end(); ++pipe_it, ++pipe_idx)
				{
					...
					for (uint32_t i = 0; i < assets::effect::Subpass::MaxInputs; ++i)
					{
						...
						if (!name.empty())
						{
							...
							{
								std::vector<assets::effect::InputAttachmentRef>::const_iterator assetAtatchmentRef = std::find_if(pipedef->inputAttachments.begin(),
									pipedef->inputAttachments.end(), [=](const assets::effect::InputAttachmentRef& ref) { return ref.targetIndex == static_cast<int8_t>(i); });
					...
					{
						ShaderModule shader =
							effect.getDevice()->createShader(BufferStream("VertexShader", (*shader_it)->source.data(), (*shader_it)->source.length()).readToEnd<uint32_t>());
						if (shader.isNull())
						{
...
void Effect_::init(const assets::effect::Effect& effect, Swapchain& swapchain, CommandBuffer& cmdBuffer, IAssetProvider& assetProvider, pvr::utils::vma::Allocator& bufferAllocator,
