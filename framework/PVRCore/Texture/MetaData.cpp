...
TextureMetaData::TextureMetaData(uint32_t fourCC, uint32_t key, uint32_t dataSize, const char* data) : _fourCC(0), _key(0), _dataSize(0), _data(NULL)
{
...
TextureMetaData::TextureMetaData(const TextureMetaData& rhs) : _fourCC(0), _key(0), _dataSize(0), _data(NULL)
{
	// Copy the data across.
	if (rhs._dataSize)
	{
...
void TextureMetaData::loadFromStream(Stream& stream)
{
	size_t dataRead = 0;
	...
	stream.read(1, _dataSize, _data, dataRead);
}

void TextureMetaData::writeToStream(Stream& stream) const
{
	size_t dataRead = 0;
	stream.write(sizeof(_fourCC), 1, &_fourCC, dataRead);
	stream.write(sizeof(_key), 1, &_key, dataRead);
	stream.write(sizeof(_dataSize), 1, &_dataSize, dataRead);

	if (_dataSize <= 0)
	{
		throw FileIOError("TextureMetadata::writeToStream: Could not write header to stream.");
	}

	stream.write(1, _dataSize, _data, dataRead);
}
