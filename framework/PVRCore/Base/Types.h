...
inline uint64_t getTotalSizeAfter(GpuDatatypes type, uint32_t arrayElements, uint64_t previousTotalSize)
{
	...
	uint64_t selfAlignedSize =
		getSelfAlignedArraySize(type) * arrayElements * (arrayElements != 1); // Equivalent to: if (arrayElements!=1) selfAlign = getSelfAl... else selfAlign=0
