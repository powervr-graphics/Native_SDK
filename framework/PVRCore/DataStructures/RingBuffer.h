...
class RingBuffer
{
	//!\cond NO_DOXYGEN
	template<typename>
	friend void swap(RingBuffer<ItemType>, RingBuffer<ItemType>);
	//!\endcond
...
public:
	...
	ItemType& back()
	{
		return _store[getIndex(_size - 1)];
	}
	...
	RingBuffer(const RingBuffer& rhs) : _store(static_cast<ItemType*>(malloc(rhs._capacity * sizeof(ItemType)))), _first(0), _size(rhs._size), _capacity(rhs._capacity)
	{
...
private:
	...
	static void copy_items(ItemType* from, ItemType* to, size_t number, size_t from_start, size_t from_capacity)
	{
		for (size_t i = 0; i < number; ++i)
		{
			...
			++from_start;
			if (from_start >= from_capacity)
			{
				from_start -= from_capacity;
			}
		}
	}
};
