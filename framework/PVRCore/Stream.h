...
public:
	...
	/// <param name="numElements">The maximum number of elements to read.</param>
	/// <param name="buffer">The buffer into which to write the data.</param>
	virtual void readExact(size_t elementSize, size_t numElements, void* buffer)
	{
		size_t dataRead;
		read(elementSize, numElements, buffer, dataRead);
		if (dataRead != numElements)
		{
			throw FileIOError(*this, std::string("Stream::readExact: Failed to read specified number of elements."));
		}
	}
	...
	/// <returns>A std::vector<Type_> containing all data from the current point to the end of the stream.</returns>
...
/// <summary>Constructor</summary>
/// <param name="stream">The stream being used.</param>
inline FileIOError::FileIOError(Stream& stream) : PvrError("[" + stream.getFileName() + "]: File IO operation failed") {}
/// <summary>Constructor</summary>
/// <param name="stream">The stream being used.</param>
/// <param name="message">A message to log alongside the exception message.</param>
inline FileIOError::FileIOError(Stream& stream, std::string message) : PvrError("[" + stream.getFileName() + "] : File IO operation failed - " + message) {}
/// <summary>Constructor</summary>
/// <param name="stream">The stream being used.</param>
