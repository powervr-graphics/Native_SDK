...
inline bool isVariableTypeSigned(VariableType item)
{
	return static_cast<uint32_t>(item) < 11 ? static_cast<uint32_t>(item) & 1 : static_cast<uint32_t>(item) != 13;
}
...
inline bool isVariableTypeNormalized(VariableType item)
{
...
public:
	...
	{
		return _format._pixelTypeChar[7] ? 4 : _format._pixelTypeChar[6] ? 3 : _format._pixelTypeChar[5] ? 2 : _format._pixelTypeChar[4] ? 1 : 0;
	}
	...
	{
		return getPixelTypeId() == rhs.getPixelTypeId();
	}
