...
struct PipelineLayoutCreateInfo
{
	...
	PipelineLayoutCreateInfo& setDescSetLayout(uint32_t index, const DescriptorSetLayout& descLayout)
	{
		...
		{
			size = static_cast<uint8_t>(index) + 1;
		}
	...
	bool operator==(const PipelineLayoutCreateInfo& rhs) const
	{
		...
		for (size_t i = 0; i < size; ++i)
		{
			if (_descLayout[i] != rhs._descLayout[i])
			{
				return false;
			}
		}
	...
	uint32_t getNumPushConstantRanges() const
	{
		return static_cast<uint32_t>(_pushConstantRange.size());
	}
...
private:
	bool isValidPushConstantRange(uint32_t index)
	{
		return _pushConstantRange[index].getSize() != 0;
	}
