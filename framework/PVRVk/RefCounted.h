/*!
\brief A smart pointer implementation very close to the spec of
std::shared_ptr but with some key differences and tweaks to make
it more suitable for the PowerVR Framework. Shadows the PVRCore
respective class
\file PVRVk/RefCounted.h
...
/// <summary>A simple compile time "assert-in-debug-nop-in-release".</summary>
#ifdef DEBUG
#define PVR_REFCOUNTED_ASSERT(a) assert(a)
#else
#define PVR_REFCOUNTED_ASSERT(a) void(0)
#endif
...
private:
	mutable volatile std::atomic<int32_t> _count; //!< Number of total references for this object
	mutable volatile std::atomic<int32_t> _weakcount; //!< Number of weak references for this object
	bool _is_deleting; //!< This flags protects from recursive deletion : If the last weak ref to an object is released during its own destroyObject
	std::recursive_mutex _mylock;
...
protected:
	...
	{
		if (std::atomic_fetch_add(&_count, 1) == 0)
		{
			std::lock_guard<std::recursive_mutex> lockguard(_mylock);
			PVR_REFCOUNTED_ASSERT(_count.load() != 0 && "RefCounted::increment_count:  Tried to increment the count of an object but it had already been destroyed!");
		}
	} //!< Number of total references for this object
	void increment_weakcount()
	{
		if (std::atomic_fetch_add(&_weakcount, 1) == 0)
		{
			std::lock_guard<std::recursive_mutex> lockguard(_mylock);
			PVR_REFCOUNTED_ASSERT(_weakcount.load() != 0 && "RefCounted::increment_weakcount:  Tried to increment the count of an object but it had already been destroyed!");
		}
	} //!< Number of weak references for this object
	...
	void decrement_count()
	{
		...
		int32_t cnt = 0;
		{
			if ((cnt = std::atomic_fetch_add(&_count, -1)) == 1)
			{
				std::lock_guard<std::recursive_mutex> lockguard(_mylock);
				_is_deleting = true; // This flag tests notifies the decrement_weakcount that the object is deleting itself
				destroyObject();
				_is_deleting = false;
				if (weakcount() == 0)
				{
	...
	{
		bool deleter = false; // DO NOT DESTROY THE OBJECT WHILE THE LOCK IS HELD!
		if (((std::atomic_fetch_add(&_weakcount, -1)) == 1))
		{
			std::lock_guard<std::recursive_mutex> lockguard(_mylock);
			if (_count.load() == 0 && !_is_deleting) // Test to the is_deleting means that decrement_count will actually delete the entry
			{
				deleter = true;
			}
		}
	...
	IRefCountEntry() : _count(1), _weakcount(0), _is_deleting(false) {}
};
...
struct RefCountEntryIntrusive : public IRefCountEntry
{
	union
	{
		...
		char _entry[sizeof(MyClass_)]; //!< Represents enough bytes of memory to store a MyClass_ object.
	};
	...
	{
		pointee = new (_entry) MyClass_(std::forward<Args>(args)...);
	}
...
protected:
	...
	{
		_pointee = const_cast<MyClass_*>(pointee);
	}

public:
	MyClass_& operator*()
	{
		PVR_REFCOUNTED_ASSERT(_pointee != 0 && "Dereferencing NULL pointer");
		return *_pointee;
	}
...
protected:
	...
	RefcountEntryHolder(IRefCountEntry* _refCountEntry) : _refCountEntry(_refCountEntry) {}
};
...
public:
	uint32_t refcount()
	{
		return RefcountEntryHolder<MyClass_>::_refCountEntry->count();
	}
	uint32_t weakrefcount()
	{
		return RefcountEntryHolder<MyClass_>::_refCountEntry->weakcount();
	}
	...
	bool isValid() const
	{
		return RefcountEntryHolder<MyClass_>::_refCountEntry != NULL && RefcountEntryHolder<MyClass_>::_refCountEntry->count() > 0;
	}
	...
	{
		IRefCountEntry* tmpRefCountEntry = RefcountEntryHolder<MyClass_>::_refCountEntry;
		RefcountEntryHolder<MyClass_>::_refCountEntry = rhs._refCountEntry;
		rhs._refCountEntry = tmpRefCountEntry;
	...
	{
		if (RefcountEntryHolder<MyClass_>::_refCountEntry)
		{
			RefcountEntryHolder<MyClass_>::_refCountEntry->increment_count();
			PVR_REFCOUNTED_ASSERT(
				RefcountEntryHolder<MyClass_>::_refCountEntry->count() >= 0 && RefcountEntryHolder<MyClass_>::_refCountEntry->weakcount() >= 0 && "BUG - Count was negative.");
		}
	}
	...
	{
		RefcountEntryHolder<MyClass_>::_refCountEntry->_count = 1;
	}
	...
	EmbeddedRefCountedResource(const RefCountedWeakReference<MyClass_>& rhs) : Dereferenceable<MyClass_>(rhs), RefcountEntryHolder<MyClass_>(rhs)
	{
		if (rhs._refCountEntry)
		{
			rhs._refCountEntry->increment_count();
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::_refCountEntry->weakcount() > 0 && "Reference count was not positive");
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::_refCountEntry->count() > 0 && "Reference count was not positive");
		}
	}
	...
	{
		swap(rhs);
		PVR_REFCOUNTED_ASSERT((!RefcountEntryHolder<MyClass_>::_refCountEntry ||
								  (RefcountEntryHolder<MyClass_>::_refCountEntry->count() >= 0 && RefcountEntryHolder<MyClass_>::_refCountEntry->weakcount() >= 0)) &&
	...
	EmbeddedRefCountedResource(const EmbeddedRefCountedResource<OldType_>& rhs, typename std::enable_if<std::is_convertible<OldType_*, MyClass_*>::value, void*>::type* = 0)
		: Dereferenceable<MyClass_>(rhs._pointee), RefcountEntryHolder<MyClass_>(rhs._refCountEntry)
	{
	...
	explicit EmbeddedRefCountedResource(const EmbeddedRefCountedResource<OldType_>& rhs, typename std::enable_if<!std::is_convertible<OldType_*, MyClass_*>::value, void*>::type* = 0)
		: Dereferenceable<MyClass_>(static_cast<MyClass_*>(rhs._pointee)), RefcountEntryHolder<MyClass_>(rhs._refCountEntry)
	{
		// Compile-time check: are these types compatible?
	...
	{
		reset();
		RefcountEntryHolder<MyClass_>::_refCountEntry = new RefCountEntry<MyPointer_>(ref);
...
protected:
	...
	EmbeddedRefCountedResource(IRefCountEntry* _entry, OriginalType* pointee) : Dereferenceable<MyClass_>(static_cast<MyClass_*>(pointee)), RefcountEntryHolder<MyClass_>(_entry)
	{
		if (RefcountEntryHolder<MyClass_>::_refCountEntry)
		{
			PVR_REFCOUNTED_ASSERT(
...
private:
	void releaseOne()
	{
		if (RefcountEntryHolder<MyClass_>::_refCountEntry)
		{
			IRefCountEntry* tmp = RefcountEntryHolder<MyClass_>::_refCountEntry;
			RefcountEntryHolder<MyClass_>::_refCountEntry = 0;
...
public:
	...
	/// between objects. For example, if we had a RefCountedResource&lt;vector&lt;Model&gt;&gt; (that is never
	...
	/// vector&lt;Mesh&gt; that does NOT contain it, then a) The object will not be properly destroyed when its
	...
	/// <param name="resource">The object from which to save the refcount</param>
	/// <param name="pointee">The object which will be returned when dereferencing</param>
	template<typename OriginalType>
	void shareRefCountFrom(const EmbeddedRefCountedResource<OriginalType>& resource, MyClass_* pointee)
	{
		shareRefCountingFrom(RefcountEntryHolder<OriginalType>(resource)._refCountEntry, pointee);
	}
	...
	/// <param name="pointee">The object which will be returned when dereferencing</param>
	template<typename OriginalType>
	void shareRefCountFrom(EmbeddedRefCountedResource<OriginalType> resource, MyClass_* pointee)
	{
		shareRefCountingFrom(RefcountEntryHolder<OriginalType>(resource)._refCountEntry, pointee);
	}

private:
	void shareRefCountingFrom(IRefCountEntry* _entry, ElementType* pointee)
	{
		EmbeddedRefCountedResource<MyClass_>::reset();
		RefcountEntryHolder<ElementType>::_refCountEntry = _entry;
		_entry->increment_count();
		PVR_REFCOUNTED_ASSERT(
			RefcountEntryHolder<MyClass_>::_refCountEntry->count() > 0 && RefcountEntryHolder<MyClass_>::_refCountEntry->weakcount() >= 0 && "BUG - Count was negative.");
	...
	template<typename OriginalType>
	RefCountedResource(const IRefCountEntry* _entry, OriginalType* pointee) : EmbeddedRefCountedResource<MyClass_>(_entry, pointee)
	{}
};
...
public:
	...
	{
		return RefcountEntryHolder<MyClass_>::_refCountEntry != NULL && RefcountEntryHolder<MyClass_>::_refCountEntry->_is_deleting;
	}
	...
	RefCountedWeakReference(const RefCountedWeakReference& rhs) : Dereferenceable<MyClass_>(rhs), RefcountEntryHolder<MyClass_>(rhs)
	{
		if (RefcountEntryHolder<MyClass_>::_refCountEntry)
		{
			RefcountEntryHolder<MyClass_>::_refCountEntry->increment_weakcount();
			PVR_REFCOUNTED_ASSERT(RefcountEntryHolder<MyClass_>::_refCountEntry->weakcount() > 0 && "BUG - Reference count was nonpositive.");
		}
	}
	...
	explicit RefCountedWeakReference(const EmbeddedRefCountedResource<OldType_>& rhs, typename std::enable_if<!std::is_convertible<OldType_*, MyClass_*>::value, void*>::type* = 0)
		: Dereferenceable<MyClass_>((MyClass_*)(rhs._pointee)), RefcountEntryHolder<MyClass_>(rhs._refCountEntry)
	{
		// Compile-time check: are these types compatible?
		OldType_* checkMe = static_cast<MyClass_*>(Dereferenceable<MyClass_>::_pointee);
		(void)checkMe;
...
private:
	...
	template<typename OriginalType>
	RefCountedWeakReference(IRefCountEntry* _entry, OriginalType* pointee) : Dereferenceable<MyClass_>(static_cast<MyClass_*>(pointee)), RefcountEntryHolder<MyClass_>(_entry)
	{
		if (RefcountEntryHolder<MyClass_>::_refCountEntry)
		{
			RefcountEntryHolder<MyClass_>::_refCountEntry->increment_weakcount();
...
protected:
	...
	// FORWARD TO THIS CLASS DIRECTLY FROM YOUR OWN FACTORY.
	// USE THE FOLLOWING IMPLEMENTATION FOR YOUR FACTORY FUNCTIONS
	// StrongReferenceType createNew(MyParamType myparams)
	// {
	//     return MyEmbeddedType::createNew(std::forward(myparams));
	// }
	template<typename... Params>
