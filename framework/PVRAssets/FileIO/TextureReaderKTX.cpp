...
void TextureReaderKTX::readNextAsset(Texture& asset)
{
	if (_assetStream->getSize() < texture_ktx::c_expectedHeaderSize)
	{
		throw InvalidOperationError("[TextureReaderKTX::readNextAsset]: File stream was shorter than KTX file length");
	}
	...
	{
		throw InvalidOperationError("[TextureReaderKTX::readNextAsset]: Stream did not contain a valid KTX file identifier");
	}
	...
	{
		throw InvalidOperationError("[TextureReaderKTX::readNextAsset]: Stream did not match KTX file endianness");
	}
	...
	if (ktxFileHeader.bytesOfKeyValueData > 0)
	{
		...
		{
			throw InvalidOperationError("[TextureReaderKTX::readNextAsset]: Stream metadata were invalid");
		}
	}
	...
	for (uint32_t mipMapLevel = 0; mipMapLevel < ktxFileHeader.numMipmapLevels; ++mipMapLevel)
	{
		...
		if (asset.getNumFaces() == 6 && asset.getNumArrayMembers() == 1)
		{
			...
			{
				throw InvalidOperationError("[TextureReaderKTX::readNextAsset]: Mipmap size read was not expected size.");
			}
		}
		else
		{
				...
				for (uint32_t iFace = 0; iFace < asset.getNumFaces(); ++iFace)
				{
					for (uint32_t texDepth = 0; texDepth < asset.getDepth(); ++texDepth)
					{
						for (uint32_t texHeight = 0; texHeight < asset.getHeight(); ++texHeight)
						{
							...
							uint64_t scanLineOffset = (textureOffset3D(0, texHeight, texDepth, asset.getWidth(), asset.getHeight()) * (asset.getBitsPerPixel() / 8));
