...
void TextureReaderXNB::readNextAsset(Texture& asset)
{
	if (_hasNewAssetStream)
	{
		...
		{
			throw InvalidOperationError("[TextureReaderXNB::readNextAsset]: Header was not loaded correctly from the Stream");
		}
	}
	...
	else
	{
		...
		throw InvalidDataError("[TextureReaderXNB::readNextAsset]: Could not determine the texture type - was none of 2D, 3D or Cube");
	}
