...
void addPipelineVertexInputBinding(effect::Effect&, const StringHash&, effect::PipelineDefinition& pipeline, pugi::xml_node& attribute_element)
{
	pipeline.vertexBinding.push_back(assets::effect::PipelineVertexBinding(
		attribute_element.attribute("index").as_uint(), stepRateFromString(attribute_element.attribute("stepRate").as_string(""), StepRate::Vertex)));
}
...
void PfxParser::readNextAsset(effect::Effect& asset)
{
	asset.clear();
	...
	{
		throw InvalidDataError("[PfxParser::readNextAsset]: Failed to parse PFX file - not valid XML");
	}
	if (!doc.root().first_child() || std::string(doc.root().first_child().name()) != std::string("pfx"))
	{
		throw InvalidDataError("[PfxParser::readNextAsset]: Failed to parse PFX file - root <pfx> element not found");
	}
