...
TextureReaderPVR::TextureReaderPVR(Stream::ptr_type assetStream) : AssetReader<Texture>(std::move(assetStream)), _texturesToLoad(true) {}

void TextureReaderPVR::readNextAsset(Texture& asset)
{
	...
	if (version == TextureHeader::Header::PVRv3)
	{
		...
		while (metaDataRead < tempMetaDataSize)
		{
			...
			metaDataBlock.loadFromStream(*_assetStream);
		...
		{
			throw InvalidDataError("[TextureReaderPVR::readNextAsset] Metadata seems to be corrupted while reading.");
		}
	...
	{
		throw InvalidOperationError("[TextureReaderPVR::readNextAsset]: Unsupported PVR Version");
	}
}
...
bool TextureReaderPVR::isSupportedFile(Stream& assetStream)
{
	...
	case texture_legacy::c_headerSizeV2:
		return dataRead == texture_legacy::c_headerSizeV2 && header.pvrMagic == texture_legacy::c_identifierV2;

	case TextureHeader::Header::PVRv3:
		return dataRead >= std::min(texture_legacy::c_headerSizeV2, static_cast<uint32_t>(TextureHeader::Header::SizeOfHeader));
	}

	return false;
}

vector<std::string> TextureReaderPVR::getSupportedFileExtensions()
{
...
void TextureReaderPVR::convertTextureHeader2To3(const texture_legacy::HeaderV2& legacyHeader, TextureHeader& newHeader)
{
	...
	if (pvrTextureHeaderV3.numSurfaces == 0)
	{
		pvrTextureHeaderV3.numSurfaces = 1;
	}
...
void TextureReaderPVR::mapLegacyEnumToNewFormat(
	const texture_legacy::PixelFormat legacyPixelType, PixelFormat& pixelType, ColorSpace& colorSpace, VariableType& channelType, bool& isPremultiplied)
{
	// Default value.
