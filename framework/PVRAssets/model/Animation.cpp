...
uint32_t AnimationData::getNumFrames() const
{
	return _data.numFrames;
}

const uint32_t* AnimationData::getPositionIndices() const
{
	return _data.positionIndices.data();
}

const uint32_t* AnimationData::getRotationIndices() const
{
	return _data.rotationIndices.data();
}

const uint32_t* AnimationData::getScaleIndices() const
{
	return _data.scaleIndices.data();
}

const uint32_t* AnimationData::getMatrixIndices() const
{
	return _data.matrixIndices.data();
}

uint32_t AnimationData::getFlags() const
{
	return _data.flags;
}

AnimationData::InternalData& AnimationData::getInternalData()
{
	return _data;
}
...
void AnimationInstance::updateAnimation(float time)
{
	...
	for (uint32_t i = 0; i < keyframeChannels.size(); ++i)
	{
		...
		if (time <= keyFrame.timeInSeconds[0])
		{
		...
		{
			glm::vec3 scale(1.0f);
			if (interp == KeyFrameData::InterpolationType::Step)
			{
				scale = keyFrame.scale[f1];
			}
			...
			{
				static_cast<Node*>(keyframeNodes.nodes[i])->getInternalData().getFrameScaleAnimation() = scale;
			}
		}
		else if (keyFrame.rotate.size())
		{
			...
			for (uint32_t i = 0; i < keyframeNodes.nodes.size(); ++i)
			{
				static_cast<Node*>(keyframeNodes.nodes[i])->getInternalData().getFrameRotationAnimation() = quat;
			}
		}
		...
		else if (keyFrame.translation.size())
		{
			...
			for (uint32_t i = 0; i < keyframeNodes.nodes.size(); ++i)
			{
				Node& n = *static_cast<Node*>(keyframeNodes.nodes[i]);
