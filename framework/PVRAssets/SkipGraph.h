...
public:
	...
	{
		return static_cast<uint32_t>(_apDependencies.size());
	}
...
private:
	...
	bool addDependent(SkipGraphNode* pDependencyNode)
	{
		...
		{
			if (_apDependencies[ui] == pDependencyNode)
			{
				return true;
			}
		}
...
private:
	...
	public:
		...
		SkipGraphNode<T>& getNode()
		{
			return _skipGraphNode;
		}

		/// <summary>Get the node associated with this element.</summary>
		/// <returns>Return a reference to the associated node</returns>
		const SkipGraphNode<T>& getNode() const
		{
			return _skipGraphNode;
		}
...
public:
	...
	{
		return static_cast<uint32_t>(_aHashTable.size());
	}
	...
	void RetreiveSortedDependencyList(std::vector<T>& aOutputArray, const uint32_t nodeID)
	{
	...
	/// <param name="nodeId">The index of the node to retrieve</param>
	/// <returns>Return handle to the node data</returns>
	const T& operator[](uint32_t nodeId) const
	{
		return *(getNodeData(nodeId));
	}
...
private:
	...
	SkipGraphNode<T>* findNode(const size_t hash)
	{
		...
		{
			if (_aHashTable[i].getHash())
			{
				return &_aHashTable[i].getNode();
			}
		}
