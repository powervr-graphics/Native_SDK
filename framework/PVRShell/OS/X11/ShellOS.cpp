...
{
	if (keycode >= sizeof(X11_To_Keycode) / sizeof(X11_To_Keycode[0]))
	{
		return Keys::Unknown;
	}
	...
	do
	{
		...
		if (res < 0)
		{
			Log(LogLevel::Warning,
				"Readlink %s failed. The application name, "
...
bool ShellOS::initializeWindow(DisplayAttributes& data)
{
	...
	*/
	...
	if (data.x == static_cast<uint32_t>(DisplayAttributes::PosDefault))
	{
		data.x = 0;
	}
	if (data.y == static_cast<uint32_t>(DisplayAttributes::PosDefault))
	{
		data.y = 0;
	}
