...
bool ShellOS::initializeWindow(DisplayAttributes& data)
{
	...
	uint32_t valueList[32] = { _OSImplementation->screen->black_pixel, 0,
	...
	*/
	...
	xcb_create_window(_OSImplementation->connection, XCB_COPY_FROM_PARENT, _OSImplementation->window, _OSImplementation->screen->root, data.x, data.y, data.width, data.height, 0,
	...
	xcb_change_property(_OSImplementation->connection, XCB_PROP_MODE_REPLACE, _OSImplementation->window, (*reply).atom, 4, 32, 1, &(*_OSImplementation->atomWmDeleteWindow).atom);
	const char* title = "VulkanHelloAPI";
	xcb_change_property(_OSImplementation->connection, XCB_PROP_MODE_REPLACE, _OSImplementation->window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8, strlen(title), title);
