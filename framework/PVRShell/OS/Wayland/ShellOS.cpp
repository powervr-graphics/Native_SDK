...
struct InternalOS
{
	...
	Shell* pvrShell;
	InternalOS()
		: display(0), registry(nullptr), compositor(nullptr), shell(nullptr), seat(nullptr), pointer(nullptr), keyboard(nullptr), wayland_surface(nullptr), shellSurface(nullptr),
		  pvrShell(nullptr)
	{}
};
...
{
	if (keycode >= ARRAY_SIZE(Wayland_To_Keycode))
	{
		return Keys::Unknown;
	}
...
{
	_shell->updatePointerPosition(PointerLocation(static_cast<int16_t>(_OSImplementation->pointerXY[0]), static_cast<int16_t>(_OSImplementation->pointerXY[1])));
}
...
static const struct wl_seat_listener seatListener = { seatHandleCapabilities, seat_handle_name };
...
static void registerGlobalCallback(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version)
{
	...
	{
		internalOS.compositor = (wl_compositor*)wl_registry_bind(registry, name, &wl_compositor_interface, 1);
	}
	else if (strcmp(interface, "wl_shell") == 0)
	{
		internalOS.shell = (wl_shell*)wl_registry_bind(registry, name, &wl_shell_interface, 1);
	}
...
bool ShellOS::initializeWindow(DisplayAttributes& data)
{
	...
	if (!initWaylandConnection(*_OSImplementation))
		return false;
