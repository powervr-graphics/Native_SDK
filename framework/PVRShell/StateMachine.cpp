...
StateMachine::StateMachine(OSApplication instance, platform::CommandLineParser& commandLine, OSDATA osdata)
	: ShellOS(instance, osdata), _currentState(StateNotInitialized), _pause(false)
{
	_shellData.os = this;
...
void setColorBpp(Shell& shell, const char* arg, const char* val)
{
	...
	case 32:
		...
		break;
	default:
		Log(LogLevel::Warning, "PVRShell recognised command-line option 'set color bpp' set to unsupported value %hs. Supported values are (16, 24 and 32).", val);
		break;
	}
}
...
Result StateMachine::execute()
{
	...
	for (;;)
	{
		...
		{
			if (result == Result::ExitRenderFrame)
			{
				result = Result::Success;
			}
			while (_currentState != StateExit)
			{
				executeOnce();
			} // Loop to tidy up
		}
		if (_currentState == StateExit)
		{
			return result;
		}
	}
