...
void CommandLineParser::prefix(const CommandLineParser& commandLine)
{
	...
	CharBuffer newData;
	newData.resize(_data.size() + commandLine._data.size());
	...
	{
		newOptions[i].arg = (const char*)((size_t)commandLine._commandLine._options[i].arg - (size_t)commandLine._data.data()) + (size_t)newData.data();
		newOptions[i].val = (const char*)((size_t)commandLine._commandLine._options[i].val - (size_t)commandLine._data.data()) + (size_t)newData.data();
	}

	for (unsigned int i = 0; i < _commandLine._options.size(); ++i)
	{
		newOptions[commandLine._commandLine._options.size() + i].arg =
			(const char*)((size_t)_commandLine._options[i].arg - (size_t)_data.data()) + (size_t)newData.data() + commandLine._data.size();
		newOptions[commandLine._commandLine._options.size() + i].val =
			(const char*)((size_t)_commandLine._options[i].val - (size_t)_data.data()) + (size_t)newData.data() + commandLine._data.size();
	}
...
void CommandLineParser::append(const CommandLineParser& commandLine)
{
	...
	for (unsigned int i = 0; i < commandLine._commandLine._options.size(); ++i)
	{
		newOptions[_commandLine._options.size() + i].arg =
			(const char*)((size_t)commandLine._commandLine._options[i].arg - (size_t)commandLine._data.data()) + (size_t)newData.data() + _data.size();
		newOptions[_commandLine._options.size() + i].val =
			(const char*)((size_t)commandLine._commandLine._options[i].val - (size_t)commandLine._data.data()) + (size_t)newData.data() + _data.size();
	}
...
const CommandLineParser::ParsedCommandLine::Options& CommandLineParser::ParsedCommandLine::getOptionsList() const
{
	return _options;
}
} // namespace platform
