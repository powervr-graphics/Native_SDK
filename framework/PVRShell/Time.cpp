...
Time::Time()
{
	...
	{
		// This is a rough estimate of GetTickCount64's frequency which is supposed to be sampled once between every 10-16ms, but there's no obvious way to query what the actual
		// frequency is. As this is a fall back method, it's probably not too much of an issue.
...
uint64_t Time::getCurrentTimeSecs() const
{
	...
	{
		retTime = static_cast<double>(GetTickCount64()) / 1000.0;
	}
